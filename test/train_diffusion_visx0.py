...
if __name__ == "__main__":
    ...
    mask = (train_cond.abs() - 1).abs() < 0.1
    num_resample = mask.sum()
    while num_resample > 0:
        train_cond[mask] = torch.randn(num_resample)
        mask = (train_cond.abs() - 1).abs() < 0.1
        num_resample = mask.sum()
    ...
    for k, i in enumerate(indices):
        # plt.plot(intermediate_z[bid][:, 0], intermediate_z[bid][:, 1], color='black', alpha=0.5)
        start = z0
        end = intermediate_z_x0[k]

        # plt.plot(z0[:, 0], z0[:, 1], color='black', alpha=0.5)
