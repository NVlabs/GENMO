...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        for batch in dataloader:
            ...
            # t, t_weights = schedule_sampler.sample(z0.shape[0], z0.device)
            t = (torch.rand(z0.shape[0], device=z0.device) * 1000).long()
            t = t.to(device)
            t_init = (torch.ones_like(t) * 999).long()

            # z_t_init = torch.randn_like(z1)
            pred_z1_init = model(z0, t_init, cond)
            loss_init = ((pred_z1_init - z1) ** 2).mean()
    ...
    if test_mode == "diffusion":
        ...
        with torch.no_grad():
            ...
            for k, i in enumerate(indices):
                ...
                alpha_bar = _extract_into_tensor(
                    test_diffusion.alphas_cumprod, t, z0.shape
                )
        ...
        with torch.no_grad():
            ...
            # cond = torch.cat([cond1, cond2], dim=0)
            cond = (
                torch.arange(-1, 1, 1.0 / (noise.shape[0] // 2))
                .to(device)
                .reshape(-1, 1)
            )
            indices = list(range(test_diffusion.num_timesteps - 1))[::-1]
        ...
        plt.legend()
        plt.savefig("dm_reg_diffusion.png")
        torch.save(model.state_dict(), "dm_reg_diffusion.pth")
        torch.save(cond, "dm_reg_diffusion_cond.pth")
    elif test_mode == "reg":
        model.eval()
        with torch.no_grad():
            z0 = torch.randn(1000, 2).to(device)
        ...
        with torch.no_grad():
            ...
            t, t_weights = schedule_sampler.sample(noise.shape[0], noise.device)
            t = (torch.ones_like(t) * 999).long()
            x = model(x, t, cond)

            pred = x.cpu().numpy()
            err = ((pred[:, :1] - 5 * cond.cpu().numpy()) ** 2).mean() * 1000
            print(f"Error DM - Reg: {err}")
        ...
        plt.legend()
        plt.savefig("dm_reg_reg.png")
        torch.save(model.state_dict(), "dm_reg_reg.pth")
        torch.save(cond, "dm_reg_reg_cond.pth")
