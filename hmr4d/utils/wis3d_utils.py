...
def get_gradient_colors(scheme="red", num_points=120, alpha=1.0):
    ...
    colors = torch.stack(
        [torch.linspace(s, e, steps=num_points) for s, e in zip(start_rgba, end_rgba)],
        dim=-1,
    )
    return colors
...
def get_colors_by_conf(conf, low="red", high="green"):
    ...
    colors = colors * torch.tensor(color_schemes[high][1]) + (
        1 - colors
    ) * torch.tensor(color_schemes[low][1])
    return colors
...
def convert_motion_as_line_mesh(motion, skeleton_type="smpl22", const_color=None):
    ...
    for chain, color_name in zip(kinematic_chain, color_names):
        ...
        color_ = get_const_colors(
            color_name, partial_shape=(length, num_line), alpha=1.0
        ).to(device)  # (L, 4, 4)
    ...
    for f in range(length):
        vertices_, faces, vertex_colors = create_skeleton_mesh(
            s_points[f], e_points[f], radius=0.02, color=m_colors[f]
        )
...
def add_motion_as_lines(
    motion, wis3d, name="joints22", skeleton_type="smpl22", const_color=None, offset=0
):
    """
    Args:
        motion (tensor): (L, J, 3)
    ...
    for f in range(len(vertices)):
        ...
        wis3d.add_mesh(
            vertices[f], faces, vertex_colors, name=name
...
def add_prog_motion_as_lines(motion, wis3d, name="joints22", skeleton_type="smpl22"):
    ...
    for chain, color_name in zip(kinematic_chain, color_names):
        ...
        color_ = color_[None, :, None, :].repeat(
            P, 1, num_line, 1
        )  # (P, L, num_line, 4)
...
def add_joints_motion_as_spheres(
    joints, wis3d, radius=0.05, name="joints", label_each_joint=False
):
...
def create_skeleton_mesh(p1, p2, radius, color, resolution=4, return_merged=True):
    ...
    x_vec = (
        torch.tensor([1, 0, 0], device=p1.device).float().unsqueeze(0).repeat(N, 1)
    )  # (N, 3)
    ...
    ortho_vec = torch.where(
        ortho_vec.norm(dim=-1, keepdim=True) > 1e-3, ortho_vec, ortho_vec_
    )
    ...
    rotation_matrix = axis_angle_to_matrix(
        unit_seg_dir[:, None] * theta[None, :, None]
    )  # (N, Q, 3, 3)
    rotated_points = (
        einsum(rotation_matrix, unit_ortho_vec, "n q i j, n i -> n q j") * radius
    )  # (N, Q, 3)
    ...
    # outside face
    face_bottom = torch.stack(
        [
            bottom_indices[:-2],
            bottom_indices[1:-1],
            bottom_indices[-1].repeat(resolution - 2),
        ],
        1,
    )
    face_top = torch.stack(
        [top_indices[1:-1], top_indices[:-2], top_indices[-1].repeat(resolution - 2)], 1
    )
    faces = torch.cat(
        [
            torch.stack(
                [bottom_indices[1:], bottom_indices[:-1], top_indices[:-1]], 1
            ),  # out face
            torch.stack(
                [bottom_indices[1:], top_indices[:-1], top_indices[1:]], 1
            ),  # out face
            face_bottom,
            face_top,
        ]
    ...
    if return_merged:
        ...
        faces = (
            faces + torch.arange(0, N, device=p1.device).unsqueeze(1).unsqueeze(1) * V
        )
...
def draw_colored_vec(wis3d, vec, name, radius=0.02, colors="r", starts=None, l=1.0):
    ...
    vertices, faces, vertex_colors = create_skeleton_mesh(
        starts, ends, radius, color_tensor, resolution=10
    )
...
def create_checkerboard_mesh(y=0.0, grid_size=1.0, bounds=((-3, -3), (3, 3))):
    ...
    for i, x in enumerate(np.arange(min_x, max_x, grid_size)):
        for j, z in enumerate(np.arange(min_z, max_z, grid_size)):
            ...
            vertices.extend(
                [v1, v2, v3, v4, v1 + offset, v2 + offset, v3 + offset, v4 + offset]
            )
            idx = len(vertices) - 8
            faces.extend(
                [
    ...
    assert (
        len(vertex_colors.shape) == 2
        and vertex_colors.shape[1] == 3
        and vertex_colors.dtype == np.uint8
    )
...
def add_a_trimesh(mesh, wis3d, name):
    ...
    export_dir = (
        Path(wis3d.out_folder)
        / wis3d.sequence_name
        / f"{wis3d.scene_id:05d}"
        / "meshes"
    )
