...
class MinimalLBS(nn.Module):
    def __init__(
        self, sp_ids, bm_dir="models/smplh", num_betas=16, model_type="smplh", **kwargs
    ):
        super().__init__()
    ...
    def load_struct_on_sp(self, bm_path, prefix="m"):
        ...
        # shapedirs
        shapedirs = to_tensor(
            to_np(data_struct.shapedirs[:, :, :num_betas])
        )  # (V, 3, NB)
    ...
    def forward(
        ...
        pose_offsets = torch.matmul(pose_feature, posedirs[:, :dim_pf]).view(
            B, T, -1, 3
        )
        ...
        # 4. Compute A
        if recompute_A:
            _, _, A = forward_kinematics_motion(
                root_orient, pose_body, trans, joints_zero
            )
