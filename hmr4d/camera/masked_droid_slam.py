import sys
sys.path.insert(0, 'third-party/DROID-SLAM/droid_slam')
sys.path.insert(0, 'third-party/DROID-SLAM')
...
def run_metric_slam(img_folder, masks=None, calib=None, is_static=False):
    ...
    #### Interpolate Scale for all frames ####
    tot = len(imgfiles)
    key_scales = np.array(scales_)

    # Create a linear interpolation function
    interp_func = interp1d(tstamp, key_scales, kind='linear', fill_value="extrapolate")

    # Interpolate scale for all frames
    all_scales = interp_func(np.arange(tot))
    ...
    return pred_cam_r, pred_cam_t, all_scales, scale
