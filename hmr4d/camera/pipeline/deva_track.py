...
sys.path.insert(0, "third-party/Tracking-Anything-with-DEVA")
...
args = [
    ...
    "480",
    "--model",
    "inputs/checkpoints/DEVA/DEVA-propagation.pth",
...
def transform_masks(masks, scores, new_h, new_w):
    ...
    for i in np.flip(np.argsort(area).tolist()):
        ...
        mask = F.interpolate(
            mask.float().unsqueeze(0).unsqueeze(0), (new_h, new_w), mode="bilinear"
        )[0, 0]
        mask = (mask > 0.5).float()
        ...
        if mask.sum() > 0:
            ...
            segments_info.append(
                ObjectInfo(id=curr_id, category_id=None, score=confidence)
            )
            curr_id += 1
