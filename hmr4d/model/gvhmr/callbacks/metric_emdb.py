...
class MetricMocap(pl.Callback):
    ...
    def on_predict_batch_end(
        ...
        if False:  # wis3d debug
            ...
            add_motion_as_lines(
                pred_cr_j3d, wis3d, name="pred_cr_j3d", const_color="blue"
            )
            add_motion_as_lines(
                target_cr_j3d, wis3d, name="target_cr_j3d", const_color="green"
            )
        ...
        if False:  # Render incam
            ...
            images = read_video_np(
                video_path, scale=resize_factor
            )  # (F, H, W, 3), uint8, numpy
            frame_id = batch["meta_render"][0]["frame_id"]
            bbx_xys_render = batch["meta_render"][0]["bbx_xys"]
            metric_vis = rearrange_by_mask(
                torch.from_numpy(self.metric_aggregator["mpjpe"][vid]), mask
            )
            ...
            for i in tqdm(range(len(images)), desc=f"Rendering {vname}"):
                ...
                if metric_vis[i] > 0:
                    ...
                    text_color = (
                        (244, 10, 20) if metric_vis[i] > 80 else (0, 205, 0)
                    )  # red or green
                    cv2.putText(
                        img,
                        text,
        ...
        if False:  # Visualize incam + global results
            ...
            scale = (verts_glob.mean(1).max(0)[0] - verts_glob.mean(1).min(0)[0])[
                [0, 2]
            ].max() * 1.5
            ...
            for i in tqdm(range(seq_length), desc=f"Rendering {vname}"):
                ...
                if (
                    ...
                    img_overlay_pred = cv2.rectangle(
                        img_overlay_pred, lu_point, rd_point, (255, 178, 102), 2
                    )
...
MainStore.store(
    ...
    node=emdb1_node,
    group="callbacks",
    package="callbacks.metric_emdb1",
)
MainStore.store(
    name="metric_emdb2",
    node=emdb2_node,
    group="callbacks",
    package="callbacks.metric_emdb2",
)
MainStore.store(
    name="metric_emdb1_occ",
    node=emdb1_occ_node,
    group="callbacks",
    package="callbacks.metric_emdb1_occ",
)
MainStore.store(
    name="metric_emdb2_occ",
    node=emdb2_occ_node,
    group="callbacks",
    package="callbacks.metric_emdb2_occ",
)
