...
class MetricInpainting(pl.Callback):
    ...
    def on_predict_batch_end(
        ...
        target_j3d_can = canonicalize(
            target_w_j3d.cpu(), target_w_params["global_orient"].cpu()
        ).cuda()
        ...
        for res_idx, out in enumerate(outputs):
            ...
            pred_j3d_can = canonicalize(
                pred_ay_j3d.cpu(), pred_smpl_params_global["global_orient"].cpu()
            ).cuda()
            # Hack: aligns the root position at the first frame with GT
            pred_j3d_can[:, :] = (
                pred_j3d_can[:, :] + target_j3d_can[[0], [0]] - pred_j3d_can[[0], [0]]
            )

            # Compute metrics
            if trainer.model.model_cfg.inpainting_3d.mode.startswith(
                "body_pose_root_rot_keyframe"
            ):
                keyframes = batch["keyframes"]
                pred_kf_j3d_relative = (
                    pred_j3d_can[keyframes[1:]] - pred_j3d_can[keyframes[1:], :1]
                )
                target_kf_j3d_relative = (
                    target_j3d_can[keyframes[1:]] - target_j3d_can[keyframes[1:], :1]
                )
                mpjpe = (pred_kf_j3d_relative - target_kf_j3d_relative).norm(
                    dim=-1
                ).mean() * 1000
                pred_kf_j3d_relative_pa = batch_compute_similarity_transform_torch(
                    pred_kf_j3d_relative, target_kf_j3d_relative
                )
                pa_mpjpe = (pred_kf_j3d_relative_pa - target_kf_j3d_relative).norm(
                    dim=-1
                ).mean() * 1000
        ...
        if self.vis:
            ...
            if trainer.global_rank == 0 and self.num_val % self.vis_every_n_val == 0:
                wandb_dict = visualize_smpl_scene(
                    f"vis_inpainting_3d_{trainer.model.model_cfg.inpainting_3d.mode}_{self.test_start_time}",
                    batch_idx,
                    f"{res_idx:02d}-{vid}",
                    pred_j3d_can_best,
                    target_j3d_can,
    ...
    def on_predict_epoch_end(self, trainer, pl_module):
        ...
        for metric_name in sorted(self.metrics):
            ...
            pl_module.logger.log_metrics(
                {f"metrics/{metric_name}": metric_avg}, step=pl_module.current_epoch
            )
