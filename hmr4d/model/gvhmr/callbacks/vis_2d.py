        ...
        for ind in range(mv2d.shape[0]):
            ...
            for t in range(mv2d.shape[1]):
                ...
                for key in results:
                    highlight_view = (
                        input_view_id
                        if key in {"obs", "mv2d_shuffle", "mv2d_sv"}
                        else None
                    )
                    mv2d_proj = draw_mv_imgs(
                        results[key][ind, t],
                        coco_joint_parents,
                        self.img_w,
                        self.img_h,
            ...
            if trainer.global_rank == 0:
                ...
                if isinstance(wandb.run, wandb.sdk.wandb_run.Run):
                    pl_module.logger.log_metrics(
                        {f"b{batch_idx:03d}_i{ind:02d}": wandb.Video(vid_file)}
                    )
...
MainStore.store(
    name="vis_2d", node=node_vis2d, group="callbacks", package="callbacks.vis_2d"
)
