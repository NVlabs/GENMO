...
def pp_static_joint(outputs, endecoder: EnDecoder, smpl_key="pred_smpl_params_global"):
    ...
    # Overwrite results:
    if False:  # for-loop
        post_w_transl = outputs["pred_smpl_params_global"][
            "transl"
        ].clone()  # (B, L, 3)
        for i in range(1, L):
        ...
        post_w_transl = torch.cumsum(
            torch.cat([pred_w_transl[:, :1], pred_w_disp_new], dim=1), dim=1
        )
...
def pp_static_joint_cam(outputs, endecoder: EnDecoder):
    ...
    pred_smpl_params_incam["transl"] = gaussian_smooth(
        pred_smpl_params_incam["transl"], sigma=5, dim=-2
    )
    pred_c_j3d = endecoder.fk_v2(**pred_smpl_params_incam)  # (B, L, J, 3)

    # compute T_c2w (static) from first frame
    R_gv = axis_angle_to_matrix(
        pred_smpl_params_global["global_orient"][:, 0]
    )  # (B, 3, 3)
    R_c = axis_angle_to_matrix(
        pred_smpl_params_incam["global_orient"][:, 0]
    )  # (B, 3, 3)
    R_c2w = R_gv @ R_c.mT  # (B, 3, 3)
    t_c2w = pred_w_j3d[:, 0, 0] - torch.einsum(
        "bij,bj->bi", R_c2w, pred_c_j3d[:, 0, 0]
    )  # (B, 3)
    ...
    cp_thr = torch.tensor([0.25, 0.25, 0.25]).to(
        post_w_j3d
    )  # Only update very bad pred
    for i in range(1, L):
...
@autocast(enabled=False)
def process_ik(
    outputs, endecoder, static_conf=None, smpl_key="pred_smpl_params_global"
):
    if static_conf is None:
        static_conf = outputs["static_conf_logits"].sigmoid()  # (B, L, J)
    post_w_j3d, local_mat, post_w_mat = endecoder.fk_v2(
        **outputs[smpl_key], get_intermediate=True
    )
    ...
    body_pose = matrix_to_axis_angle(
        matrix.get_rotation(local_mat[:, :, 1:])
    )  # (B, L, J-1, 3, 3)
