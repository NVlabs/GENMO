...
class UNIMFM(pl.LightningModule):
    ...
    def create_condition_mask(self, batch, cond_mask_cfg, mode):
        ...
        if (
            ...
            if mask_img_prob > 0:
                mask_img = has_text & (torch.rand(batch["B"]) < mask_img_prob).to(
                    device
                )
            ...
            if mask_cam_prob > 0:
                mask_cam = has_text & (torch.rand(batch["B"]) < mask_cam_prob).to(
                    device
                )
    ...
    def validation_3d(self, batch, batch_idx, dataloader_idx=0):
        ...
        if "inpainting_3d" in self.model_cfg:
            ...
            if self.model_cfg['inpainting_3d']['mode'] == 'body_pose_dense':
                ...
                keyframes = [0, np.random.choice(keyframes[L//2:], 1)[0]]
                ...
                raise ValueError(f"Unknown inpainting mode [{self.model_cfg['inpainting_3d']['mode']}]")
