        ...
        if not train:
            ...
            if self.endecoder.encode_type == 'gvhmr':
                if self.args.get('infer_version', 2) == 2:
                    ...
                    pred_smpl_params_global, pred_T_w2c, pred_smpl_params_body_pose, static_conf_logits = get_smpl_params_w_Rt_v3(  # This function has for-loop
                    ...
                    outputs["pred_smpl_params_global"]["body_pose"] = pred_smpl_params_body_pose
                    outputs["pred_smpl_params_incam"]["body_pose"] = pred_smpl_params_body_pose
            else:
                outputs["pred_smpl_params_global"] = {
                    "body_pose": decode_dict["body_pose"],
                    "betas": decode_dict["betas"],
...
def merge_slam_w_hmr(w_j3d_slam, w_j3d_hmr, t_w_slam, t_w_hmr, static_conf_logits, static_label_, conf_scales):
    is_static = static_label_.sum(dim=-1) > 0  # (B, L-1)
    joint_ids = [7, 10, 8, 11, 20, 21]  # [L_Ankle, L_foot, R_Ankle, R_foot, L_wrist, R_wrist]
    ...
    post_w_transl_hmr[..., 0] = gaussian_smooth(post_w_transl_hmr[..., 0], sigma=5, dim=-1)
    post_w_transl_hmr[..., 1] = gaussian_smooth(post_w_transl_hmr[..., 1], sigma=5, dim=-1)
    post_w_transl_hmr[..., 2] = gaussian_smooth(post_w_transl_hmr[..., 2], sigma=5, dim=-1)
