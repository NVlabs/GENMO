...
class Pipeline(nn.Module):
    ...
    def forward(self, inputs, train=False, postproc=False, static_cam=False, global_step=0, mode=None):
        ...
        if mode == 'diffusion' and 'regression_outputs' in inputs:
            inputs['f_condition_valid_mask'] = inputs['regression_outputs']['f_condition_valid_mask']
            inputs['f_condition'] = inputs['regression_outputs']['f_condition']
            inputs['clean_f_condition'] = inputs['regression_outputs']['clean_f_condition']
        else:
            f_condition = dict()
            ...
            outputs["f_condition_valid_mask"] = inputs["f_condition_valid_mask"] = f_condition_valid_mask
            outputs["f_condition"] = inputs["f_condition"] = f_condition
            outputs["clean_f_condition"] = inputs["clean_f_condition"] = clean_f_condition
