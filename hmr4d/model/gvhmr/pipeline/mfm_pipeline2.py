...
class Pipeline(nn.Module):
    def __init__(self, args, args_denoiser3d, **kwargs):
        ...
        if self.args.normalize_cam_angvel:
            ...
            self.register_buffer(
                "cam_tvel_mean", torch.tensor(cam_tvel_stats["mean"]), persistent=False
            )
            self.register_buffer(
                "cam_tvel_std", torch.tensor(cam_tvel_stats["std"]), persistent=False
            )
    ...
    def forward(
        ...
        for k in f_condition.keys():
            ...
            if train:
                ...
                mask = (
                    torch.rand(f_condition[k].shape[:2], device=f_condition[k].device)
                    < uncond_prob
                )
        ...
        model_output = self.denoiser3d(
            inputs, train=train, postproc=postproc, static_cam=static_cam
        ...
        if "pred_x_init" in model_output:
            ...
            decode_dict_init = self.endecoder.decode(
                model_output["pred_x_init"]
            )  # (B, L, C) -> dict
            outputs_init.update(
                {"model_output": model_output, "decode_dict": decode_dict_init}
            )
        ...
        if not train:
            if self.args.infer_version == 2:
                ...
                pred_smpl_params_global, pred_T_w2c = (
                ...
                raise NotImplementedError(
                    f"infer_version {self.args.infer_version} not implemented"
                )
        ...
        if "pred_x_start_init" in model_output:
            ...
            simple_loss_init = F.mse_loss(
                pred_x_start_init, target_x_start, reduction="none"
            )[:, :, -151:]
            mask_simple_init = (
                mask[:, :, None].expand(-1, -1, simple_loss_init.size(2)).clone()
            )  # (B, L, C)
            mask_simple_init[inputs["mask"]["spv_incam_only"], :, -9:] = (
                False  # 3dpw training
            )
            simple_loss_init = (
                simple_loss_init
        ...
        if "pred_x_start_drift_init" in model_output:
            ...
            simple_loss_drift_init = F.mse_loss(
                pred_x_start_drift_init,
                model_output["target_drift_init"],
                reduction="none",
            )[:, :, -151:]
            mask_simple_drift_init = (
                mask[:, :, None].expand(-1, -1, simple_loss_drift_init.size(2)).clone()
            )  # (B, L, C)
            mask_simple_drift_init[inputs["mask"]["spv_incam_only"], :, -9:] = (
                False  # 3dpw training
            )
            simple_loss_drift_init = (
                simple_loss_drift_init
                * mask_simple_drift_init
                * valid_loss_mask
                * t_weights[:, None, None]
        ...
        if "cam_t_vel" in self.args.out_attr:
            ...
            if "pred_cam_t_vel_init" in model_output:
                ...
                cam_t_vel_loss_init = (
                    F.mse_loss(pred_cam_t_init, target_cam_t) * self.weights.cam_t_vel
                )
        ...
        if "cam_scale" in self.args.out_attr:
            ...
            if "pred_cam_scale_init" in model_output:
                ...
                pred_cam_t_init = (
                    torch.cumsum(noisy_cam_tvel, dim=-2) * pred_cam_scale_init
                )
                cam_scale_loss_init = (
                    F.mse_loss(pred_cam_t_init, target_cam_t) * self.weights.cam_t_vel
                )
...
def compute_extra_incam_loss(inputs, outputs, ppl):
    ...
    if weights.cr_j3d > 0.0:
        ...
        cr_j3d_loss = (
            cr_j3d_loss * mask[..., None, None] * t_weights[..., None, None, None]
        ).mean()
    ...
    if weights.transl_c > 0.0:
        ...
        transl_c_loss = (
            transl_c_loss * mask[..., None] * valid_mask * t_weights[:, None, None]
        ).mean()
    ...
    if weights.cr_verts > 0:
        ...
        cr_vert_loss = (
            cr_vert_loss * mask[:, :, None, None] * t_weights[:, None, None, None]
        ).mean()
...
def compute_extra_global_loss(inputs, outputs, ppl):
    ...
    if weights.transl_w > 0:
        ...
        trans_w_loss = (
            trans_w_loss * mask[..., None] * t_weights[..., None, None]
        ).mean()
    ...
    if weights.static_conf_bce > 0:
        ...
        static_conf_loss = (
            static_conf_loss * mask[..., None] * t_weights[..., None, None]
        ).mean()
...
def get_smpl_params_w_Rt_v3(
    ...
    **kwargs,
    ...
    if True:
        ...
        body_pose_hmr = process_ik(
            hmr_out, endecoder, static_conf_logits.sigmoid().float()
        )
        ...
        post_w_transl_slam = merge_slam_w_hmr(
            pred_w_j3d_slam,
            pred_w_j3d_hmr,
            post_w_transl_slam,
            static_conf_logits,
...
def merge_slam_w_hmr(
    w_j3d_slam, w_j3d_hmr, t_w_slam, static_conf_logits, static_label_, conf_scales
):
    is_static = static_label_.sum(dim=-1) > 0  # (B, L-1)
    joint_ids = [
        7,
