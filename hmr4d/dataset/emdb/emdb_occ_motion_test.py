...
class EmdbOccSmplFullSeqDataset(data.Dataset):
    def __init__(self, split=1, flip_test=False):
        ...
        """
        super().__init__()
        self.dataset_name = "EMDB-OCC"
        self.split = split
        self.dataset_id = f"EMDB_{split}-OCC"
        ...
        self.labels = torch.load(self.emdb_dir / "emdb_occ_labels.pt")
    ...
    def _load_data(self, idx):
        ...
        # if enable flip_test
        if self.flip_test:
            imgfeat_dir = self.emdb_dir / "imgfeats/emdb_occ_flip"
...
MainStore.store(
    name="v1",
    node=builds(EmdbOccSmplFullSeqDataset, populate_full_signature=True),
    group="test_datasets/emdb1_occ",
)
MainStore.store(
    name="v1_fliptest",
    node=builds(EmdbOccSmplFullSeqDataset, flip_test=True, populate_full_signature=True),
    group="test_datasets/emdb1_occ",
)
MainStore.store(
    name="v1",
    node=builds(EmdbOccSmplFullSeqDataset, split=2, populate_full_signature=True),
    group="test_datasets/emdb2_occ",
)
MainStore.store(
    name="v1_fliptest",
    node=builds(EmdbOccSmplFullSeqDataset, split=2, flip_test=True, populate_full_signature=True),
    group="test_datasets/emdb2_occ",
)
