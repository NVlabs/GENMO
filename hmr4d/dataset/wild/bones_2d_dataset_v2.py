...
class Bones2DDatasetV2(KP2DDatasetV2):
    def __init__(
        ...
        split_folder="inputs/mv2d/splits/v1",
        num_keypoints=17,
        num_frames=120,
        num_data=None,
        split="train",
        ...
        use_orig_length=False,
        hand_leg_aug=False,
        precompute_data_folder="/lustre/fsw/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets/GVHMR/bones2d/v1",
        return_smpl=False,
        device="cpu",
        **kwargs,
    ):
        ...
        self.coco17_regressor = torch.load(
        ...
        if length > self.num_frames:
            ...
            for key in {"obs_kp2d", "mask", "conf"}:
                ...
                data[key] = np.concatenate(
                    [val, np.zeros((self.num_frames - val.shape[0],) + val.shape[1:])],
                    axis=0,
                )
    ...
    def __getitem__(self, item):
        if self.precompute:
            ...
            outputs = {
                ...
                "K_fullimg": self.cam_intrinsics.repeat(
                    self.num_frames, 1, 1
                ),  # (F, 3, 3)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index", type=int, default=0, help="Index of the dataset item to process"
    )
    parser.add_argument(
        "--num_jobs", type=int, default=2, help="Index of the dataset item to process"
    )
    ...
    """ test """
    dataset = Bones2DDatasetV2SingleView(
        use_orig_length=True, num_frames=250, device="cuda"
    )
    for i in range(100):
        data = dataset[i]
        print(i, data["idx"])
    ...
    """ gen data """
    dataset = Bones2DDatasetV2SingleView(
        use_orig_length=True, num_frames=800, device="cuda", precompute_data_folder=None
    )
    start = args.index * len(dataset) // args.num_jobs
    end = (
        ((args.index + 1) * len(dataset) // args.num_jobs)
        if args.index + 1 < args.num_jobs
        else len(dataset)
    )
