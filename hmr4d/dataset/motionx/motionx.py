...
class MotionXDataset(ImgfeatMotionDatasetBase):
    def __init__(
        ...
        self.text_embed_file = Path(
            "inputs/MotionXpp_ye/t5_embeddings_v1_half/all_text_embed.pth"
    ...
    def _load_dataset(self):
        ...
        self.train_labels = torch.load(
            self.root / f"motionxpp_smplxposev3_{self.split}.pth"
        )
    ...
    def _process_data(self, data, idx):
        ...
        if self.version == "v2d":
            ...
            return_data["obs_kp2d"] = repeat_to_max_len(
                torch.from_numpy(return_data["obs_kp2d"]), max_len
            ).numpy()
            return_data["conf"] = repeat_to_max_len(
                torch.from_numpy(return_data["conf"]), max_len
            ).numpy()
...
MainStore.store(
    name="v2d",
    node=builds(
        MotionXDataset, version="v2d", motion_start_mode="sample", split="train"
    ),
    group="train_2d_datasets/imgfeat_motionx",
)
MainStore.store(
    name="v2d_train",
    node=builds(
...
MainStore.store(
    name="v2d_val",
    node=builds(MotionXDataset, version="v2d", motion_start_mode="sample", split="val"),
    group="test_datasets/imgfeat_motionx",
)
MainStore.store(
    name="v2d_test",
    node=builds(
        MotionXDataset, version="v2d", motion_start_mode="sample", split="test"
    ),
    group="test_datasets/imgfeat_motionx",
)
MainStore.store(
...
MainStore.store(
    name="vlocal",
    node=builds(
        MotionXDataset, version="vlocal", motion_start_mode="sample", split="train"
    ),
    group="train_datasets/imgfeat_motionx",
)
MainStore.store(
    name="vlocal_train",
    node=builds(
...
MainStore.store(
    name="vlocal_val",
    node=builds(
        MotionXDataset, version="vlocal", motion_start_mode="sample", split="val"
    ),
    group="test_datasets/imgfeat_motionx",
)
MainStore.store(
    name="vlocal_test",
    node=builds(
        MotionXDataset, version="vlocal", motion_start_mode="sample", split="test"
    ),
    group="test_datasets/imgfeat_motionx",
)
...
MainStore.store(
    name="vglobal",
    node=builds(
        MotionXDataset, version="vglobal", motion_start_mode="sample", split="train"
    ),
    group="train_datasets/imgfeat_motionx",
)
MainStore.store(
...
MainStore.store(
    name="vglobal_val",
    node=builds(
        MotionXDataset, version="vglobal", motion_start_mode="sample", split="val"
    ),
    group="test_datasets/imgfeat_motionx",
)
MainStore.store(
    name="vglobal_test",
    node=builds(
        MotionXDataset, version="vglobal", motion_start_mode="sample", split="test"
    ),
    group="test_datasets/imgfeat_motionx",
)
