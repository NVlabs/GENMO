...
def perspective_projection(
    points, rotation, translation, focal_length, camera_center, distortion=None
):
    """
    ...
    if distortion is not None:
        ...
        dx = 2 * kc[:, [2]] * points[:, :, 0] * points[:, :, 1] + kc[:, [3]] * (
            r2 + 2 * points[:, :, 0] ** 2
        )

        dy = 2 * kc[:, [3]] * points[:, :, 0] * points[:, :, 1] + kc[:, [2]] * (
            r2 + 2 * points[:, :, 1] ** 2
        )

        x = (
            1 + kc[:, [0]] * r2 + kc[:, [1]] * r2.pow(2) + kc[:, [4]] * r2.pow(3)
        ) * points[:, :, 0] + dx
        y = (
            1 + kc[:, [0]] * r2 + kc[:, [1]] * r2.pow(2) + kc[:, [4]] * r2.pow(3)
        ) * points[:, :, 1] + dy
