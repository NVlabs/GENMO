...
from hmr4d.network.hmr2.utils.geometry import rot6d_to_rotmat
from hmr4d.dataset.track_dataset import TrackDataset

from .vit import vit_huge
from .modules import *
from .smpl import SMPL
from .tools import parse_chunks
...
class HMR_VIMO(nn.Module):
    ...
    def forward(self, batch, **kwargs):
        ...
        pred_pose, pred_shape, pred_cam, token_out = self.smpl_head(feature)
        ...
        feat_preds.append(out['pred_feats'].clone())
        iter_preds = [rotmat_preds, shape_preds, cam_preds, j3d_preds, j2d_preds, feat_preds]
    ...
    def inference(self, imgfiles, boxes, img_focal=None, img_center=None, valid=None, frame=None, device='cuda'):
        ...
        for frame_ck, boxes_ck in zip(frame_chunks, boxes_chunks):
            ...
            pred_feats.append(results['pred_feats'])
    ...
    def inference_chunk(self, imgfiles, boxes, img_focal, img_center, device='cuda'):
        ...
        for i in tqdm(range(len(db))):
            ...
            pred_feats.append(out['pred_feats'].cpu())
