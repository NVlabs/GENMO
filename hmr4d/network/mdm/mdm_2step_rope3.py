...
class MDMBase(nn.Module):
    ...
    def get_diffusion_pred_target(self, batch, target_x, static_gt):
        ...
        pred_x_start_init = denoise_out_init["pred_x_start"]
        ...
        inpaint_mask[batch["mask"]["spv_incam_only"], :, self.s_pred_ind:self.s_pred_ind+ 6] *= 0
        ...
        x_start = clean_motion.clone() * inpaint_mask + pred_x_start_init.detach() * (1 - inpaint_mask) * vis_mask[:, :, None]
        # x_start = pred_x_start_init.detach()
        ...
        if 'cam_t_vel' in self.args.out_attr:
            ...
            pred_cam_t_vel_init = denoise_out_init["pred_cam_t_vel"]

        if 'cam_scale' in self.args.out_attr:
            pred_cam_scale = denoise_out["pred_cam_scale"]
            pred_cam_scale_init = denoise_out_init["pred_cam_scale"]
