...
from hmr4d.network.base_arch.transformer.encoder_rope import (
    EncoderRoPEBlock as EncoderRoPEBlock_old,
)
...
class MDMDenoiserROPE(nn.Module):
    def __init__(
        ...
        **kargs,
        ...
        self.add_cond_linear = nn.Linear(
            self.input_feats * 2 + self.latent_dim, self.latent_dim
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if x.shape[2] == 6 + 9 + 151:
            ...
            cam_angvel = (
                x[:, :, :6] * (1 - motion_mask[:, :, :6])
                + observed_motion[:, :, :6] * motion_mask[:, :, :6]
            )
            f_to_add = self.cam_angvel_embedder(cam_angvel)
            assert self.s_pred_ind == 6, (
                f"s_pred_ind should be 6, but got {self.s_pred_ind}"
            )
            ...
            cam_vel = (
                x[:, :, 6:9] * (1 - motion_mask[:, :, 6:9])
                + observed_motion[:, :, 6:9] * motion_mask[:, :, 6:9]
            )
            f_to_add = self.cam_angvel_embedder(cam_angvel) + self.cam_vel_embedder(
                cam_vel
            )
            assert self.s_pred_ind == 9, (
                f"s_pred_ind should be 9, but got {self.s_pred_ind}"
            )
        ...
        if nframes > self.num_frames:
            ...
            attnmask_full = torch.ones(
                (nframes + pose_start_ind, nframes + pose_start_ind),
                device=x.device,
                dtype=torch.bool,
            )
        ...
        pmask = ~y["mask"]  # (B, L)
        pmask = torch.cat(
            [torch.zeros_like(pmask[:, :1]).repeat(1, pose_start_ind), pmask], dim=1
        )
        ...
        if True:
            ...
            betas = (output[..., s_ind + 126 : s_ind + 136] * (~pmask[..., None])).sum(
                1
            ) / length[:, None]  # (B, C)
            betas = repeat(betas, "b c -> b l c", l=L)
            output = torch.cat(
                [output[..., : s_ind + 126], betas, output[..., s_ind + 136 :]], dim=-1
            )
        ...
        if self.use_motion_mask and motion_mask is not None:
            if self.obs_motion_constraints == "hard":
                output[..., self.s_pred_ind :] = (
                    output[..., self.s_pred_ind :] * (1 - motion_mask)
