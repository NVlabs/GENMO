...
class MDMDenoiserROPE(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        xseq_start = self.sequence_pos_encoder(
            xseq_start, batch_first=True
        )  # [bs, seqlen+1, 2d]
        ...
        if self.use_motion_mask and motion_mask is not None:
            if self.obs_motion_constraints == "hard":
                sample[..., self.s_pred_ind :] = (
                    sample[..., self.s_pred_ind :] * (1 - motion_mask)
        ...
        if "cam_scale" in self.args.out_attr:
            ...
            pred_cam_scale = (pred_cam_scale * ~pmask[..., None]).sum(1) / length[
                :, None
            ]
            pred_cam_scale = repeat(pred_cam_scale, "b c -> b l c", l=L).clone()
            output["pred_cam_scale"] = torch.clamp_min_(
                pred_cam_scale, 0.1
            )  # min_clamp s to 0.1 (prevent negative prediction)
