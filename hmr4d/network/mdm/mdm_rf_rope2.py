...
class MDMBase(nn.Module):
    ...
    def get_diffusion_pred_target(self, batch, target_x, static_gt, gt_pred_cam=None, valid_cam_mask=None):
        ...
        # t_init = torch.zeros_like(t)
        t_init = torch.rand(motion.shape[0], device=motion.device) * (self.sde.T - self.sde.sampling_eps) + self.sde.sampling_eps
        t_expand_init = t_init.view(-1, 1, 1)
        z0_init = torch.randn_like(motion)
        # z0_init = torch.zeros_like(z0_init)  # initialize with zeros
        z1_init = motion.clone()
        zt_init = t_expand_init * z1_init + (1 - t_expand_init) * z0_init
        f_cond = f_cond * vis_mask[..., None]
        target_drift_init = z1_init - z0_init
        ...
        denoise_out_init = self.denoiser(
            zt_init, (t_init * 999).long(), return_aux=False, clip_cam=False, **denoiser_kwargs
        )

        if self.pred_mode == "drift":
            pred_x_start_drift_init = denoise_out_init["pred_x_start"]
            pred_x_start_init = zt_init + pred_x_start_drift_init * (1 - t_expand_init)
            ...
            pred_x_start_drift_init = denoise_out_init["pred_x_start"]
            pred_x_start_init = z0_init + pred_x_start_drift_init
        ...
        inpaint_mask = motion_mask
        # z1 = clean_motion.clone() * inpaint_mask + pred_x_start_init.detach() * (1 - inpaint_mask) * vis_mask[:, :, None]
        z1 = pred_x_start_init.detach() * valid_mask[:, :, None]
        ...
        if 'pred_cam' in self.args.out_attr:
            if self.pred_mode == "drift":
                ...
                torch.clamp_min_(pred_cam[..., 0], 0.25)
                pred_cam_init = denoise_out_init["pred_cam"]
                torch.clamp_min_(pred_cam_init[..., 0], 0.25)
        ...
        static_conf_logits_init = pred_x_start_init[:, :, self.s_pred_ind:self.s_pred_ind + 6]
        assert pred_x_start_init.shape[-1] == self.s_pred_ind + 6 + 151
        sample_init = pred_x_start_init[:, :, self.s_pred_ind + 6:]

        output = {
            "pred_x_start": pred_x_start,
            "pred_x_start_init": pred_x_start_init,
            "pred_x_start_drift_init": pred_x_start_drift_init,
            "target_drift_init": target_drift_init,
            "target_x_start": target_x_start,
            "pred_x_start_drift": pred_x_start_drift,
        ...
        if 'pred_cam' in self.args.out_attr:
            ...
            output["pred_cam_init"] = pred_cam_init
        if 'cam_t_vel' in self.args.out_attr:
            output["pred_cam_t_vel"] = pred_cam_t_vel
            output["pred_cam_t_vel_init"] = pred_cam_t_vel_init
        if 'cam_scale' in self.args.out_attr:
            output["pred_cam_scale"] = pred_cam_scale
            output["pred_cam_scale_init"] = pred_cam_scale_init
        return output
