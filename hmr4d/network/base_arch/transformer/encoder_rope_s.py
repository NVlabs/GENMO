...
class RoPEAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, key_padding_mask=None, pose_start_ind=0):
        # x: (B, L, C)
        # attn_mask: (L, L)
        ...
        if pose_start_ind > 0:
            xq_start = xq[:, :, :pose_start_ind]
            xq_core = xq[:, :, pose_start_ind:]
            xk_start = xk[:, :, :pose_start_ind]
            xk_core = xk[:, :, pose_start_ind:]

            xq_core = self.rope.rotate_queries_or_keys(xq_core)  # B, N, L, C
            xk_core = self.rope.rotate_queries_or_keys(xk_core)  # B, N, L, C

            xq = torch.cat((xq_start, xq_core), dim=2)
            xk = torch.cat((xk_start, xk_core), dim=2)
        else:
        ...
        if key_padding_mask is not None:
            key_padding_mask = key_padding_mask.reshape(B, 1, 1, L).expand(
...
class EncoderRoPEBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tgt_key_padding_mask=None, pose_start_ind=0):
    ...
    def _sa_block(self, x, attn_mask=None, key_padding_mask=None, pose_start_ind=0):
        # x: (B, L, C)
        x = self.attn(
            x,
            attn_mask=attn_mask,
