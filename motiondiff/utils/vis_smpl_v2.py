...
class SMPLActor():
    ...
    def __init__(self, pl, verts, faces, uv, color='#FF8A82', visible=True, tex_paths=[]):
        self.pl = pl
        self.verts = verts
        self.face = faces
        self.uv = uv
        ...
        if self.tex_list is not None:
            ...
            self.actor = self.pl.add_mesh(self.mesh, texture=self.tex_list[self.tex_ind], render=False)
        else:
            self.actor = self.pl.add_mesh(self.mesh, color=color, pbr=True, metallic=0.0, roughness=0.3, diffuse=1, render=False)
    ...
    def next_tex(self):
        if self.tex_list:
            ...
            if self.tex_list[self.tex_ind] is None: # if we're on no texture, need to change mesh
                self.pl.remove_actor(self.actor, render=False)
                self.actor = self.pl.add_mesh(self.mesh, color=self.color, pbr=True, metallic=0.0, roughness=0.3, diffuse=1, render=False)
...
class SkeletonActor():
    ...
    def __init__(self, pl, joint_parents, joint_color='green', bone_color='yellow', joint_radius=0.03, bone_radius=0.02, visible=True, name='skel1'):
        self.pl = pl
        ...
        for j, pa in enumerate(self.joint_parents):
            ...
            joint_actor = self.pl.add_mesh(joint_mesh, color=joint_color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True, render=False)
            joint_actor = ActorWrapper(joint_actor, j, 'joint', joint_mesh, self, joint_color, parent=self)
            ...
            if pa >= 0:
                ...
                bone_actor = self.pl.add_mesh(bone_mesh, color=bone_color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True, render=False)
                bone_actor = ActorWrapper(bone_actor, len(self.bone_actors), 'bone', bone_mesh, self, bone_color, parent=self)
...
class JointConstraints:
    ...
    def __init__(self, pos, frame, joint_actor, visualizer, color='purple', size=0.04, root_color='purple'):
        self.pos = pos
        self.frame = frame
        ...
        self.color = color
        self.mesh = pyvista.Sphere(radius=size, center=pos, theta_resolution=10, phi_resolution=10)
        actor = self.pl.add_mesh(self.mesh, color=color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True, render=False)
        self.actor = ActorWrapper(actor, self.ind, 'joint_constraint', self.mesh, self, color)
        if DEBUG_MODE:
            self.label = self.pl.add_point_labels(pos, [f"F{frame} {self.jname} ({pos[0]:.2f}, {pos[1]:.2f}, {pos[2]:.2f})"])
        ...
        parent_skel = np.array([joint_actor.parent.joint_actors[ji].pos for ji in range(len(joint_actor.parent.joint_actors))])
        ...
        self.root_mesh = pyvista.Sphere(radius=size, center=self.root_pos, theta_resolution=10, phi_resolution=10)
        root_actor = self.pl.add_mesh(self.root_mesh, color=root_color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True, render=False)
        self.root_actor = ActorWrapper(root_actor, root_idx, 'joint_constraint', self.root_mesh, self, root_color)
        if DEBUG_MODE:
            self.root_label = self.pl.add_point_labels(self.root_pos, [f"F{frame} {'Root'} ({self.root_pos[0]:.2f}, {self.root_pos[1]:.2f}, {self.root_pos[2]:.2f})"])
...
class BodyConstraints:
    ...
    def __init__(self, frame, skel_actor, visualizer, joint_color='purple', bone_color='purple', joint_radius=0.04, bone_radius=0.03):
        self.frame = frame
        ...
        # assumes skeleton is currently posed for the timestep "frame"
        self.jpos = np.array([skel_actor.joint_actors[ji].pos for ji in range(len(skel_actor.joint_actors))])
        self.viz_actor = SkeletonActor(self.pl, visualizer.smpl_joint_parents, 
        ...
        non_root_inds = np.concatenate([np.arange(root_idx), np.arange(root_idx+1, self.jpos.shape[0])])
        self.jpos = self.jpos[non_root_inds]

        if DEBUG_MODE:
            self.label = self.pl.add_point_labels(self.root_pos, [f"F{frame} full-body"])
...
class TrajWaypoint:
    ...
    def __init__(self, pos, frame, visualizer, color, size, line_color, line_size, prev_waypoint=None, add_label=True, add_point_actor=True, add_disc_actor=False):
        self.pos = pos
        self.frame = frame
        self.visualizer = visualizer
        self.pl = visualizer.pl
        ...
        if add_point_actor:
            self.mesh = pyvista.Sphere(radius=size, center=pos, theta_resolution=10, phi_resolution=10)
            actor = self.pl.add_mesh(self.mesh, color=color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True, render=False)
            self.actor = ActorWrapper(actor, -1, 'traj_waypoint', self.mesh, self, color)
        if add_disc_actor:
            disc_pos = copy.copy(pos)
            disc_pos[2] = 0.01 # float off ground slightly so visible
            self.disc_mesh = pyvista.Disc(center=disc_pos, inner=size+0.1, outer=size+0.2, c_res=30)
            disc_actor = self.pl.add_mesh(self.disc_mesh, color=color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True, render=False)
            self.disc_actor = ActorWrapper(disc_actor, -1, 'traj_waypoint', self.disc_mesh, self, color)
        self.line_mesh_actor = None
        if prev_waypoint is not None:
            self.line_mesh = pyvista.Cylinder(radius=line_size, center=(0, 0, 0), direction=(0, 0, 1), resolution=30)
            self.line_mesh_actor = self.pl.add_mesh(self.line_mesh, color=line_color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True, opacity=0.0, render=False)
            self.update_line_transform(prev_waypoint.pos, pos)
        if add_label and DEBUG_MODE:
            self.label = self.pl.add_point_labels(pos, [f"F{self.frame} ({pos[0]:.2f}, {pos[1]:.2f})"])
        else:
            self.label = None
...
class RootTrajConstraints:
    ...
    def __init__(self, visualizer, waypt_only=False, point_color='cyan', point_size=0.03, line_color='cyan', line_size=0.02, time_cmap='cool'):
        self.visualizer = visualizer
    ...
    def add_way_point(self, pos, frame, threshold=0.02, is_spline_waypoint=False):
        dist = np.linalg.norm(pos - self.way_points[-1].pos) if len(self.way_points) > 0 else 10.0
        if dist < threshold:
            return
        ...
        if is_spline_waypoint:
            self.way_points.append(TrajWaypoint(pos, frame, self.visualizer, cur_color, self.point_size, cur_color, self.line_size, prev_waypoint, add_label=False, add_point_actor=False))
        elif self.waypt_only:
            self.way_points.append(TrajWaypoint(pos, frame, self.visualizer, cur_color, self.point_size, cur_color, self.line_size, None, add_disc_actor=True))
        else:
            self.way_points.append(TrajWaypoint(pos, frame, self.visualizer, cur_color, self.point_size, cur_color, self.line_size, prev_waypoint))
...
class ConstraintConfigHandler:
    ...
    def add_config(self, constraint_name):
        ...
        for hyperparam, info in settings.items():
            ...
            num_configs = self.num_model_configs if constraint_name in self.model_configs \
                                                    else self.num_guidance_configs
            yloc = 0.87 - num_configs*0.13
            xloc = (0.73, 0.95) if constraint_name in self.model_configs else (0.03, 0.25)

            if numer_type == 'bool':
                # checkbox
                pos = [self.visualizer.pl.window_size[0] * xloc[0], self.visualizer.pl.window_size[1]*yloc]
                ...
                check_label = self.visualizer.pl.add_text(label_str, position=pos, font_size=18, color='black')
    ...
    def reset_configs(self):
        ...
        if self.visualizer.timeline:
            self.visualizer.pl.slider_widgets = [self.visualizer.timeline] # don't want to delete the timeline
...
class SMPLVisualizer(Visualizer3D):
    ...
    def __init__(self, smpl_model_dir=None, generator_func=None, constraint_config=None, device=torch.device('cpu'), show_smpl=SMPL, show_skeleton=True, **kwargs):
        ...
        self.smpl_uv = np.load(os.path.join(smpl_model_dir, 'cont_uv_map', 'uv_table.npy'))
        para_file = os.path.join(smpl_model_dir, 'cont_uv_map', 'paras_h0256_w0256_SMPL.npz')
    ...
    def update_smpl_seq(self, smpl_seq=None, mode='default'):
        ...
        for i, (skel_name, pose_dict) in enumerate(smpl_seq.items()):
            ...
            pose_dict['skeleton_jpos'] = SkeletonActor(self.pl, self.smpl_joint_parents, joint_color=colors[0], bone_color=colors[1], name=skel_name)
            ...
            if self.show_smpl and 'pose' in pose_dict and 'trans' in pose_dict:
                ...
                pose_dict['smpl_actor'] = SMPLActor(self.pl, smpl_verts[0], self.smpl_faces, self.smpl_uv, color="#e0c3fc", visible=False, tex_paths=self.smpl_tex)
    ...
    def calc_smpl_vert_seq(self, pose_dict):
        ...
        smpl_verts = smpl_motion.vertices[:, self.smpl_vt2v].cpu().numpy() #.reshape(*orig_pose_shape[:-1], -1, 3)
        return smpl_verts
    ...
    def add_skel_vis_checkboxes(self, smpl_seq=None):
        ...
        for i, skel_name in enumerate(smpl_seq.keys()):
            def create_box(box_name):
                colors = self.color_sequences[i % len(self.color_sequences)]
                pos = [self.pl.window_size[0] * 0.4, self.pl.window_size[1]*0.85 - 0.05*self.pl.window_size[1]*i]
                ...
                if self.show_smpl:
                    ...
                    pos[0] += 60.0
                self.text_actors.append(self.pl.add_text(NAME_MAP[box_name], position=pos, font_size=18, color=colors[1]))
    ...
    def init_scene(self, init_args):
        ...
        self.text_constr_actor = self.pl.add_text("", position='lower_edge', font='courier', font_size=20, color='black')

        # text to elicit user input prompt
        self.text_in_actor = self.pl.add_text("", position=(0.15, 0.5), viewport=True,
                                              font='arial', font_size=28, color='black', shadow=True, render=False)

        self.config_handler.init_configs()

        self.fps_text_actor = self.pl.add_text(FPS_TEXT % (self.fps), position='upper_right')
        self.select_mode_text_actor = self.pl.add_text(SELECTION_TEXT % (PICKING_NAME_MAP[self.picking_mode]), position='upper_left')
        self.text_actors += [self.text_constr_actor, self.fps_text_actor, self.select_mode_text_actor] 
    ...
    def enable_actor_picking(self):
        ...
        self.pl.enable_mesh_picking(callback=callbacks[self.picking_mode], use_actor=True, left_clicking=True, show=False, show_message=False)
    ...
    def enable_surface_picking(self):
        ...
        self.pl.enable_surface_picking(callback=callback_surface, left_clicking=True, show_point=False, show_message=False)
        self.set_all_unpickable()
        self.pl.pickable_actors = [self.floor_actor]

    def add_path_constraints(self, point, is_spline=False, waypt_only=False, frame=None):
        if self.root_traj_constraint is None:
            self.root_traj_constraint = RootTrajConstraints(self, waypt_only=waypt_only, point_color='cyan', point_size=0.03, line_color='purple', line_size=0.02)
            if not is_spline and not waypt_only:
                self.root_traj_constraint.add_way_point(np.array([0, 0, 0]), 0) # at frame 0
    
        if frame is None:
            frame = self.fr
        if is_spline:
            self.root_traj_constraint.add_way_point(point, frame, threshold=0, is_spline_waypoint=True)
        else:
    ...
    def create_spline_path(self):
        if self.root_traj_constraint is not None:
            ...
            frame_ind = np.array([x.frame for x in self.root_traj_constraint.way_points])
            ...
            dense_frame_indices = np.arange(self.orig_num_fr)  # This creates an array from the first to the last frame index
            ...
            for i, wp in enumerate(dense_waypoints):
                self.add_path_constraints(wp, frame=dense_frame_indices[i], is_spline=True)
        return
    ...
    def add_joint_constraints(self):
        if self.picked_actor is not None:
            if isinstance(self.picked_actor, SkeletonActor):
                ...
                new_joint_constr = JointConstraints(self.picked_actor.pos, self.fr, self.picked_actor, self)
    ...
    def update_scene(self):
        ...
        if self.show_skeleton:
            for skel_name, pose_dict in self.smpl_seq.items():
                pose_dict['skeleton_jpos'].update_joints(pose_dict['joints_pos'][min(self.fr, pose_dict['joints_pos'].shape[0]-1)].cpu().numpy())
        if self.show_smpl:
            for skel_name, pose_dict in self.smpl_seq.items():
                pose_dict['smpl_actor'].update_verts(pose_dict['vert_seq'][min(self.fr, pose_dict['vert_seq'].shape[0]-1)])
    ...
    def prepare_constraints(self):
        ...
        if self.root_traj_constraint:
            ...
            if self.root_traj_constraint.waypt_only:
                ...
                interpf = interp1d(frame_ind, wp, axis=0, assume_sorted=True, fill_value="extrapolate")
                root_traj = interpf(np.arange(self.orig_num_fr))
                # root_traj = interp_scipy_ndarray(wp, new_len=self.orig_num_fr, dim=0)
            root_traj_xy = np.stack([root_traj[:, 1], root_traj[:, 0]], axis=-1)     # switch x, y (go from UI frame to model frame) -- TODO should move this to the test_gui 
    ...
    def hide_ui(self):
        all_text = [text for text in self.text_actors if text != self.text_constr_actor] # want to keep this on for visualization
    ...
    def setup_key_callback(self):
        ...
        def update_motion_constr():
            gen_txt = self.pl.add_text("Generating...", position=(0.38, 0.5), viewport=True, font_size=32, color='red', shadow=True)
            self.update_motion_from_constraints()
            self.pl.remove_actor(gen_txt, render=False)

        def toggle_picking_mode():
            picking_modes = ['actor_joint', 'actor_body', 'surface_root_traj', 'surface_root_key']
            cur_picking_mode = self.picking_mode
            self.picking_mode = picking_modes[(picking_modes.index(self.picking_mode) + 1) % len(picking_modes)]
            self.select_mode_text_actor.SetText(2, SELECTION_TEXT % (PICKING_NAME_MAP[self.picking_mode]))
            self.reset_picked_actors()
        ...
        def new_gt_motion():
            gen_txt = self.pl.add_text("Loading next\nmotion...", position=(0.38, 0.5), viewport=True, font_size=32, color='red', shadow=True)
            self.reset_constraints()
