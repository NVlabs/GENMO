...
from motiondiff.data_pipeline.bones.motion_process import recover_from_ric_264, recover_from_ric, recover_from_ric_with_joint_rot
...
class Metrics():
    def __init__(self, motion_mean, motion_std, motion_global_mean, motion_global_std,
                  normalize_global_pos=True, pred_samples_rep='global_root_local_joints',
                  metrics=['foot_skate'],
                  neutral_joints=None,
                  joint_parents=None):
    ...
    def preprocess(self, samples):
        ...
        elif self.pred_samples_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
        ...
        if self.pred_samples_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
            ...
            if self.pred_samples_rep == 'global_root_local_joints_root_rot':
                global_joint_pos, global_rot_quat = recover_from_ric_with_joint_rot(samples, self.neutral_joints, self.joint_parents, 
                                                                                    root_dim=5, r_rot_quat=pred_r_rot_quat, r_pos=pred_r_pos, return_r_rot=True)
            else:
                global_joint_pos, global_rot_quat = recover_from_ric_264(samples, NUM_JOINTS, r_rot_quat=pred_r_rot_quat, r_pos=pred_r_pos)   
        elif self.pred_samples_rep == 'orig':
            global_joint_pos, global_rot_quat = recover_from_ric(samples, NUM_JOINTS, return_r_rot=True)
    ...
    def compute(self, pred_samples=None, global_joint_pos=None):
        ...
        if pred_samples is not None:
            pred_samples = pred_samples.permute(0,2,3,1) # [batch, 1, nframes, nfeat]
...
def foot_skate(global_joint_pos, height_thresh=0.05):
    ...
    toe_on_floor = torch.concatenate([toe_on_floor[:,:,0:1], toe_on_floor[:,:,0:1], toe_on_floor[:,:,1:2], toe_on_floor[:,:,1:2]], dim=-1)[:,:-1]
    feet_vel = torch.norm(feet_pos[:,1:] - feet_pos[:,:-1], dim=-1) # [batch, nframes-1, 4]
    contact_feet_vel = feet_vel * toe_on_floor
    mean_vel = torch.sum(contact_feet_vel, (1,2)) / (torch.sum(toe_on_floor, (1,2)) + 1e-6)
