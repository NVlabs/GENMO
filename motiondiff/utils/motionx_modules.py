...
class MotionDecoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim, output_dim, unit_length):
        ...
        self.gru = nn.GRU(
            input_size=latent_dim, hidden_size=hidden_dim, batch_first=True
        )
    ...
    def forward(self, latent, lengths):
        ...
        max_len = (
            int(torch.max(lengths).item()) * self.unit_length
        )  # Scale lengths by unit_length

        # Repeat latent code for the required sequence length
        latent_expanded = latent.unsqueeze(1).repeat(
            1, max_len, 1
        )  # Shape: (batch_size, max_len, latent_dim)
...
class MovementConvEncoderWithDropout(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(
            0, 2, 1
        )  # (batch_size, input_dim, seq_len) -> (batch_size, seq_len, input_dim)
...
class MotionDecoderWithDropout(nn.Module):
    def __init__(
        self, latent_dim, hidden_dim, output_dim, unit_length, dropout_prob=0.3
    ):
