    ...
    def generate_smpl_scene(self, smpl_seq=None, window_size=None):
        ...
        for i, (skel_name, pose_dict) in enumerate(smpl_seq.items()):
            ...
            if 'joints_pos' in pose_dict:
                ...
                pose_dict['skeleton_jpos'] = SkeletonActor(scene, f'{skel_name}_jpos', self.joint_parents, joint_color=colors[0], bone_color=colors[2] if 'skeleton_fk' in pose_dict else colors[1], joint_constr_color='Brown' if skel_name == 'gt' else 'Cyan')
                if 'keyframe_idx' in pose_dict:
                    pose_dict['skeleton_keyframe_jpos'] = SkeletonActor(scene, f'{skel_name}_keyframe_jpos', self.joint_parents, joint_color='Orange', bone_color='Yellow')
                if 'target_jpos' in pose_dict:
                    pose_dict['skeleton_keyframe_jpos_unknownt'] = SkeletonActor(scene, f'{skel_name}_keyframe_jpos_unknownt', self.joint_parents, joint_color='Orange', bone_color='Pink', joint_constr_color='Purple')
