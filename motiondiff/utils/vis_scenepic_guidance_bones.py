...
class TrajActor():
    def __init__(self, scene, name, joint_pos, color='Orange', radius=0.1):
        ...
        for i in range(root_pos.shape[0]-1):
            self.traj_mesh.add_thickline(color=self.color,
...
class SkeletonActor():
    ...
    def __init__(self, scene, name, joint_parents, joint_color='Yellow', bone_color='Green', root_color='Yellow', 
                    joint_constr_color='Cyan', joint_radius=0.06, bone_radius=0.04, joint_constr_radius=0.1, 
                    opacity=1.0):
        self.scene = scene
        self.name = name
    ...
    def add_joints_mesh_to_frames(self, sp_frame, jpos, joint_inds=[21,20,7,8], proj_to_floor=False):
        sp_frame.add_mesh(self.floor_mesh)
        for idx in joint_inds:
            cur_pos = jpos[idx]
            if proj_to_floor and idx == 0: # root TODO: this is hacky, shouldn't assume only the root is projected
...
class ScenepicVisualizer:
    ...
    def __init__(self, device=torch.device('cpu'), show_skeleton_jpos=True, show_ik_smpl_pose=False, **kwargs):
        super().__init__(**kwargs)
        self.joint_parents = [-1, 0, 1, 2, 3, 4, 5, 4, 7, 8, 9, 10, 10, 12, 4, 14, 15, 16, 17, 17, 19, 0, 21, 22, 23, 0, 25, 26, 27]
        self.device = device
    ...
    def vis_smpl_scene(self, smpl_seq=None, html_path=None, window_size=(400, 400), fps=30):
    ...
    def generate_smpl_scene(self, smpl_seq=None, window_size=None, return_canvas=False, fps=30):
        scene = sp.Scene()
        scene.framerate = fps
        ...
        for i, (skel_name, pose_dict) in enumerate(smpl_seq.items()):
            ...
            if self.show_ik_smpl_pose and 'pose' in pose_dict:
                pose_dict['skeleton_fk'] = SkeletonActor(scene, skel_name, self.joint_parents, joint_color=colors[0], bone_color=colors[1])
            ...
            if 'joints_pos' in pose_dict:
                ...
                pose_dict['skeleton_jpos'] = SkeletonActor(scene, f'{skel_name}_jpos', self.joint_parents, joint_color=colors[0],
                ...
                if 'foot_contacts' in pose_dict:
                    pose_dict['foot_contacts_jpos'] = SkeletonActor(scene, f'{skel_name}_foot_contacts_jpos', self.joint_parents, joint_color='Red', bone_color='Yellow',
                                                                        root_color='Red', joint_radius=0.07)
                ...
                if 'keyframes' in pose_dict:
                    ...
                    for key_name, keyframe_dict in pose_dict['keyframes'].items(): 
                        ...
                        if not keyframe_dict.get('is_dense', False):
                            if unknownt:
                                ...
                                keyframes_unknownt_names.append(np.array([key_name]*num_keys))
                            else:
                                keyframes_knownt_idx.append(keyframe_dict['idx'])
                                keyframes_knownt_names.append(np.array([key_name]*num_keys))
                        else:
                        ...
                        if is_root_traj:
                            ...
                            traj_path = TrajActor(scene, f'{skel_name}_{key_name}', keyframe_dict['joints_pos'].cpu().numpy(), color='Orange', radius=0.03)
                            keyframe_dict['traj_path'] = traj_path

                        # skeleton visualizing full body of ground truth where keyframe is from
                        skeleton_key_jpos = SkeletonActor(scene, f'{skel_name}_{key_name}', self.joint_parents, 
                        ...
                        if 'joint_inds' in keyframe_dict:
                            skeleton_key_jinds = SkeletonActor(scene, f'{skel_name}_{key_name}_joints', self.joint_parents, 
                                                            joint_color='Teal' if unknownt else 'Orange', 
                                                                bone_color='Purple' if unknownt else 'Yellow',
                                                                root_color='Red', joint_radius=0.12)
                    ...
                    if len(keyframes_unknownt_idx) > 0:
                        ...
                        keyframes_unknownt_names = np.concatenate(keyframes_unknownt_names)
                        sort_unknownt = np.argsort(keyframes_unknownt_idx)             
                        keyframes_unknownt_idx = keyframes_unknownt_idx[sort_unknownt]
                        keyframes_unknownt_names = keyframes_unknownt_names[sort_unknownt]
        
        if not self.show_skeleton_jpos:
            layer_settings = {f'{skel_name}_jpos': {'filled': False} for skel_name, pose_dict in smpl_seq.items() if 'skeleton_jpos' in pose_dict}
            canvas.set_layer_settings(layer_settings)
        else:
            layer_settings = {f'{skel_name}_jpos': {'filled': True} for skel_name, pose_dict in smpl_seq.items() if 'skeleton_jpos' in pose_dict}
            keyframe_settings = dict()
            for i, (skel_name, pose_dict) in enumerate(smpl_seq.items()):
                if 'keyframes' in pose_dict:
                    for key_name, keyframe_dict in pose_dict['keyframes'].items():
                        if 'skeleton_jinds' in keyframe_dict and not 'traj_path' in keyframe_dict:
                            # keyframe is on a subset of joints, so we need to turn down opacity of full-body to visualize individual joints
                            keyframe_settings[f'{skel_name}_{key_name}'] = {'filled': True, 'opacity' : 0.15}
                ...
                if 'skeleton_jpos' in pose_dict:
                    ...
                    if 'keyframes' in pose_dict:
                        ...
                        if len(keyframes_knownt_idx) > 0:
                            ...
                            if np.sum(ind_diff >= 0) > 0: # we're not past the last keyframe
                                viz_ind = np.argmax(ind_diff >= 0)
                                viz_ind_list = np.nonzero(ind_diff == ind_diff[viz_ind])[0]
                                for viz_ind in viz_ind_list:
                                    ...
                                    key_ind = viz_key['idx'].index(viz_ind) # keyframe index within this constraint (if multiple)
                                    if 'skeleton_jinds' in viz_key:
                                        viz_key['skeleton_jinds'].add_joints_mesh_to_frames(main_frame, viz_key['joints_pos'][key_ind].cpu().numpy(), 
                                                                                                viz_key['joint_inds'], viz_key['root_proj_to_floor'])
                                    if 'hide_full_pose' not in viz_key or not viz_key['hide_full_pose']:
                                        viz_key['skeleton_jpos'].add_mesh_to_frames(main_frame, viz_key['joints_pos'][key_ind].cpu().numpy())

                        # visualize all unknownt keyframes that have not been passed
                        if len(keyframes_unknownt_idx) > 0:
                            for viz_ind, viz_name in zip(keyframes_unknownt_idx, keyframes_unknownt_names):
                                if viz_ind > ind:
                                    viz_key = pose_dict['keyframes'][viz_name]
                                    key_ind = viz_key['idx'].index(viz_ind) # keyframe index within this constraint (if multiple)
                                    if 'skeleton_jinds' in viz_key:
                        ...
                        if len(keyframes_dense_names) > 0:
                            for viz_name in keyframes_dense_names:
                                ...
                                if np.sum(ind_diff >= 0) > 0: # we're not past the last keyframe
                                    viz_ind = np.argmax(ind_diff >= 0)
                                    if 'skeleton_jinds' in viz_key:
                                        viz_key['skeleton_jinds'].add_joints_mesh_to_frames(main_frame, viz_key['joints_pos'][viz_ind].cpu().numpy(), 
                                                                                                viz_key['joint_inds'], viz_key['root_proj_to_floor'])
                                    if 'hide_full_pose' not in viz_key or not viz_key['hide_full_pose']:
                                        viz_key['skeleton_jpos'].add_mesh_to_frames(main_frame, viz_key['joints_pos'][viz_ind].cpu().numpy())
                    ...
                    if 'foot_contacts' in pose_dict:
                        ...
                        if np.sum(cur_contacts) > 0:
                            foot_inds = np.array([27, 28, 23, 24])
                            contact_inds = foot_inds[cur_contacts]
                            pose_dict['foot_contacts_jpos'].add_joints_mesh_to_frames(main_frame, pose_dict['joints_pos'][ind].cpu().numpy(), contact_inds)
