...
def joints_to_smpl(smpl_joint_pos, smpl, device):
    ...
    rest_body_pos = (
        smpl_rest_joints[smpl.gender]
        .repeat_interleave(smpl_body_pos.shape[0], dim=0)
        .to(device)
    )
    phis = torch.tensor([1.0, 0.0], device=device).expand(
    ...
    smpl_rot_mats, global_rot_mat, global_body_pos = batch_inverse_kinematics_transform(
        smpl_body_pos,
        None,
        phis,
        rest_body_pos,
    ...
    smpl_pose = rotation_matrix_to_angle_axis(smpl_rot_mats.contiguous()).reshape(
        *shape_in[:2], 72
    )
    ...
    smpl_pose[..., :3] = quaternion_to_angle_axis(
        quat_mul(
            base_rot.expand(smpl_pose.shape[:-1] + (4,)),
            angle_axis_to_quaternion(smpl_pose[..., :3]),
        )
    )
...
def humanml_to_smpl(
    ...
    sample_raw_norm = (
        sample_raw.permute(0, 2, 3, 1) * std + mean
    )  # sample_raw_norm: [batch, 1, seq_len, nfeat]
    ...
    if interp:
        ...
        f = interp1d(
            ...
            sample_recon.cpu().numpy(),
            axis=1,
            assume_sorted=True,
            fill_value="extrapolate",
        )
        sample_recon = torch.from_numpy(f(np.linspace(0, T, new_len))).type_as(
            sample_recon
        )
        if get_orig_rot:
            f = interp1d(
            ...
            f = interp1d(
                np.linspace(0, T, orig_len),
                sample_raw_norm.cpu().numpy(),
                axis=2,
                assume_sorted=True,
                fill_value="extrapolate",
            )
            sample_raw_norm = torch.from_numpy(f(np.linspace(0, T, new_len))).type_as(
                sample_raw_norm
            )
    ...
    if get_orig_rot:
        ...
        smpl_pose = torch.cat(
            [r_rot_aa.unsqueeze(2), smpl_pose, torch.zeros_like(smpl_pose[:, :, :2])],
            dim=2,
        ).reshape(sample.shape[0], -1, 72)
        joints_3d = None
    else:
        hand_joints = sample[:, :, -2:].clone()
        hand_joints[..., 1] += 1e-2
        ...
        leaf_thetas = torch.eye(3, device=device).expand(
        ...
        joints_3d = quat_apply(
            base_rot.expand(smpl_body_pos_orig.shape[:-1] + (4,)), smpl_body_pos_orig
        )
