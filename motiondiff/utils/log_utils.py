...
import logging
import time
from motiondiff.utils.tools import get_eta_str, convert_sec_to_time
...
class TextLogger(Logger):
    def __init__(self, file_path, cfg=None, write_file=True, training=True, max_steps=-1, log_nsteps=1, loss_fmt='%.4f'):
        super().__init__()
    ...
    def log_train(self):
        ...
        loss_str = ' | '.join([(loss_fmt_str % (x, y)) for x, y in self.cur_metrics.items() if x not in self.metrics_to_ignore])
