...
class LLAMADecoder(nn.Module):
    ...
    def load_pretrained(self, cp_path, config):
        ...
        for layer_idx, layer in enumerate(self.layers):
            layer_dict = {
                "self_attn.q_proj.weight": state_dict[
                    f"layers.{layer_idx}.attention.wq.weight"
                ],
                "self_attn.k_proj.weight": state_dict[
                    f"layers.{layer_idx}.attention.wk.weight"
                ],
                "self_attn.v_proj.weight": state_dict[
                    f"layers.{layer_idx}.attention.wv.weight"
                ],
                "self_attn.o_proj.weight": state_dict[
                    f"layers.{layer_idx}.attention.wo.weight"
                ],
                "input_layernorm.weight": state_dict[
            ...
            if config.pretrained_load_ffn:
                layer_dict.update(
                    {
                        "mlp.gate_proj.weight": state_dict[
                            f"layers.{layer_idx}.feed_forward.w1.weight"
                        ],
                        "mlp.up_proj.weight": state_dict[
                            f"layers.{layer_idx}.feed_forward.w3.weight"
                        ],
                        "mlp.down_proj.weight": state_dict[
                            f"layers.{layer_idx}.feed_forward.w2.weight"
                        ],
                    }
                )
            missing_keys, unexpected_keys = layer.load_state_dict(
                layer_dict, strict=False
            )
            if layer_idx == 0:
                if len(missing_keys) > 0:
                    print(
                        f"[LLAMADecoder] layer{layer_idx} Missing keys: {missing_keys}"
                    )
                if len(unexpected_keys) > 0:
                    print(
                        f"[LLAMADecoder] layer{layer_idx} Unexpected keys: {unexpected_keys}"
                    )
        return
