...
from motiondiff.utils.torch_utils import tensor_to, interp_tensor_with_scipy, slerp_joint_rots
...
from motiondiff.data_pipeline.bones.motion_process import recover_from_ric, recover_root_rot_pos, recover_from_ric_with_joint_rot
from motiondiff.utils.torch_transform import quat_apply, normalize, quat_mul
...
class MDMBase(pl.LightningModule):
    ...
    def __init__(self, cfg, is_inference=False):
        ...
        self.motion_mean = torch.tensor(np.load(f'{self.model_cfg.motion_stats_folder}/mean.npy')).float()
        self.motion_std = torch.tensor(np.load(f'{self.model_cfg.motion_stats_folder}/std.npy')).float()
        if self.normalize_global_pos:
            motion_root_stats_dir  = cfg.model.get('motion_root_stats_dir', 'assets/bones/global_stats/v1/frames_196')
            print(f'Normalizing global root with stats from {motion_root_stats_dir}...')
            self.motion_global_mean = torch.tensor(np.load(os.path.join(motion_root_stats_dir, 'mean.npy'))).float()
            self.motion_global_std = torch.tensor(np.load(os.path.join(motion_root_stats_dir, 'std.npy'))).float()
    ...
    def generate_motion_mask(self, motion_mask_cfg, motion, lengths, use_mask_type=None, return_keyframes=False, mask_cfgs=None):
        '''
        ...
        def get_root_mask_indices(mode):
            if mode in {'root+joints', 'root'}:
                ...
                elif self.motion_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
        ...
        def local_joints(_cfg):
            ...
            joint_indices = [bones_joint_names.index(joint_name) for joint_name in joint_names]
            all_keyframe_idx = _cfg.get('keyframe_idx', None)
            for i in range(motion.shape[0]):
                ...
                if _cfg.get('consistent_joints', False):    # consistent_joints: masked joints are the same for all the frames
                    ...
                    motion_mask[i, root_mask_ind, :, keyframe_idx] = 1.0 # root_mask_ind is always a slice
                else:
    ...
    def convert_motion_rep(self, motion):
        if self.motion_rep in {'orig', 'orig_rot'}:
            ...
            motion_norm = motion.permute(0, 2, 3, 1) * self.motion_std.to(self.device) + self.motion_mean.to(self.device) # [batch, 1, seq_len, nfeat]
            _, r_pos, r_rot_ang = recover_root_rot_pos(motion_norm, return_r_rot_ang=True)
            ...
            if self.normalize_global_pos:
                motion[:, :5] = (motion[:, :5] - self.motion_global_mean[None, :, None, None].to(self.device)) / self.motion_global_std[None, :, None, None].to(self.device)
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if use_twopass:
            ...
            new_loss, new_loss_dict, new_loss_uw_dict = self.compute_loss(data, t, t_weights)
            ...
            loss_uw_dict.update({k + '_twopass' : v for k,v in new_loss_uw_dict.items()})
    ...
    def infer_texts_guided(self, texts, num_frames, target_motion, motion_mask=None, observed_motion=None, rm_text_flag=None, 
        ...
        if motion_mask is not None and observed_motion is not None:
            cond['motion_mask'], cond['observed_motion'] = tensor_to([motion_mask, observed_motion], device=self.device)
    ...
    def infer_texts(self, texts, num_frames, motion_mask=None, observed_motion=None, rm_text_flag=None, progress=True):
        diffusion = self.test_diffusion
        batch_size = len(texts)
        _, cond = collate(
            [{'inp': torch.tensor([[0.]]), 'target': 0, 'text': txt, 'tokens': None, 'lengths': num_frames} for txt in texts]
    ...
    def obtain_joints(self, samples, interp=True, return_contacts=False, return_joint_rot=False):
        base_rot = torch.tensor([[0.5, 0.5, 0.5, 0.5]], device=samples.device, dtype=samples.dtype)
        if self.motion_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
            samples_perm = samples.permute(0, 2, 3, 1) # now permulted to: [batch, 1, seq_len, nfeat]
            root_samp = samples_perm[..., :5]
            if self.normalize_global_pos:
                root_samp = root_samp * self.motion_global_std.to(self.device) + self.motion_global_mean.to(self.device)
            r_pos, rot_cos_sin, local_feats = root_samp[..., :3], root_samp[..., 3:5], samples_perm[..., 5:]
            rot_cos_sin = normalize(rot_cos_sin)
            r_rot_ang = torch.atan2(rot_cos_sin[..., [1]], rot_cos_sin[..., [0]])
            r_rot_quat = torch.cat([torch.cos(r_rot_ang / 2), torch.zeros_like(rot_cos_sin[..., [0]]), torch.sin(r_rot_ang / 2), torch.zeros_like(rot_cos_sin[..., [0]])], dim=-1)

            local_feats_norm = local_feats * self.motion_std[4:].to(self.device) + self.motion_mean[4:].to(self.device) # [batch, 1, seq_len, nfeat]
            ...
            if self.motion_rep == 'global_root_local_joints':
                joints_pos = recover_from_ric(local_feats_norm_pad, self.num_joints, r_rot_quat, r_pos)[:, 0]
            elif self.motion_rep == 'global_root_local_joints_root_rot':
                joints_pos = recover_from_ric_with_joint_rot(local_feats_norm_pad, self.neutral_joints.to(samples_perm), self.joint_parents.to(samples_perm.device), 
                                                         r_rot_quat=r_rot_quat, 
                                                         r_pos=r_pos)[:, 0]
                
                joints_pos, joints_rot = recover_from_ric_with_joint_rot(local_feats_norm_pad, self.neutral_joints.to(samples_perm), self.joint_parents.to(samples_perm.device), 
                                                         r_rot_quat=r_rot_quat, 
                                                         r_pos=r_pos,
            ...
            if return_joint_rot:
                ...
                root_rot_quat = quat_mul(base_rot[:,None,None].expand_as(root_rot_quat), root_rot_quat)
            ...
            samples_perm = samples_perm * self.motion_std.to(samples_perm.device) + self.motion_mean.to(samples_perm.device)
            ...
            samples_perm = samples_perm * self.motion_std.to(samples_perm.device) + self.motion_mean.to(samples_perm.device)
            joints_pos, joints_rot = recover_from_ric_with_joint_rot(samples_perm, self.neutral_joints.to(samples_perm), self.joint_parents.to(samples_perm.device),
                                                                        return_joint_rot=True)
            if interp:
                joints_pos = interp_tensor_with_scipy(joints_pos, scale=1.5, dim=1)
            joints_pos = quat_apply(base_rot.expand(joints_pos.shape[:-1] + (4,)), joints_pos)
        ...
        if return_contacts:
            ...
            foot_contacts = samples_perm[:, 0, :, -4:] # [batch, seq_len, 4] where 4 is ("L_Ankle", "L_Toe", "R_Ankle", "R_Toe")
            # should be between 0 and 1 unnormalized
            foot_contacts_norm = foot_contacts * self.motion_std[-4:].to(samples_perm.device) + self.motion_mean[-4:].to(samples_perm.device)
            if interp:
