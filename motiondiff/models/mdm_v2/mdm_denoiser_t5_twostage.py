...
class MDMDenoiser(nn.Module):
    def __init__(self, pl_module, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
                 ...
                 ablation=None, activation="gelu", padding_mask=False, data_rep='hml_vec', dataset='amass', llm_dim=512, max_text_len=50,
                 arch='trans_enc', use_moe=False, moe_cfg=None, llama_cfg=None, emb_trans_dec=False, add_x_to_memory=False, llm_type="t5", llm_version="t5-small", text_enc_mode='seq_concat', pretrained_checkpoint=None, 
                 use_motion_mask=False, obs_motion_constraints='soft', pred_root_with_joints=True, sep_root_body=False, 
                 project_output_to_mask=False, project_root_to_mask=False, **kargs):
        super().__init__()
        ...
        self.motion_localjoints_dim = self.ext_models['pl_module'].motion_localjoints_dim
        self.motion_root_joints_dim = self.motion_root_dim + self.motion_localjoints_dim
        input_dim = (self.input_feats * 2 if use_motion_mask else self.input_feats) + self.gru_emb_dim - (self.motion_root_dim - 4)
        ...
        self.root_input_feats = (self.input_feats * 2 if use_motion_mask else self.input_feats) if self.pred_root_with_joints else self.motion_root_dim
        self.root_output_feats = self.motion_root_dim
        self.root_input_process = InputProcess('root', self.root_input_feats, self.latent_dim)
        if self.arch == 'trans_enc':
            if self.use_moe:
                root_seqTransEncoderLayer = MoETransformerEncoderLayer(d_model=self.latent_dim,
                ...
                root_seqTransEncoderLayer = nn.TransformerEncoderLayer(d_model=self.latent_dim,
                                                                    nhead=self.num_heads,
                                                                    dim_feedforward=self.ff_size,
                                                                    dropout=self.dropout,
                                                                    activation=self.activation)

            self.root_seqTransEncoder = nn.TransformerEncoder(root_seqTransEncoderLayer,
                                                            num_layers=self.num_layers)
        elif self.arch == 'trans_dec':
            if self.use_moe:
                root_seqTransDecoderLayer = MoETransformerEncoderLayer(d_model=self.latent_dim,
                                                                nhead=self.num_heads,
                                                                dim_feedforward=self.ff_size,
                                                                dropout=self.dropout,
                                                                activation=self.activation,
                ...
                root_seqTransDecoderLayer = nn.TransformerDecoderLayer(d_model=self.latent_dim,
                                                                    nhead=self.num_heads,
                                                                    dim_feedforward=self.ff_size,
                                                                    dropout=self.dropout,
                                                                    activation=self.activation)
            self.root_seqTransDecoder = nn.TransformerDecoder(root_seqTransDecoderLayer,
                                                            num_layers=self.num_layers)
        elif self.arch == 'llama':
            self.root_seqTransEncoder = LLAMADecoder(llama_cfg)
        else:
            raise ValueError('Please choose correct architecture')
        self.root_output_process = OutputProcess(self.data_rep, self.root_output_feats, self.latent_dim, self.root_output_feats, 1)

        if self.arch == 'trans_enc':
            print("TRANS_ENC init")
            if self.use_moe:
                seqTransEncoderLayer = MoETransformerEncoderLayer(d_model=self.latent_dim,
                                                                   nhead=self.num_heads,
                                                                   dim_feedforward=self.ff_size,
                                                                   dropout=self.dropout,
                                                                   activation=self.activation,
            ...
            if self.use_moe:
                seqTransDecoderLayer = MoETransformerEncoderLayer(d_model=self.latent_dim,
                                                                   nhead=self.num_heads,
                                                                   dim_feedforward=self.ff_size,
                                                                   dropout=self.dropout,
                                                                   activation=self.activation,
        ...
        if self.sep_root_body:
            ...
            self.root_sequence_pos_encoder = PositionalEncoding(self.latent_dim, self.dropout)
            self.root_embed_timestep = TimestepEmbedder(self.latent_dim, self.root_sequence_pos_encoder)
        ...
        self.output_process = OutputProcess(self.data_rep, self.input_feats - self.root_output_feats, self.latent_dim, self.njoints - self.root_output_feats,
                                            self.nfeats)
    ...
    def convert_root_global_to_local(self, root_motion):
        ...
        if self.normalize_global_pos:
            root_motion = root_motion * self.motion_global_std.to(root_motion.device) + self.motion_global_mean.to(root_motion.device)

        r_pos, rot_cos, rot_sin = root_motion[..., :3], root_motion[..., 3], root_motion[..., 4]
        r_rot_ang = torch.atan2(rot_sin, rot_cos)
        r_rot_quat_inv = torch.stack([torch.cos(r_rot_ang / 2), torch.zeros_like(rot_cos), -torch.sin(r_rot_ang / 2), torch.zeros_like(rot_cos)], dim=-1)
        ...
        local_motion_norm = (local_motion - self.motion_mean[:4].to(local_motion.device)) / self.motion_std[:4].to(local_motion.device)
    ...
    def forward_root(self, x, emb, emb_text, y, x_gt, using_gt_joints_cond):
        ...
        if self.arch in {'trans_enc', 'llama'}:
            ...
            if self.text_enc_mode == 'seq_concat':
                xseq = torch.cat((emb_text.transpose(0, 1), xseq), axis=0) # [max_text_len + 1 + seqlen, bs, d]
                pose_start_ind += emb_text.shape[1]
            xseq = self.root_sequence_pos_encoder(xseq) if self.sep_root_body else self.sequence_pos_encoder(xseq) # [seqlen+1, bs, d]
            src_mask = None
            src_key_padding_mask = None
            if self.padding_mask and 'mask' in y:
                src_key_padding_mask = torch.zeros((xseq.shape[1], xseq.shape[0]), dtype=torch.bool, device=xseq.device) # [bs, max_text_len + 1 + seqlen]
                src_key_padding_mask[:, pose_start_ind:] = ~y['mask'][:, 0, 0]
            output = self.root_seqTransEncoder(xseq, mask=src_mask, src_key_padding_mask=src_key_padding_mask)[pose_start_ind:]
            ...
            if self.emb_trans_dec:
                ...
                pose_start_ind += 1
            xseq = self.root_sequence_pos_encoder(xseq) if self.sep_root_body else self.sequence_pos_encoder(xseq) # [seqlen+1, bs, d]
            memory = self.root_sequence_pos_encoder(memory) if self.sep_root_body else self.sequence_pos_encoder(memory) # [seqlen+1, bs, d]
            tgt_mask = memory_mask = None
            tgt_key_padding_mask = None
            if self.padding_mask and 'mask' in y:
                tgt_key_padding_mask = torch.zeros((xseq.shape[1], xseq.shape[0]), dtype=torch.bool, device=xseq.device)
                tgt_key_padding_mask[:, pose_start_ind:] = ~y['mask'][:, 0, 0]

            output = self.root_seqTransDecoder(tgt=xseq, memory=memory, tgt_mask=tgt_mask, memory_mask=memory_mask, tgt_key_padding_mask=tgt_key_padding_mask)[pose_start_ind:] # [seqlen, bs, d] # FIXME - maybe add a causal mask


        xr = self.root_output_process(output)
        return xr
    
    def project_motion(self, pred_out, interp_mask, observed_motion, num_interp_feats,
                            pos_weight=1e-3, vel_weight=1.0, accel_weight=1.0):
        ...
        interp_mask = interp_mask[:,:num_interp_feats].permute(0,1,3,2) # [batch, num_interp_feats, num_frames, 1]
        observed_motion = observed_motion[:,:num_interp_feats].permute(0,1,3,2) # [batch, num_interp_feats, num_frames, 1]
        ...
        assert torch.sum(interp_mask[:,:num_interp_feats] - interp_mask[:,0:1]) < 1e-6, "Projection only supports full-body keyframes"

        do_interp = (torch.sum(interp_mask, dim=(1,2,3)) > 0).to(interp_mask) # only do this if there are keyframes in the mask
        ...
        for bi in range(pred_out.shape[0]):
            cur_lhs, cur_rhs = interp_mats(interp_mask[bi,0,:,0], pos_weight, vel_weight, accel_weight)
            interp_rhs_mat[bi] = cur_rhs
            interp_lhs_mat[bi] = cur_lhs

        # do the interpolation
        xdiff = pred_out[:,:num_interp_feats] - interp_mask * observed_motion # [batch, num_interp_feats, num_frames, 1]
        ...
        pred_out[:,:num_interp_feats] = (
            do_interp[:,None,None,None] * torch.matmul(interp_lhs_mat.unsqueeze(1), rhs) +
    ...
    def forward(self, x, timesteps, y=None, motion_mask=None, observed_motion=None, rm_text_flag=None,
                gt_motion=None, using_gt_root_cond=None, using_gt_joints_cond=None, fixed_root_input=None,
                **kwargs):
        """
        ...
        if 'text' in self.cond_mode:
            ...
            text_cond = self.mask_cond(enc_text, force_mask=force_mask, rm_text_flag=rm_text_flag)
            emb_text = self.embed_text(text_cond)
            if self.text_enc_mode == 'mean':
                emb_text = emb_text.mean(dim=1)
                emb += emb_text
            root_emb_text = self.root_embed_text(text_cond) if self.sep_root_body else emb_text
        ...
        if gt_motion is not None:
            ...
            x_gt = x.clone()    # use current x as gt, for example, keep the root motion fixed as gt

        # predict root motion first
        if fixed_root_input is not None:
            root_motion_pred = fixed_root_input.clone()
        else:
            root_motion_pred = self.forward_root(x, root_emb, root_emb_text, y, x_gt, using_gt_joints_cond)

        if self.project_root_to_mask and motion_mask is not None:
            root_motion_pred = self.project_motion(root_motion_pred, motion_mask, observed_motion, self.motion_root_dim) # project only root

        with torch.no_grad():
            if using_gt_root_cond is None:
                root_motion_for_joints = root_motion_pred
            else:
                root_motion_for_joints = torch.where(using_gt_root_cond[:, None, None, None], x_gt[:, :self.motion_root_dim], root_motion_pred)
            root_motion_local = self.convert_root_global_to_local(root_motion_for_joints)

        x_new = torch.cat([root_motion_local.detach(), x[:, self.motion_root_dim:]], axis=1)
        ...
        if self.arch in {'trans_enc', 'llama'}:
            ...
            output = self.seqTransDecoder(tgt=xseq, memory=memory, tgt_mask=tgt_mask, memory_mask=memory_mask, tgt_key_padding_mask=tgt_key_padding_mask)[pose_start_ind:] # [seqlen, bs, d] # FIXME - maybe add a causal mask
        ...
        if self.project_output_to_mask and motion_mask is not None:
            output = self.project_motion(output, motion_mask, observed_motion, self.motion_root_joints_dim) # project root + local joint pos
...
def interp_mats(interp_mask, pos_weight, vel_weight, acc_weight):
    ...
    interp_lhs_mat = interp_rhs_mat * (1 - interp_mask[:, None]) * (1 - interp_mask[None, :])
