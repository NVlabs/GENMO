...
from motiondiff.utils.tools import import_type_from_str, find_last_version, get_checkpoint_path, load_ema_weights_from_checkpoint
...
class DPO(MDM):
    ...
    def load_ref_model(self, ref_model_specs):
        ...
        self.ext_models['ref_model'] = import_type_from_str(ref_cfg.model.type)(ref_cfg, is_inference=True, preload_checkpoint=False)
        ...
        model_cp, cp_name = get_checkpoint_path(checkpoint_dir, ref_model_specs.cp, return_name=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if motion_w.device != self.device:
            motion_w, cond_w, motion_l, cond_l = tensor_to([motion_w, cond_w, motion_l, cond_l], device=self.device)
    ...
    def get_diffusion_pred_target(self, data, t, noise=None):
        ...
        data['model_pred_w'] = self.denoiser(x_t_w, diffusion._scale_timesteps(t), **data['cond_w'])
        data['model_pred_l'] = self.denoiser(x_t_l, diffusion._scale_timesteps(t), **data['cond_l'])
        
        with torch.no_grad():
            data['ref_pred_w'] = self.ext_models['ref_model'].denoiser(x_t_w, diffusion._scale_timesteps(t), **data['cond_w'])
            data['ref_pred_l'] = self.ext_models['ref_model'].denoiser(x_t_l, diffusion._scale_timesteps(t), **data['cond_l'])

        if diffusion.model_mean_type == ModelMeanType.PREVIOUS_X:
            data['target_w'] = diffusion.q_posterior_mean_variance(x_start=x_w, x_t=x_t_w, t=t)[0]
            data['target_l'] = diffusion.q_posterior_mean_variance(x_start=x_l, x_t=x_t_l, t=t)[0]
    ...
    def compute_loss(self, data, t, t_weights):
        ...
        def dpo_loss(cfg):
            ...
            model_w_err = comp_err(data['model_pred_w'], data['target_w'], data['cond_w']['y']['mask'])
            model_l_err = comp_err(data['model_pred_l'], data['target_l'], data['cond_l']['y']['mask'])
            ref_w_err = comp_err(data['ref_pred_w'], data['target_w'], data['cond_w']['y']['mask'])
            ref_l_err = comp_err(data['ref_pred_l'], data['target_l'], data['cond_l']['y']['mask'])
        ...
        def dpo_loss_v2(cfg):
            ...
            model_ref_w_err = comp_err(data['model_pred_w'], data['ref_pred_w'], data['cond_w']['y']['mask'])
            model_ref_l_err = comp_err(data['model_pred_l'], data['ref_pred_l'], data['cond_l']['y']['mask'])
