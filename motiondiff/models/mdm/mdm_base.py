...
class MDMBase(pl.LightningModule):
    def __init__(self, cfg, is_inference=False):
        ...
        self.humanml_root_stats_file = cfg.model.get(
            "humanml_root_stats_file", "data/stats/HumanML3D_global_pos_stats.npy"
        )
    ...
    def get_diffusion_pred_target(self, data, t, noise=None):
        ...
        sched_samp_repeat_timesteps_only = self.cfg.train.get(
            "sched_samp_repeat_timesteps_only", False
        )
        ...
        if repeat_final_timesteps is None:
            ...
            num_repeat_steps = int(
                int(repeat_final_timesteps.replace("%", ""))
                / 100
        ...
        if sched_samp_prob_root > 0.0 or sched_samp_prob_joints > 0.0:
            using_gt_root_cond = torch.bernoulli(
                torch.ones_like(t) * sched_samp_prob_root
            ).bool()
            using_gt_joints_cond = torch.bernoulli(
                torch.ones_like(t) * sched_samp_prob_joints
            ).bool()
    ...
    def generate_motion_mask(
        ...
        def global_joints(_cfg):
            ...
            def get_local_joints(sample, g_motion, g_joint_mask):
                def get_local_joints(in_motion):
                    g_joints = (
                        in_motion[:, :, 0]
                        .transpose(1, 2)
                        .view(sample_root_pos.shape[:2] + (-1, 3))
                    )
                    l_joints = g_joints[..., :22, :].clone()
                    l_joints[..., :2] -= sample_root_pos[..., :2]
                    l_joints = qrot(
                        sample_r_rot.expand(l_joints.shape[:-1] + (4,)), l_joints
                    )
                    return l_joints.view(l_joints.shape[:-2] + (-1,))

                sample_global, sample_r_rot = self.get_global_position(
                    sample, return_r_rot=True
                )
            ...
            global_joint_mask = torch.zeros((motion.shape[0], motion.shape[-1], 66)).to(
                self.device
            )
            ...
            joint_indices = [
                SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in joint_names
            ]
            all_keyframe_idx = _cfg.get("keyframe_idx", None)
            for i in range(motion.shape[0]):
                if all_keyframe_idx is None:
                ...
                if _cfg.get(
                    ...
                    global_joint_mask[i, keyframe_idx, :] = (
                        torch.from_numpy(joint_mask)
                        .repeat_interleave(3)
                        .to(motion.device)
                    )
        ...
        def unknownt_keyframes(_cfg):
            ...
            root_mask_ind = get_root_mask_indices(mode)
            if not mode in {"root", "root_xy+rot", "root_xy"}:
                unknownt_motion_mask[:, root_dim : root_dim + ljoint_dim] = (
                    1.0  # only root + local joint positions
                )
            unknownt_motion_mask[:, root_mask_ind] = 1.0
            rm_text_flag = torch.from_numpy(
    ...
    def get_global_position(self, motion, return_r_rot=False):
        base_rot = torch.tensor(
            [[0.5, 0.5, 0.5, 0.5]], device=self.device, dtype=motion.dtype
        )
        if self.motion_rep in {"global_position", "full263"}:
            # step 1: denormalize
            motion_norm = motion.permute(0, 2, 3, 1) * self.humanml_std.to(
                self.device
            ) + self.humanml_mean.to(self.device)  # [batch, 1, seq_len, nfeat]
            # step 2: local to global conversion
            if return_r_rot:
                global_motion, r_rot = recover_from_ric(
                    motion_norm, 22, return_r_rot=True
                )
                if self.global_pos_z_up:
                    r_rot = qmul(
                        r_rot, qinv(base_rot).expand_as(r_rot)
                    )  # qinv(qmul(base_rot.expand_as(r_rot), qinv(r_rot)))
            else:
            ...
            if self.global_pos_z_up:
                global_motion = quat_apply(
                    base_rot.expand(smpl_body_pos.shape[:-1] + (4,)), smpl_body_pos
                )
            global_motion = global_motion.view(
                global_motion.shape[:-2] + (-1,)
            ).permute(0, 3, 1, 2)
        elif self.motion_rep == "global_root_local_joints":
            samples = motion.permute(0, 2, 3, 1)
            root_samp = samples[..., :5]
            if self.normalize_global_pos:
                root_samp = root_samp * self.humanml_global_std.to(
                    self.device
                ) + self.humanml_global_mean.to(self.device)
            ...
            local_feats_norm = local_feats * self.humanml_std[4:].to(
                self.device
            ) + self.humanml_mean[4:].to(self.device)  # [batch, 1, seq_len, nfeat]
            local_feats_norm_pad = torch.cat(
                [torch.zeros_like(local_feats_norm[..., :4]), local_feats_norm], dim=-1
            )
            global_motion = recover_from_ric(
                local_feats_norm_pad, 22, r_rot_quat, r_pos
            )
            global_motion = global_motion.view(
                global_motion.shape[:-2] + (-1,)
            ).permute(0, 3, 1, 2)
    ...
    def convert_motion_rep(self, motion):
        if self.motion_rep == "position":
            ...
            r_rot_quat, r_pos, r_rot_ang = recover_root_rot_pos(
            ...
            if self.normalize_global_pos:
                motion[:, :5] = (
                    motion[:, :5]
                    - self.humanml_global_mean[None, :, None, None].to(self.device)
                ) / self.humanml_global_std[None, :, None, None].to(self.device)
            ...
            motion = torch.cat(
                [r_pos.permute(0, 3, 1, 2), rot_cos_sin.permute(0, 3, 1, 2)], dim=1
            )
            if self.normalize_global_pos:
                motion = (
                    motion
    ...
    def compute_loss(self, data, t, t_weights):
        ...
        def foot_sliding(cfg):
            ...
            contact_label = contact_label * self.humanml_std[-4:, None].to(
                self.device
            ) + self.humanml_mean[-4:, None].to(self.device)
        ...
        if self.motion_rep == "position":
            motion_pad = (
                ...
                if gt_motion is not None
                    ...
                    (samples.shape[0], 263 - samples.shape[1], *samples.shape[2:])
                )
            )
    ...
    def obtain_joints_and_smpl_pose(
        ...
        if self.motion_rep == "global_position":
            ...
            smpl_pose = smpl_trans = None
            base_rot = torch.tensor(
                [[0.5, 0.5, 0.5, 0.5]], device=self.device, dtype=samples.dtype
            )
            ...
            joints_pos = recover_from_ric(local_feats_norm_pad, 22, r_rot_quat, r_pos)[
                :, 0
            ]
            ...
            pos_v_xy, pos_y, ang_v, local_feats = (
                root_samp[..., :2],
                root_samp[..., [2]],
                root_samp[..., [3]],
                samples[..., 4:],
            ...
            observed_motion = torch.zeros(
                (samples.shape[0], body_model.motion_rep_dim, *samples.shape[2:]),
                device=samples.device,
            )
            observed_motion[:, : root_motion_local.shape[1]] = root_motion_local
            res = body_model.generate_motion_mask(
                body_model.model_cfg.motion_mask,
                observed_motion,
                samples.shape[-1],
            ...
            joints_pos, smpl_pose, smpl_trans = body_model.obtain_joints_and_smpl_pose(
                body_samples, smpl, infer_kwargs=body_infer_kwargs, interp=interp
            )
        else:
            smpl_pose, smpl_trans, joints_pos = humanml_to_smpl(
                samples,
                mean=self.humanml_mean.to(self.device),
                std=self.humanml_std.to(self.device),
                smpl=smpl,
            )
    ...
    def convert_root_global_to_local(self, root_motion):
        ...
        r_rot_quat = torch.stack(
            [rot_cos, torch.zeros_like(rot_cos), rot_sin, torch.zeros_like(rot_cos)],
            dim=-1,
        )
