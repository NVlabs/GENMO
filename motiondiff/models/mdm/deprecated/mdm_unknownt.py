...
class MDMUnknownt(MDMBase):
    ...
    def generate_motion_mask(self, motion_mask_cfg, motion, lengths, use_mask_type=None, return_keyframes=False, all_keyframe_idx=None):
        if use_mask_type is not None:
            mask_type = use_mask_type
        else:
            mask_type = np.random.choice(motion_mask_cfg.mask_types, p=motion_mask_cfg.mask_probs)
        ...
        for i in range(motion.shape[0]):
            if sample_keyframes:
                ...
                t = np.random.randint(mlen) # random step from 0 to mlen for this sequence
            else:
                t = all_keyframe_idx[i]
                if not isinstance(t, int):
                    assert len(t) == 1, "unknown t trained model only supports single keyframe"
                    t = t[0]
        ...
        def keyframes(_cfg):
            ...
            motion_mask[:, root_dim:root_dim + ljoint_dim] = 1.0 # only root + local joint positions
            motion_mask[:, root_mask_ind] = 1.0
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def local_joints(_cfg):
            ...
            motion_mask[:, root_dim:root_dim + ljoint_dim] = torch.from_numpy(joint_mask).repeat_interleave(3)[:, None, None].to(motion.device)
            motion_mask[:, root_mask_ind] = 1.0
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
