...
from motiondiff.utils.torch_utils import bound_angle, move_module_dict_to_device
...
class MDMDenoiser(nn.Module):
    def __init__(
        ...
        root_transform_after_denoise=False,
        ...
        **kargs,
        ...
        humanml_global_mean, humanml_global_std = np.load(
            "data/stats/HumanML3D_global_pos_stats.npy"
        )
    ...
    def motion_local_to_global(self, motion):
        ...
        motion_norm = motion.permute(0, 2, 3, 1) * self.humanml_std.to(
            device
        ) + self.humanml_mean.to(device)  # [batch, 1, seq_len, nfeat]
    ...
    def motion_global_to_local(self, motion_g, r_rot_quat):
        ...
        body_pos = qrot(
            r_rot_quat[..., None, :].expand(body_pos.shape[:-1] + (4,)), body_pos
        )
        ...
        local_motion = torch.cat(
            [
                ang_v,
                l_pos[..., [0, 2]],
                r_pos_y,
                body_pos.view(body_pos.shape[:-2] + (-1,)),
            ],
            dim=-1,
        )
        local_motion_norm = (
            local_motion - self.humanml_mean[:67].to(local_motion.device)
        ) / self.humanml_std[:67].to(local_motion.device)
    ...
    def align_angle(self, root_theta, r_rot_ang, root_target_theta, end_fr, start_fr=0):
        ...
        theta_correction = (
            torch.arange(end_fr - start_fr + 1, device=root_theta.device)
            * amortized_theta
        )
        theta_correction = torch.cat(
            [
                theta_correction,
                theta_correction[[-1]].repeat(root_theta.shape[0] - end_fr - 1),
            ],
            dim=0,
        )
        root_theta_new = root_theta + theta_correction
        r_rot_ang_new = (
            r_rot_ang + theta_correction * 0.5
        )  # the rot_ang represents the half angle
    ...
    def transform_root_traj(
        self, local_motion, global_motion_target, global_joint_mask
    ):
        ...
        rot_cos, rot_sin = r_rot_quat[..., 0], r_rot_quat[..., 2]
        r_rot_ang = torch.atan2(rot_sin, rot_cos + 1e-4)
        global_motion_target = global_motion_target.squeeze(2).transpose(
            1, 2
        )  # [batch, seq_len, nfeat]
        ...
        root_target_theta = torch.atan2(
            root_target_xy[..., 1], root_target_xy[..., 0] + 1e-4
        )
        ...
        for i in range(local_motion.shape[0]):
            ...
            root_theta_i, r_rot_ang_i = self.align_angle(
                root_theta[i], r_rot_ang[i], root_target_theta[i], end_fr=fr
            )
        ...
        root_xy_new = torch.stack(
            [root_r * torch.cos(root_theta), root_r * torch.sin(root_theta)], dim=-1
        )
        r_rot_quat_new = torch.stack(
            [
                torch.cos(r_rot_ang),
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.root_transform_after_denoise:
            if global_motion is not None:
                output = self.transform_root_traj(
                    output, global_motion, global_joint_mask
                )
