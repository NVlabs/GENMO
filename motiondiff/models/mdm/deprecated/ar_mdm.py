...
class ARMDM(MDMBase):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(motion.shape[0]):
            motion_start = torch.randint(
                max(lengths[i].item() - ar_window, 1), (1,)
            ).item()
            ...
            if motion_prefix_i.shape[-1] > motion_prefix_len:
                ...
                motion_prefix_i = torch.cat(
                    [
                        torch.zeros(
                            (
                                *motion_prefix_i.shape[:-1],
                                motion_prefix_len - motion_prefix_i.shape[-1],
                            ),
                            device=self.device,
                        ),
                        motion_prefix_i,
    ...
    def infer_texts(self, texts, num_frames, progress=True):
        ...
        while cur_sample is None or cur_sample.shape[-1] < num_frames:
            ...
            if cur_sample is None:
                ...
                motion_prefix = torch.cat(
                    [
                        torch.zeros(
                            (
                                *cur_sample.shape[:-1],
                                motion_prefix_len - cur_sample.shape[-1],
                            ),
                            device=self.device,
                        ),
                        cur_sample,
    ...
    def validate_loss(self, batch, batch_idx):
        with torch.no_grad():
            ...
            for i in range(motion.shape[0]):
                ...
                motion_ar_i = motion[i, ..., motion_start : motion_start + ar_window]
                mask_i = cond["y"]["mask"][
                    i, ..., motion_start : motion_start + ar_window
                ]
                motion_prefix_i = motion[i, ..., :motion_start]
                if motion_prefix_i.shape[-1] > motion_prefix_len:
                    motion_prefix_i = motion_prefix_i[..., -motion_prefix_len:]
