...
class MDMDenoiser(nn.Module):
    def __init__(self, pl_module, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
                 ...
                 use_precomp_text_embed=False, llm_embed_lmdb_path=None, use_motion_mask=False, obs_motion_constraints='hard', use_obs_diff=False, use_unknownt_motion_mask=False, use_unknownt_obs_diff=False, use_global_constraints=False, 
                 global_feat_type=['g_motion_l_joints', 'l_joints_diff', 'global_joint_mask'], normalize_global_pos=False, pred_root_with_joints=False, use_foot_skate_feats=False, **kargs):
        super().__init__()
        ...
        self.motion_root_joints_dim = self.motion_root_dim + self.motion_localjoints_dim # 67 : previously, may need to change to run old models
    ...
    def convert_root_global_to_local(self, root_motion):
        ...
        if self.ext_models['pl_module'].motion_rep == 'global_root_vel_local_joints':
            pos_v_xy, pos_y, ang_v = root_motion[..., :2], root_motion[..., [2]], root_motion[..., [3]]
            ...
            r_pos = torch.zeros(pos_v_xy.shape[:-1] + (3,), dtype=pos_v_xy.dtype, device=root_motion.device)
    ...
    def forward_root(self, x, emb, emb_text, y, x_gt, using_gt_joints_cond):
        ...
        if self.arch == 'trans_enc':
            ...
            if self.autoregressive:
                src_mask = generate_ar_mask(xseq.shape[0], xseq.shape[0], tgt_start_dim=pose_start_ind, src_start_dim=pose_start_ind).to(xr.device)
    ...
    def get_foot_skate_feats(self, x):
        ...
        foot_vel = foot_vel.view(foot_vel.shape[:-2] + (-1, )).transpose(1, 2).unsqueeze(2)
        return foot_vel

    def forward(self, x, timesteps, y=None, motion_mask=None, observed_motion=None, rm_text_flag=None, global_motion=None, global_joint_mask=None, global_joint_func=None,
                unknownt_motion_mask=None, unknownt_observed_motion=None, gt_motion=None, using_gt_root_cond=None, using_gt_joints_cond=None, fixed_root_input=None):
        """
        ...
        if self.use_motion_mask:
            if motion_mask is not None:
                ...
                if self.use_obs_diff:
                    motion_diff = ((observed_motion - x_orig) * motion_mask)[:, :self.motion_root_joints_dim]
                    x = torch.cat([x, motion_mask, motion_diff], axis=1)
                else:
                    x = torch.cat([x, motion_mask], axis=1)
            else:
                if self.use_obs_diff:
                    x = torch.cat([x, torch.zeros_like(x), torch.zeros_like(x[:, :self.motion_root_joints_dim])], axis=1)
                else:
        ...
        if self.use_unknownt_motion_mask:
            if unknownt_motion_mask is not None:
                ...
                unknownt_observed_motion = unknownt_observed_motion.repeat(1, 1, 1, x.shape[-1])
                unknownt_observed_motion = unknownt_observed_motion * unknownt_motion_mask
                if self.use_unknownt_obs_diff:
                    unknownt_motion_diff = ((unknownt_observed_motion - x_orig) * unknownt_motion_mask)[:, :self.motion_root_joints_dim]
                    x = torch.cat([x, unknownt_observed_motion, unknownt_motion_mask, unknownt_motion_diff], axis=1)
                else:
                    x = torch.cat([x, unknownt_observed_motion, unknownt_motion_mask], axis=1)
            else:
                if self.use_unknownt_obs_diff:
                    x = torch.cat([x, torch.zeros_like(x_orig), torch.zeros_like(x_orig), torch.zeros_like(x_orig[:, :self.motion_root_joints_dim])], axis=1)
                else:
                    x = torch.cat([x, torch.zeros_like(x_orig), torch.zeros_like(x_orig)], axis=1)
        ...
        if fixed_root_input is not None:
            ...
            root_motion_pred = self.forward_root(x, emb, emb_text, y, x_gt, using_gt_joints_cond)
        ...
        if self.use_foot_skate_feats:
            x_after_root_pred = torch.cat([root_motion_for_joints, x_orig[:, self.motion_root_dim:]], axis=1)
            foot_skate_feats = self.get_foot_skate_feats(x_after_root_pred)
            x_new = torch.cat([root_motion_local.detach(), x[:, self.motion_root_dim:-foot_skate_feats.shape[1]], foot_skate_feats], axis=1)
        else:
