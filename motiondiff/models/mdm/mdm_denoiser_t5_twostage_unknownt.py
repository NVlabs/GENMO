...
class MDMDenoiser(nn.Module):
    def __init__(self, pl_module, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
                 ...
                 global_feat_type=['g_motion_l_joints', 'l_joints_diff', 'global_joint_mask'], normalize_global_pos=False, pred_root_with_joints=False, **kargs):
        super().__init__()
        ...
        input_dim = (self.input_feats * 2 if use_motion_mask else self.input_feats) + self.gru_emb_dim - 1
        if use_obs_diff:
            input_dim += 67
        ...
        self.root_input_feats = (self.input_feats * 2 if use_motion_mask else self.input_feats) if self.pred_root_with_joints else 5
        if use_obs_diff:
            self.root_input_feats += 67
        self.root_output_feats = 5
    ...
    def forward_root(self, x, emb, emb_text, y, x_gt, using_gt_joints_cond):
        ...
        if self.pred_root_with_joints:
            if using_gt_joints_cond is None:
                ...
                xr_gt = torch.cat([xr, x_gt[:, 5:]], axis=1)
    ...
    def forward(self, x, timesteps, y=None, motion_mask=None, observed_motion=None, rm_text_flag=None, global_motion=None, global_joint_mask=None, global_joint_func=None,
        ...
        if self.use_motion_mask:
            if motion_mask is not None:
                len_mask = y['mask']
                motion_mask = motion_mask.repeat(1, 1, 1, x.shape[-1])
                motion_mask = motion_mask * len_mask
                observed_motion = observed_motion.repeat(1, 1, 1, x.shape[-1])
                observed_motion = observed_motion * motion_mask
                assert observed_motion is not None
                x_orig = x
                if self.use_obs_diff:
                    motion_diff = ((observed_motion - x_orig) * motion_mask)[:, :67] # BUG: chopped to only include root + other joint positions
        ...
        with torch.no_grad():
            if using_gt_root_cond is None:
                ...
                root_motion_for_joints = torch.where(using_gt_root_cond[:, None, None, None], x_gt[:, :5], root_motion_pred)
            root_motion_local = self.convert_root_global_to_local(root_motion_for_joints)
        x_new = torch.cat([root_motion_local.detach(), x[:, 5:]], axis=1)
