...
class MDMBase(pl.LightningModule):
    ...
    def __init__(self, cfg, is_inference=False):
        ...
        self.motion_mean = torch.tensor(np.load(f'dataset/EgoSMPL3D/Mean_{self.rep_version}.npy')).float()
        self.motion_std = torch.tensor(np.load(f'dataset/EgoSMPL3D/Std_{self.rep_version}.npy')).float()
        self.motion_std[self.motion_std < 0.1] = 0.1
        self.mean_kpt = torch.tensor(np.load('dataset/EgoSMPL3D/Mean_normed_kpt2d.npy')).float()
        self.mean_cam = torch.tensor(np.load('dataset/EgoSMPL3D/Mean_cam_vel.npy')).float()
        self.std_kpt = torch.tensor(np.load('dataset/EgoSMPL3D/Std_normed_kpt2d.npy')).float()
        self.std_cam = torch.tensor(np.load('dataset/EgoSMPL3D/Std_cam_vel.npy')).float()
        self.std_local_rt = torch.tensor(np.load('dataset/EgoSMPL3D/Std_local_rt.npy')).float()
        self.mean_local_rt = torch.tensor(np.load('dataset/EgoSMPL3D/Mean_local_rt.npy')).float()
        ...
        self.smpl = SMPL(self.cfg.smpl_model_dir, create_transl=False, gender='neutral').to(self.device)
        return
    ...
    def get_diffusion_pred_target(self, data, t, noise=None):
        ...
        if 'kpt2d' in self.cfg.model.denoiser.cond_mode and False:
            data['model_pred'], out_aux = self.denoiser(x_t, diffusion._scale_timesteps(t), return_aux=True, **denoiser_kwargs)

            data['pred_local_orient'] = out_aux['pred_local_orient']
            data["cond_mask_2d"] = out_aux["cond_mask_2d"]
        else:
            data['model_pred'] = self.denoiser(x_t, diffusion._scale_timesteps(t), return_aux=False, **denoiser_kwargs)
    ...
    def generate_motion_mask(self, motion_mask_cfg, motion, lengths, cond, use_mask_type=None, use_unknownt_mask_type=None, return_keyframes=False, mask_cfgs=None, unknownt_mask_cfg=None):
        '''
        ...
        def keyframes(_cfg):
            ...
            for i in range(motion.shape[0]):
                ...
                if not mode in {'root', 'root_pos+rot', 'root_pos'}:
                    motion_mask[i, :3, :, keyframe_idx] = 1.0 # only root + local joint positions
                    motion_mask[i, 9:-4, :, keyframe_idx] = 1.0 # only root + local joint positions
        ...
        def local_joints(_cfg):
            ...
            for i in range(motion.shape[0]):
                ...
                if _cfg.get('consistent_joints', False):    # consistent_joints: masked joints are the same for all the frames
                    ...
                    motion_mask[i, 9:-4, :, keyframe_idx] = torch.from_numpy(joint_mask).repeat_interleave(6)[:, None, None].to(motion.device)
                    motion_mask[i, root_mask_ind, :, keyframe_idx] = 1.0
                else:
                    for fr in keyframe_idx:
                        joint_mask = np.zeros(23, dtype=np.float32)
                        joint_mask[joint_indices] = np.random.binomial(1, _cfg.obs_joint_prob, size=len(joint_indices))
                        motion_mask[i, 9:-4, :, fr] = torch.from_numpy(joint_mask).repeat_interleave(6).to(motion.device)
        ...
        def unknownt_root_traj(_cfg):
            ...
            if self.motion_rep in {'full263', 'position'}:
                ...
                if mode == 'pos+rot':
                    ...
                    root_mask_ind = np.array([0, 1, 3, 4, 5, 6, 7, 8])  # in the original coordinate, y is up, so we use z.
        ...
        def unknownt_keyframes(_cfg):
            ...
            if not mode in {'root', 'root_xy+rot', 'root_xy'}:
                unknownt_motion_mask[:, root_dim:-4] = 1.0 # only root + local joint positions
            unknownt_motion_mask[:, root_mask_ind] = 1.0
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def unknownt_local_joints(_cfg):
            ...
            joint_mask = np.zeros(23, dtype=np.float32)
            joint_mask[joint_indices] = np.random.binomial(1, _cfg.obs_joint_prob, size=len(joint_indices))
            unknownt_motion_mask[:, root_dim:-4] = torch.from_numpy(joint_mask).repeat_interleave(3)[:, None, None].to(motion.device)
            unknownt_motion_mask[:, root_mask_ind] = 1.0
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def plucker_kpt(_cfg):
            ...
            motion_mask = kpt_mask.repeat(1, 1, 1, 7).reshape(bs, nframes, 17 * 7).permute(0, 2, 1).unsqueeze(2).to(motion.device)
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def v12_kpt2d_cam_vel(_cfg):
            ...
            if 'init_observed_motion' in cond['y']:
                observed_motion = cond['y']['init_observed_motion'].transpose(1, 2).unsqueeze(2)
            else:
        ...
        def kpt2d_cam_vel(_cfg):
            ...
            bs, nframes = motion.shape[0], motion.shape[-1]
            kpt_mask = cond['y']['kpt_mask'].reshape(bs, nframes, 17, 1)    # [B, T, 17]
            normed_kpt2d = cond['y']['normed_kpt2d'].reshape(bs, nframes, 17 * 2 + 3)    # [B, T, 17 * 2 + 3]
            ...
            _kpt_mask = kpt_mask.repeat(1, 1, 1, 2).contiguous().reshape(bs, nframes, -1).to(motion.device)
            motion_mask = torch.cat((_kpt_mask, torch.ones(bs, nframes, 3 + 9, device=motion.device)), dim=-1).permute(0, 2, 1).unsqueeze(2)
        ...
        def local_smplfeat_camvel(_cfg):
            nonlocal motion_mask, observed_motion, rm_text_flag
            bs, nframes = motion.shape[0], motion.shape[-1]
            local_smplfeat = cond['y']['local_smplfeat'].reshape(bs, nframes, 147)    # [B, T, 63]
            cam_vel = cond['y']['cam_vel'].reshape(bs, nframes, 9)    # [B, T, 9]
            observed_motion = torch.cat((local_smplfeat, cam_vel), dim=-1).permute(0, 2, 1).unsqueeze(2).to(motion.device) # [B, 63 + 9, 1, T]
            motion_mask = torch.ones(bs, 147 + 9, nframes, device=motion.device).unsqueeze(2)
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def local_smplfeat_cam2world(_cfg):
            ...
            cam2world = cond['y']['cam2world'].reshape(bs, nframes, 9)    # [B, T, 9]
            observed_motion = torch.cat((local_smplfeat, cam2world), dim=-1).permute(0, 2, 1).unsqueeze(2).to(motion.device) # [B, 63 + 9, 1, T]
            motion_mask = torch.ones(bs, 147 + 9, nframes, device=motion.device).unsqueeze(2)
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def local_smplfeat_camplucker(_cfg):
            ...
            bs, nframes = motion.shape[0], motion.shape[-1]
            local_smplfeat = cond['y']['local_smplfeat'].reshape(bs, nframes, 147)
            camplucker = cond['y']['camplucker'].reshape(bs, nframes, 150)    # [B, T, 150]
            observed_motion = torch.cat((local_smplfeat, camplucker), dim=-1).permute(0, 2, 1).unsqueeze(2).to(motion.device) # [B, 147 + 150, 1, T]
            motion_mask = torch.ones(bs, 147 + 150, nframes, device=motion.device).unsqueeze(2)
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def global_smplfeat_cam2world(_cfg):
            ...
            cam2world = cond['y']['cam2world'].reshape(bs, nframes, 9)    # [B, T, 9]
            observed_motion = torch.cat((global_smplfeat, cam2world), dim=-1).permute(0, 2, 1).unsqueeze(2).to(motion.device) # [B, 63 + 9, 1, T]
            motion_mask = torch.ones(bs, 147 + 9, nframes, device=motion.device).unsqueeze(2)
            rm_text_flag = torch.from_numpy(np.random.binomial(1, _cfg.mask_text_prob, size=motion.shape[0])).to(motion.device)
        ...
        def global_kpt3d(_cfg):
            ...
            # cam2world = cond['y']['cam2world'].reshape(bs, nframes, 9)    # [B, T, 9]
            observed_motion = global_kpt3dfeat.permute(0, 2, 1).unsqueeze(2).to(motion.device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        data['init_observed_motion'] = cond['y']['init_observed_motion'].transpose(1, 2).unsqueeze(2)


        if 'motion_mask' in self.model_cfg:
            res = self.generate_motion_mask(self.model_cfg.motion_mask, data['motion'], data['cond']['y']['lengths'], data['cond'])
    ...
    def compute_loss(self, data, t, t_weights):
        ...
        def masked_local_root_l2(cfg):
            mask = data['mask']
            root_motion_pred, root_motion_target = data['model_pred'][:, :self.motion_root_dim], data['target'][:, :self.motion_root_dim]     # this is for global_root_local_joints
        ...
        def foot_sliding(cfg):
            ...
            contact_label = contact_label * self.motion_std[-4:, None].to(self.device) + self.motion_mean[-4:, None].to(self.device)
    ...
    def infer_kpt2d(
        ...
        kpt_mask = cond['y']['kpt_mask'].reshape(batch_size, num_frames, 17, 1)    # [B, T, 17]

        motion_mask_cfg = self.model_cfg.get('motion_mask', {})
        if motion_mask_cfg and 'plucker_kpt' in motion_mask_cfg['mask_types']:
            plucker_kpt = cond['y']['plucker_kpt']  # [B, T, 17, 6]
            plucker_kpt = plucker_kpt * kpt_mask
            plucker_kpt = torch.cat((plucker_kpt, kpt_mask), dim=-1).reshape(batch_size, num_frames, 17 * 7)
            plucker_kpt = plucker_kpt.permute(0, 2, 1).unsqueeze(2) # [B, 17 * 7, 1, T]

            cond["observed_motion"] = plucker_kpt
            cond['motion_mask'] = kpt_mask.repeat(1, 1, 1, 7).reshape(batch_size, num_frames, 17 * 7).permute(0, 2, 1).unsqueeze(2)
        elif motion_mask_cfg and 'kpt2d_cam_vel' in motion_mask_cfg['mask_types']:
            kpt_mask = kpt_mask.reshape(batch_size, num_frames, 17, 1).to(self.device)    # [B, T, 17]
            normed_kpt2d = cond['y']['normed_kpt2d'].reshape(batch_size, num_frames, 17 * 2 + 3).to(self.device)    # [B, T, 17 * 2 + 3]
            cam_vel = cond['y']['cam_vel'].reshape(batch_size, num_frames, 9).to(self.device)    # [B, T, 9]
            normed_kpt2d = normed_kpt2d.reshape(batch_size, num_frames, 17 * 2 + 3).to(self.device)    # [B, T, 17 * 2 + 3]
            cam_vel = cam_vel.reshape(batch_size, num_frames, 9).to(self.device)    # [B, T, 3]

            cond["observed_motion"] = torch.cat((normed_kpt2d, cam_vel), dim=-1).permute(0, 2, 1).unsqueeze(2) # [B, 17 * 2 + 3 + 6 + 3, 1, T]
            _kpt_mask = kpt_mask.repeat(1, 1, 1, 2).contiguous().reshape(batch_size, num_frames, -1)
            cond['motion_mask'] = torch.cat((_kpt_mask, torch.ones(batch_size, num_frames, 3 + 9, device=self.device)), dim=-1).permute(0, 2, 1).unsqueeze(2)
    ...
    def infer_texts(self, texts, num_frames, motion_mask=None, observed_motion=None, rm_text_flag=None, rm_kpt_flag=None, global_motion=None, global_joint_mask=None, global_joint_func=None, unknownt_motion_mask=None, unknownt_observed_motion=None, progress=True):
        diffusion = self.test_diffusion
        batch_size = len(texts)
        _, cond = collate(
            [{'inp': torch.tensor([[0.]]), 'target': 0, 'text': txt, 'tokens': None, 'lengths': num_frames} for txt in texts]
    ...
    def obtain_joints_and_smpl_pose(self, samples, smpl, infer_kwargs=None, interp=True, return_contacts=False):
        ...
        smpl_pose, smpl_trans, joints_pos = self.motion2global(
            samples, mean=self.motion_mean.to(self.device), std=self.motion_std.to(self.device), smpl=smpl,
            mean_kpt=self.mean_kpt.to(self.device), std_kpt=self.std_kpt.to(self.device), mean_cam=self.mean_cam.to(self.device), std_cam=self.std_cam.to(self.device), mean_local_rt=self.mean_local_rt.to(self.device), std_local_rt=self.std_local_rt.to(self.device))

        if return_contacts and self.motion_rep in {'egosmpl_v1_contact', 'egosmpl_v2_contact', 'egosmpl_v3_contact', 'egosmpl_v4_contact'}:
            # print(samples.size())
            ...
            foot_contacts = samples[:, -4:, 0, :].permute(0, 2, 1) # [batch, seq_len, 4] where 4 is ("L_Ankle", "L_Toe", "R_Ankle", "R_Toe")
            
            # print(foot_contacts.size())
            # should be between 0 and 1 unnormalized
            foot_contacts_norm = foot_contacts * self.motion_std[-4:].to(samples.device) + self.motion_mean[-4:].to(samples.device)
            # if interp:
