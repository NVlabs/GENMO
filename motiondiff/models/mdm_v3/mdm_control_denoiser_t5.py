...
class ControlTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.max_control_layer = (
            max(self.control_layers_dict.values())
            if self.control_layers_dict is not None
            else -1
        )

        self.zero_conv_layers = nn.ModuleList(
            [zero_module(nn.Linear(d_model, d_model)) for _ in range(num_layers)]
        )
    ...
    def forward(
        ...
        for mod, hint_mod, z_conv in zip(
            enc_layers, self.hint_layers, self.zero_conv_layers
        ):
            output = mod(
                output,
                src_mask=mask,
            ...
            if layer_id in self.control_layers_dict.keys() and hint is not None:
                output = (
                    output
                    + z_conv(hint_out_dict[self.control_layers_dict[layer_id]])
                    * hint_mask[None, :, None]
                )
            layer_id += 1
        ...
        **kwargs,
        ...
        if self.use_motion_mask:
            ...
            if motion_mask is not None:
                ...
                if self.use_obs_diff:
                    x = torch.cat(
                        [x, torch.zeros_like(x), torch.zeros_like(x[:, :67])], axis=1
                    )
                else:
