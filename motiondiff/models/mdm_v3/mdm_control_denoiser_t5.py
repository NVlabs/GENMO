...
class ControlTransformerEncoder(nn.Module):
    ...
    def __init__(self, hint_layer, d_model, num_layers, control_layers_dict=None, norm=None, enable_nested_tensor=True, mask_check=True):
        super(ControlTransformerEncoder, self).__init__()
        self.hint_layers = _get_clones(hint_layer, num_layers)

        self.control_layers_dict = {}
        if control_layers_dict is not None:
            for k, v in control_layers_dict.items():
                self.control_layers_dict[int(k[1:])] = int(v[1:])

        self.max_control_layer = max(self.control_layers_dict.values()) if self.control_layers_dict is not None else -1

        self.zero_conv_layers = nn.ModuleList([zero_module(nn.Linear(d_model, d_model)) for _ in range(num_layers)])
        self.zero_conv_init = zero_module(nn.Linear(d_model, d_model))
    ...
    def forward(self, enc_layers, src: Tensor, hint: Tensor, hint_mask: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        first_layer = self.hint_layers[0]
        ...
        if hint is not None:
            hint_out = self.zero_conv_init(hint) + src

        for mod, hint_mod, z_conv in zip(enc_layers, self.hint_layers, self.zero_conv_layers):
            output = mod(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask_for_layers)

            if layer_id <= self.max_control_layer and hint is not None:
                hint_out = hint_mod(hint_out, src_mask=mask, src_key_padding_mask=src_key_padding_mask_for_layers)
                hint_out_dict[layer_id] = hint_out

            if layer_id in self.control_layers_dict.keys() and hint is not None:
                output = output + z_conv(hint_out_dict[self.control_layers_dict[layer_id]]) * hint_mask[None, :, None]
            layer_id += 1
...
class MDMDenoiser(nn.Module):
    def __init__(self, pl_module, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        self.hint_rep = kargs.get("hint_rep", "plucker_kpt")
        if self.hint_rep == "plucker_kpt":
            self.hint_dim = (17 * 7) * 2
        elif self.hint_rep == "local_smplfeat_camvel":
            self.hint_dim = (147 + 9) * 2
        elif self.hint_rep == "local_smplfeat_cam2world":
            self.hint_dim = (147 + 9) * 2
        elif self.hint_rep == "local_smplfeat_camplucker":
            self.hint_dim = (147 + 150) * 2
        elif self.hint_rep == "global_smplfeat_cam2world":
            self.hint_dim = (147 + 9) * 2
        elif self.hint_rep == "kpt2d_cam_vel":
            self.hint_dim = (17 * 2 + 3 + 6 + 3) * 2
        elif self.hint_rep == "kpt2d_cam2world":
            self.hint_dim = (17 * 2 + 3 + 6 + 3) * 2
        elif self.hint_rep == "global_kpt3d":
            self.hint_dim = 51 * 2
        elif self.hint_rep == 'motion_v10':
            self.hint_dim = 268 * 2
        ...
        self.hint_dim = self.hint_dim + self.gru_emb_dim
        if use_obs_diff:
            input_dim += 67
        ...
        self.hint_process = InputProcess(self.data_rep, self.hint_dim, self.latent_dim)
        ...
        if self.arch == 'trans_enc':
            ...
            hint_seqTransEncoderLayer = nn.TransformerEncoderLayer(
            ...
            control_layers_dict = kargs.get("control_layers")
            self.control_seqTransEncoder = ControlTransformerEncoder(
                hint_layer=hint_seqTransEncoderLayer,
                d_model=self.latent_dim,
                num_layers=self.num_layers,
                control_layers_dict=control_layers_dict,
            )
    ...
    def forward(self, x, timesteps, y=None, motion_mask=None, observed_motion=None, rm_text_flag=None, rm_kpt_flag=None, global_motion=None, global_joint_mask=None, global_joint_func=None, return_aux=False, **kwargs):
        ...
        # project hint
        if observed_motion is not None and rm_kpt_flag is None:
            # observed_motion, motion_mask: [bs, nfeats, 1, nframes]
            hint = (
                torch.zeros_like(observed_motion) * (1 - motion_mask)
                + observed_motion * motion_mask
            )
            hint = torch.cat([hint, motion_mask], axis=1)
            hint = self.hint_process(hint)
            hint_mask = (motion_mask.sum(dim=(1, 2, 3)) > 0).float()
        else:
            hint = None
            hint_mask = torch.zeros_like(x[0, :, 0])
        ...
        if self.arch == "trans_enc":
            ...
            if hint is not None:
                hintseq = torch.cat((emb, hint), axis=0)  # [seqlen+1, bs, d]
            else:
                hintseq = None
            ...
            if self.text_enc_mode == 'seq_concat':
                ...
                if hint is not None:
                    hintseq = torch.cat((emb_text.transpose(0, 1), hintseq), axis=0)
                pose_start_ind += emb_text.shape[1]
            xseq = self.sequence_pos_encoder(xseq)  # [seqlen+1, bs, 2d]
            if hint is not None:
                hintseq = self.sequence_pos_encoder(hintseq)  # [seqlen+1, bs, d]
            ...
            output = self.control_seqTransEncoder(
                self.seqTransEncoder.layers,
                src=xseq,
                hint=hintseq,
                hint_mask=hint_mask,
