        ...
        if "pretrained_checkpoint" in self.model_cfg:
            ...
            if len(missing_keys) > 0:
                for k in missing_keys:
                    assert (
                        k.startswith("denoiser.control_seqTransEncoder")
                        or k.startswith("denoiser.hint_process")
                        or k.startswith("smpl.")
                    ), k
                print(f"Missing keys: {missing_keys}")
            if len(unexpected_keys) > 0:
                print(f"Unexpected keys: {unexpected_keys}")
            assert (
                "denoiser.control_seqTransEncoder.hint_layers.0.linear1.weight"
                in self.state_dict().keys()
            )
            if (
                "denoiser.control_seqTransEncoder.hint_layers.0.linear1.weight"
                ...
                for k, v in state_dict.items():
                    if k.startswith("denoiser.seqTransEncoder"):
                        new_k = k.replace(
                            "denoiser.seqTransEncoder.layers", "hint_layers"
                        )
                        seqTransEncoder_state_dict[new_k] = v.clone()

                missing_keys, unexpected_keys = (
                    self.denoiser.control_seqTransEncoder.load_state_dict(
                        seqTransEncoder_state_dict, strict=False
                    )
                )
