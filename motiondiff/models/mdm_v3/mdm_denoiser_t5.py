        ...
        **kwargs,
        ...
        if "kpt2d+cam_angvel" in self.cond_mode:
            ...
            if rm_kpt_flag is not None or force_mask:
                ...
                if self.training:
                    cond_mask_2d = 1 - torch.bernoulli(
                        torch.ones(bs, device=x.device) * self.cond_kptcammask_prob
                    ).view(bs, 1, 1)
                else:
            ...
            _kpt_mask = (
                kpt_mask.unsqueeze(-1)
                .repeat(1, 1, 1, 2)
                .contiguous()
                .reshape(bs, nframes, -1)
            )
            _kpt_mask = torch.cat(
                (_kpt_mask, torch.ones_like(_kpt_mask[..., :3])), dim=-1
            )
            ...
            cam_angvel_emb = self.embed_cam_angvel(
                cond_mask_2d * cam_angvel
            )  # [B, T, N]
        ...
        if "local_smplfeat" in self.cond_mode:
            ...
            if rm_kpt_flag is not None or force_mask:
                ...
                if self.training:
                    cond_mask_local_smplfeat = 1 - torch.bernoulli(
                        torch.ones(bs, device=x.device) * self.cond_kptcammask_prob
                    ).view(bs, 1, 1)
                else:
                    cond_mask_local_smplfeat = torch.ones(bs, 1, 1, device=x.device)
            local_smplfeat_emb = self.embed_local_smplfeat(
                cond_mask_local_smplfeat * local_smplfeat
            )
        ...
        if "global_smplfeat" in self.cond_mode:
            ...
            if rm_kpt_flag is not None or force_mask:
                ...
                if self.training:
                    cond_mask_global_smplfeat = 1 - torch.bernoulli(
                        torch.ones(bs, device=x.device) * self.cond_kptcammask_prob
                    ).view(bs, 1, 1)
                else:
                    cond_mask_global_smplfeat = torch.ones(bs, 1, 1, device=x.device)
            global_smplfeat_emb = self.embed_global_smplfeat(
                cond_mask_local_smplfeat * local_smplfeat
            )
        ...
        if "cam2world" in self.cond_mode:
            ...
            if rm_kpt_flag is not None or force_mask:
                ...
                if self.training:
                    cond_mask_cam2world = 1 - torch.bernoulli(
                        torch.ones(bs, device=x.device) * self.cond_kptcammask_prob
                    ).view(bs, 1, 1)
                    if "local_smplfeat" in self.cond_mode:
                        cond_mask_cam2world = (
                            cond_mask_cam2world * cond_mask_local_smplfeat
                        )
                    if "global_smplfeat" in self.cond_mode:
                        cond_mask_cam2world = (
                            cond_mask_cam2world * cond_mask_global_smplfeat
                        )
        ...
        if "cam_vel" in self.cond_mode:
            ...
            if rm_kpt_flag is not None or force_mask:
                ...
                cam_vel = y["cam_vel"]
                if self.training:
                    cond_mask_camvel = 1 - torch.bernoulli(
                        torch.ones(bs, device=x.device) * self.cond_camvelmask_prob
                    ).view(bs, 1, 1)
        ...
        if self.use_motion_mask:
            ...
            if motion_mask is not None:
                ...
                if force_motion_mask is not None:
                    ...
                    init_observed_motion = init_observed_motion.transpose(
                        1, 2
                    ).unsqueeze(2)
                    motion_mask = (
                        motion_mask * (1 - force_motion_mask)
                        + init_motion_mask * force_motion_mask
                    )
                    observed_motion = (
                        observed_motion * (1 - force_motion_mask)
                        + init_observed_motion * force_motion_mask
                    )
        ...
        if self.arch == "trans_enc":
            ...
            if "kpt2d+cam_angvel" in self.cond_mode:
                xseq = torch.cat(
                    (x, kpt2d_emb.transpose(0, 1), cam_angvel_emb.transpose(0, 1)),
                    axis=2,
                )  # [seqlen, bs, 2d]
            ...
            if True:
                xseq[pose_start_ind:,] = xseq[pose_start_ind:,] * y["mask"].squeeze(
                    1
                ).permute(2, 0, 1)
        ...
        pred_cam = pred_cam * self.pred_cam_std.reshape(
            1, 3, 1, 1
        ) + self.pred_cam_mean.reshape(1, 3, 1, 1)
