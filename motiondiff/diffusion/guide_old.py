...
class GuideFunctions:
    def __init__(
        ...
        self.alpha = [
            [float(guide["alpha"])]
            if isinstance(guide["alpha"], float) or isinstance(guide["alpha"], int)
            else list(guide["alpha"])
            for _, guide in guide_dict.items()
        ]
        # any additional params for each guidance function
        self.guide_kwargs = [
            guide["kwargs"] if "kwargs" in guide else None
            for _, guide in guide_dict.items()
        ]
    ...
    def guide(self, model_in, pred, target, model_variance, t, model_kwargs):
        ...
        with torch.enable_grad():
            ...
            loss_tot = 0.0
            for cur_obj, cur_name, cur_alphas, cur_kwargs in zip(
                self.obj, self.guide_names, self.alpha, self.guide_kwargs
            ):
                ...
                assert len(losses) == len(cur_alphas), (
                    "Number of alphas given for %s must match the number of losses returned"
                    % (cur_obj)
                )

                for loss, loss_name, loss_alpha in zip(
                    losses, self.loss_names[cur_obj], cur_alphas
                ):
                    if loss is None:
                        continue
    ...
    def evaluate(self, pred, target):
        ...
        eval_res = dict()
        for cur_obj, cur_name, cur_kwargs in zip(
            self.obj, self.guide_names, self.guide_kwargs
        ):
            if cur_obj not in self.eval_funcs:
                print(f"{cur_obj}'s eval function is not implemented. skipping")
                eval_res.update(
                    {cur_obj + "_missing_eval": -1 * torch.ones([pred_p.size(0)])}
                )
                continue
            base_kwargs = {
                "pred": pred_c,
                "target": target_c,
                "t": torch.zeros((pred_p.size(0))),
            }
            if cur_kwargs is not None:
                base_kwargs.update(cur_kwargs)
    ...
    def init_model_mask(self, gt_motion, gt_data_lengths, mask_motion_lengths):
        ...
        for cur_obj, cur_mask, cur_kwargs in zip(
            self.obj, self.mask, self.guide_kwargs
        ):
            ...
            if cur_obj == "keyframes":
                ...
                for i in range(gt_motion.shape[0]):
                    ...
                    if cur_kwargs.keyframe_inds == "all":
                        ...
                        keyframe_idx = np.array(cur_kwargs.keyframe_inds) * (
            ...
            if cur_mask is not None and cur_mask != "no_mask":
                ...
                if cur_mask.split("_")[0] == "unknownt":
                    assert cur_kwargs.unknown_time, (
    ...
    def unified_keyframes_loss(
        ...
        assert use_global_loss or use_local_loss, (
            "must pass either global or local joint names to keyframes loss"
        )
        ...
        if not use_local_loss:
            ...
            local_joint_names = set(
                SMPL_BONE_ORDER_NAMES[1:22]
            )  # no hands and leave out root
        else:
        ...
        if keyframe_inds != "all":
            ...
            keyframe_inds = (
                torch.tensor([keyframe_inds]).expand((B, -1)).to(pred.device)
            )
            if keyframe_ratio:
                data_len = (
                    torch.ones((B, 1)) * nframes
                    if self.tgt_data_len is None
                    else self.tgt_data_len.unsqueeze(1)
                )
        ...
        if (
            use_global_loss or unknown_time
        ):  # will always need global for unknown time to compute min dist keyframe
            global_pred, global_target = self.local_to_global_264(
                pred=local_pred_pose, target=local_target_pose
            )
        ...
        if use_global_loss or unknown_time:
            if keyframe_inds != "all":
                global_pos_keyframe_inds = keyframe_inds[:, None, :, None, None].expand(
                    B, 1, nkeys, 22, 3
                )  # expand to same size as poses
                # compare all global joint positions at each desired keyframe
                global_pred_keyframes = (
                    global_pred_pose
                    if unknown_time
                    else torch.gather(global_pred_pose, 2, global_pos_keyframe_inds)
                )
                global_tgt_keyframes = torch.gather(
                    global_target_pose, 2, global_pos_keyframe_inds
                )
            else:
        ...
        all_joint_inds = [
            SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in all_joint_names
        ]
        global_joint_inds = [
            SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in global_joint_names
        ]
        if unknown_time:
            if keyframe_inds == "all":
            ...
            global_pred_keyframes = global_pred_keyframes[:, :, None].expand(
                B, 1, nkeys, nframes, 22, global_pred_keyframes.size(-1)
            )  # [B, 1, nkeys, nframes, 22, 3 or 2]
            global_tgt_keyframes = global_tgt_keyframes[
                :, :, :, None
            ]  # [B, 1, nkeys, 1, 22, 3]
            ...
            if min_dist_idx is None:
                ...
                unknown_time_pose_diff = (
                    global_pred_keyframes[:, :, :, :, all_joint_inds]
                    - global_tgt_keyframes[:, :, :, :, all_joint_inds]
                )  # [B, 1, nkeys, nframes, njoints, 3]
                ...
                pose_dist = torch.norm(
                    unknown_time_pose_diff, dim=-1
                )  # [B, 1, nkeys, nframes, njoints]
                ...
                if SOFTMIN:
                    ...
                    # hardmin
                    min_dist_idx = torch.argmin(
                        pose_dist, dim=-1, keepdim=True
                    )  # [B, 1, nkeys, 1]
        ...
        if (use_global_loss or unknown_time) and self.pred_motion_rep == "global_root":
            print(
                "WARNING: with global_root rep, global keyframe loss will only be on pelvis location"
            )
        if use_global_loss and unknown_time:
            global_pose_diff = (
                global_pred_keyframes[:, :, :, :, global_joint_inds]
                - global_tgt_keyframes[:, :, :, :, global_joint_inds]
            )  # [B, 1, nkeys, nframes, njoints, 3]
            njoints = len(global_joint_inds)
            pos_min_dist_idx = min_dist_idx[..., None, None].expand(
                B, 1, nkeys, 1, njoints, global_pred_keyframes.size(-1)
            )  # [B, 1, nkeys, 1, njoints, 3 or 2]
            global_pose_loss = torch.gather(global_pose_diff**2, 3, pos_min_dist_idx)[
                :, :, :, 0
            ]  # [B, 1, nkeys, njoints, 3 or 2]
            ...
            global_loss = torch.mean(
                global_pose_loss, dim=-1
            )  # [B, 1, num_keys, num_joints]
        ...
        if use_global_loss and use_root_orient and "Pelvis" in global_joint_names:
            if keyframe_inds != "all":
                global_rot_keyframe_inds = keyframe_inds[:, None, :, None].expand(
                    B, 1, keyframe_inds.size(1), 4
                )  # expand to same size as rot quats
                pred_rot_keyframe = (
                    global_pred_rot_quat
                    if unknown_time
                    else torch.gather(global_pred_rot_quat, 2, global_rot_keyframe_inds)
                )
                tgt_rot_keyframe = torch.gather(
                    global_target_rot_quat, 2, global_rot_keyframe_inds
                )
            else:
            ...
            if unknown_time:
                ...
                pred_rot_keyframe = pred_rot_keyframe[:, :, None].expand(
                    B, 1, nkeys, nframes, 4
                )  # [B, 1, nkeys, nframes, 4]
                tgt_rot_keyframe = tgt_rot_keyframe[
                    :, :, :, None
                ]  # [B, 1, nkeys, 1, 4]
                # get the weighting based on previously computed position distance over desired joints
                global_rot_diff = (
                    pred_rot_keyframe - tgt_rot_keyframe
                )  # [B, 1, nkeys, nframes, 4]
                if SOFTMIN:
                    raise NotImplementedError("unified keyframes only supports hardmin")
                else:
                    # hardmin
                    rot_min_dist_idx = min_dist_idx.unsqueeze(-1).expand(
                        B, 1, nkeys, 1, 4
                    )  # [B, 1, nkeys, 1, 4]
                    global_rot_loss = torch.gather(
                        global_rot_diff**2, 3, rot_min_dist_idx
                    )[:, :, :, 0]  # [B, 1, nkeys, 4]
                global_rot_loss = torch.mean(
                    global_rot_loss, dim=-1, keepdim=True
                )  # [B, 1, nkeys, 1]
            else:
            ...
            global_loss = torch.cat(
                [global_loss, global_rot_loss], dim=-1
            )  # [B, 1, num_keys, num_joints+1]
        ...
        if use_global_loss and keyframe_inds == "all":
            ...
            global_loss = torch.cat(
                [
                    global_loss[bi, :, : self.tgt_data_len[bi]].flatten()
                    for bi in range(B)
                ]
            )
        ...
        if use_local_loss:
            assert "Pelvis" not in local_joint_names, (
                "Cannot apply a local loss to the root (it is at the origin)"
            )
            # pull out only keyframe poses
            local_pred_keyframes = local_tgt_keyframes = None
            if keyframe_inds != "all":
                local_keyframe_inds = keyframe_inds[:, None, :, None].expand(
                    B, 1, keyframe_inds.size(1), nfeat
                )  # expand to same size as poses
                # compare all global joint positions at each desired keyframe
                local_pred_keyframes = (
                    local_pred_pose
                    if unknown_time
                    else torch.gather(local_pred_pose, 2, local_keyframe_inds)
                )  # [batch, 1, nkeys, nfeat]
                local_tgt_keyframes = torch.gather(
                    local_target_pose, 2, local_keyframe_inds
                )  # [batch, 1, nkeys, nfeat]
            else:
            ...
            local_joint_inds = [
                SMPL_BONE_ORDER_NAMES.index(joint_name) - 1
                for joint_name in local_joint_names
            ]  # the root is not considered here so subtract 1
            local_joint_inds = np.concatenate(
                [np.arange(5 + idx * 3, 5 + idx * 3 + 3) for idx in local_joint_inds]
            )
            if unknown_time:
                if keyframe_inds == "all":
                    # in this case each batch index will have a different number of "keyframes" (i.e. sequence length) which gets annoying to deal with
                    raise NotImplementedError(
                        "unknown time only supports sparse keyframes"
                    )

                # pred_keyframes is [batch, 1, nframes, nfeat]
                # tgt_keyframes is [batch, 1, nkeys, nfeat]
                local_pred_keyframes = local_pred_keyframes[:, :, None].expand(
                    B, 1, nkeys, nframes, nfeat
                )
                local_tgt_keyframes = local_tgt_keyframes[
                    :, :, :, None
                ]  # [batch, 1, nkeys, 1, nfeat]
                local_pose_diff = (
                    local_pred_keyframes[..., local_joint_inds]
                    - local_tgt_keyframes[..., local_joint_inds]
                )  # [batch, 1, nkeys, nframes, njoint_feats]

                local_min_dist_idx = min_dist_idx.unsqueeze(-1).expand(
                    B, 1, nkeys, 1, local_pose_diff.size(-1)
                )  # [B, 1, nkeys, 1, njoint_feats]
                local_loss = torch.gather(local_pose_diff**2, 3, local_min_dist_idx)[
                    :, :, :, 0
                ]  # [B, 1, nkeys, njoint_feats]
            else:
                local_pose_diff = (
                    local_pred_keyframes[:, :, :, local_joint_inds]
                    - local_tgt_keyframes[:, :, :, local_joint_inds]
            ...
            if keyframe_inds == "all":
                ...
                local_loss = torch.cat(
                    [
                        local_loss[bi, :, : self.tgt_data_len[bi]].flatten()
                        for bi in range(B)
                    ]
                )
    ...
    def unified_keyframes_eval(
        ...
        loss_out = self.unified_keyframes_loss(
            pred,
            target,
            t,
            keyframe_inds,
        ...
        if global_loss is not None:
            ...
            if unknown_time:
                ...
                global_joints_min_idx = min_dist_idx[..., None].expand(
                    B, 1, nkeys, 1, njoints_global
                )
                global_joints_err = torch.gather(
                    global_joints_err, 3, global_joints_min_idx
                )[:, :, :, 0]  # [B, 1, nkeys, njoints]

            if keyframe_inds == "all":
                global_joints_err = torch.stack(
                    [
                        torch.mean(global_joints_err[bi, :, : self.tgt_data_len[bi]])
            ...
            if global_rot_diff is not None:
                ...
                if unknown_time:
                    ...
                    global_rot_min_idx = min_dist_idx[..., None].expand(
                        B, 1, nkeys, 1, D
                    )
                    global_rot_diff = torch.gather(
                        global_rot_diff, 3, global_rot_min_idx
                    )[:, :, :, 0]

                global_rot_diff = global_rot_diff[
                    :, :, :, [0, 2]
                ]  # [batch, 1, num_keys, 2]
                ...
                ang_err = torch.abs(
                    torch.rad2deg(torch.atan2(err_vec[:, :, :, 1], err_vec[:, :, :, 0]))
                )
                if keyframe_inds == "all":
                    ang_err = torch.stack(
                        [
                            torch.mean(ang_err[bi, :, : self.tgt_data_len[bi]])
                            for bi in range(pred.size(0))
                        ]
                    )
                else:
        ...
        if local_loss is not None:
            if unknown_time:
                ...
                local_min_idx = min_dist_idx[..., None].expand(
                    B, 1, nkeys, 1, njoint_feats
                )
                local_pose_diff = torch.gather(local_pose_diff, 3, local_min_idx)[
                    :, :, :, 0
                ]

            # local_pose_diff is [batch, 1, nkeys, njoint_feats]
            njoints = 21 if local_joint_names == "all" else len(local_joint_names)
            local_pose_diff = local_pose_diff.reshape(
                (pred.size(0), 1, -1, njoints, 3)
            )  # [batch, 1, nkeys, njoints, 3]
            local_joints_err = torch.norm(
                local_pose_diff, dim=-1
            )  # [batch, 1, num_keys, num_joints]
            if keyframe_inds == "all":
                local_joints_err = torch.stack(
                    [
                        torch.mean(local_joints_err[bi, :, : self.tgt_data_len[bi]])
                        for bi in range(pred.size(0))
                    ]
                )
            else:
            ...
            local_in_glob_loss_out = self.unified_keyframes_loss(
                pred,
                target,
                t,
                keyframe_inds,
            ...
            if unknown_time:
                ...
                # compute loss at the timestep that the loss determines was optimal/minimal
                global_joints_min_idx = min_dist_idx[..., None].expand(
                    B, 1, nkeys, 1, njoints_local
                )
                local_in_glob_joints_err = torch.gather(
                    local_in_glob_joints_err, 3, global_joints_min_idx
                )[:, :, :, 0]  # [B, 1, nkeys, njoints]

            if keyframe_inds == "all":
                local_in_glob_joints_err = torch.stack(
                    [
                        torch.mean(
                            local_in_glob_joints_err[bi, :, : self.tgt_data_len[bi]]
                        )
                        for bi in range(pred.size(0))
                    ]
                )
            else:
                local_in_glob_joints_err = torch.mean(
                    local_in_glob_joints_err, (1, 2, 3)
                )
        ...
        if min_dist_idx is not None:
            ...
            if len(self.unknown_min_dist_tracker) > 0:
                eval_out["unknown_keyframe_ind_track"] = np.stack(
                    self.unknown_min_dist_tracker, axis=0
                )
    ...
    def foot_skate_loss(
        self, pred, target, t, exp_weighting=False, exp_alpha=0.01, return_raw=False
    ):
        # get foot contacts
        foot_contacts = pred[
            ..., -4:
        ]  # [batch, 1, nframes, 4] where 4 is ("L_Ankle", "L_Toe", "R_Ankle", "R_Toe")

        # compute velocities of global foot joints
        foot_joint_names = ["L_Ankle", "L_Toe", "R_Ankle", "R_Toe"]
        foot_joint_idx = [
            SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in foot_joint_names
        ]
    ...
    def foot_skate_eval(self, pred, target, t, exp_weighting, exp_alpha):
        _, foot_vel, foot_contacts = self.foot_skate_loss(
            pred, target, t, False, exp_alpha, return_raw=True
        )
        ...
        vel_err = torch.sum(vel_err, (1, 2, 3)) / torch.sum(
            foot_contacts, (1, 2, 3)
        )  # mean over contacting frames
