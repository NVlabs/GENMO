...
from motiondiff.data_pipeline.bones.motion_process import (
    ...
    recover_local_joint_pos_with_joint_rot,
)
from motiondiff.models.common.bones import BONES_BONE_ORDER_NAMES
from motiondiff.diffusion.guide_funcs_bones import GUIDE_OBJECTIVES
import motiondiff.diffusion.guide_funcs_bones as guide_funcs

GUIDE_TYPES = {'soft'}
GRAD_THRESH_OPTIONS = {None, 'none', 'var', 'clip_norm', 'clip_value'}
PRED_THRESH_OPTIONS = {None, 'none', 'static', 'dynamic'}

MOTION_REPS = {'orig', 'global_root_local_joints', 'global_root_local_joints_root_rot'}

NUM_JOINTS = len(BONES_BONE_ORDER_NAMES)
NUM_BODY_JOINTS = NUM_JOINTS-1

class Guide():
    def __init__(self, cfg, model, diff_steps, batch_size, pred_nframes, tgt_data_len=None,
                    pred_motion_rep='orig', tgt_motion_rep='orig', num_joints=29):
        '''
        Diffusion guidance.
        '''
        self.cfg = cfg
        ...
        self.mean = model.motion_mean.to(model.device)
        self.std = model.motion_std.to(model.device)
        self.normalize_global_pos = model.normalize_global_pos
        self.global_mean = model.motion_global_mean.to(model.device)
        self.global_std = model.motion_global_std.to(model.device)
        self.model = model
        ...
        self.guides = {
            k : GUIDE_OBJECTIVES[guide['objective']](weight=guide['alpha'], **guide['kwargs']) if 'kwargs' in guide else
                GUIDE_OBJECTIVES[guide['objective']](weight=guide['alpha'])
        ...
        for cur_guide in self.guides.values():
            if isinstance(cur_guide, guide_funcs.KeyframeBase):
                cur_guide.init_keyframe_inds(self.batch_size, self.pred_nframes, self.tgt_data_len, model.device)
    ...
    def guide(self, model_in, pred, target, model_variance, t):
        ...
        with torch.enable_grad():
            ...
            global_pred, global_target = self.local_to_global_264(pred=pred_c, target=target_c)
            global_pred_pose, global_pred_rot_quat = global_pred
            global_target_pose, global_target_rot_quat = global_target

            start_local = 5
            end_local = start_local + NUM_BODY_JOINTS * 3
            ...
            for cur_name, cur_guide in self.guides.items():
                if cur_guide.guide_type == guide_funcs.FootSkate.guide_type:
                    ...
                    loss_out = cur_guide.loss(global_pred_rot_quat, global_target_rot_quat)
                elif cur_guide.guide_type == guide_funcs.LocalJointPos.guide_type:
                    loss_out = cur_guide.loss(local_pred_pose, local_target_pose)
                elif cur_guide.guide_type == guide_funcs.LocalJointPosFromRot.guide_type:
                    assert self.model.motion_rep in {'global_root_local_joints_root_rot'}
                    r_pos = global_pred_pose[:,:,:,0]
                    local_pred_pose_from_rot = recover_local_joint_pos_with_joint_rot(pred_c, self.model.neutral_joints.to(pred_c.device), 
                                                                                      self.model.joint_parents.to(pred_c.device), r_pos,
                                                                                       root_dim=5)[:,:,1:] # no root
                    local_pred_pose_from_rot = local_pred_pose_from_rot[:,None].reshape(local_target_pose.shape)
                    loss_out = cur_guide.loss(local_pred_pose_from_rot, local_target_pose)
                else:
                    raise NotImplementedError('Guide type %s has not been implemented!' % (cur_guide.guide_type))
    ...
    def evaluate(self, target, pred_full_rep=None, global_pred_pose=None, pred_contacts=None):
        '''
        ...
        target_p = self.preprocess(target.permute(0,2,3,1), motion_rep=self.tgt_motion_rep)
        target_c = self.convert_to_264(target_p, motion_rep=self.tgt_motion_rep)
        global_target_pose, global_target_rot_quat = self.local_to_global_264(target=target_c)
        start_local = 5
        end_local = start_local + NUM_BODY_JOINTS * 3
        local_target_pose = target_c[..., start_local:end_local]

        if pred_full_rep is not None:
            pred_p = self.preprocess(pred_full_rep.permute(0,2,3,1), motion_rep=self.pred_motion_rep)
            pred_c = self.convert_to_264(pred_p, motion_rep=self.pred_motion_rep)
            global_pred_pose, global_pred_rot_quat = self.local_to_global_264(pred=pred_c)
            local_pred_pose = pred_c[..., start_local:end_local]
            pred_contacts = pred_c[..., -4:]
        elif global_pred_pose is not None:
            global_pred_pose = global_pred_pose.unsqueeze(1) # [batch, 1, nframes, 22, 3]
            pred_contacts = pred_contacts.permute(0, 2, 3, 1) # [batch, 1, nframes, 4]
            pred_contacts = pred_contacts * self.std[-4:] + self.mean[-4:] # unnormalize
            # don't have these, will have to skip metrics that involve them
            global_pred_rot_quat = None
            local_pred_pose = None
        else:
            print("Couldn't evaluate guidance! Must pass in either the predicted full rep or joint positions.")
            return dict()
        ...
        for cur_name, cur_guide in self.guides.items():
            if cur_guide.guide_type == guide_funcs.FootSkate.guide_type:
                ...
                eval_out = cur_guide.eval(local_pred_pose, local_target_pose,
                ...
                if pred_full_rep is not None:
                    ...
                    local_pred_pose_from_rot = recover_local_joint_pos_with_joint_rot(pred_c, self.model.neutral_joints.to(pred_c.device), 
                                                                                        ...
                                                                                        root_dim=5)[:,:,1:] # no root
                    local_pred_pose_from_rot = local_pred_pose_from_rot[:,None].reshape(local_target_pose.shape)
                    global_pred_pose_from_rot, _ = self.local_to_global_264(pred=pred_c, calc_pos_from_rot=True)
                    eval_out = cur_guide.eval(local_pred_pose_from_rot, local_target_pose,
                                                global_pred_pose_from_rot, global_target_pose)
            else:
                raise NotImplementedError('Guide type %s has not been implemented!' % (cur_guide.guide_type))
    ...
    def init_model_mask(self, gt_motion, gt_data_lengths, mask_motion_lengths):
        ...
        for cur_guide, cur_mask in zip(self.guides.values(), self.mask):
            ...
            if isinstance(cur_guide, guide_funcs.KeyframeBase):
                ...
                for i in range(gt_motion.shape[0]):
                    ...
                    if cur_guide.keyframe_inds_in == 'all':
                        ...
                        keyframe_idx = np.array(cur_guide.keyframe_inds_in) * (gt_data_lengths[i].item() - 1)
            ...
            if cur_mask is not None and cur_mask != 'no_mask':
                ...
                if cur_mask in {'local_joints_ee_sf', 'local_joints_ee_af'}:
                    ...
                    assert cur_guide.guide_type in {'keyframes', 'global_joint_pos', 'global_root_orient'}
                    if cur_guide.guide_type != 'global_root_orient':
                        assert cur_guide.use_root_2d, "guidance and mask are inconsistent wrt 2D or 3D root constraint"
                    if cur_guide.guide_type == 'keyframes':
                        # make sure we're only using rotation if desired
                        cur_mask_cfg.mode = 'pos_xy+rot' if cur_guide.use_root_orient else 'pos_xy'
                    else:
                        cur_mask_cfg.mode = 'pos_xy+rot' if cur_guide.guide_type == 'global_root_orient' else 'pos_xy'
                elif cur_mask in {'root_traj'}:
                    # root 3d full trajectory
                    assert cur_guide.guide_type in {'keyframes', 'global_joint_pos', 'global_root_orient'}
                    if cur_guide.guide_type != 'global_root_orient':
                        assert not cur_guide.use_root_2d, "guidance and mask are inconsistent wrt 2D or 3D root constraint"
                    if cur_guide.guide_type == 'keyframes':
                        cur_mask_cfg.mode = 'pos+rot' if cur_guide.use_root_orient else 'pos'
                elif cur_mask in {'keyframes'}:
                    # full-body keyframes
                    assert cur_guide.guide_type in {'keyframes', 'global_joint_pos', 'global_root_orient', 'local_joint_pos'}
        ...
        if len(mask_type_list) == 0 and self.model.denoiser.use_motion_mask:
        ...
        if len(mask_type_list) > 0:
            ...
            res = {
                'rm_text_flag': torch.ones(gt_motion.shape[0], device=gt_motion.device),
    ...
    def convert_to_264(self, x, motion_rep='orig'):
        bs, _, s, n = x.shape
        if motion_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
            return x
        elif motion_rep == 'orig':
    ...
    def preprocess(self, x, motion_rep='orig'):
        # [batch, 1, seq_len, nfeat]
        if motion_rep == 'orig':
            out = x * self.std + self.mean # x: [batch, 1, seq_len, nfeat]
        elif motion_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
    ...
    def local_to_global_264(self, pred=None, target=None, root_only=False, calc_pos_from_rot=False):
        '''
        Convert from local pose rep to global with the 264 dim rep.

        Always returns positions from joint pos prediction in the rep, NOT from FK of joint rotations.
        '''
        out_list = []
        ...
        for x, motion_rep in zip(in_list, rep_list):
            if x is not None:
                ...
                if motion_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
                    ...
                    pred_r_rot_quat = torch.cat([torch.cos(r_rot_ang / 2), torch.zeros_like(rot_cos_sin[..., [0]]), torch.sin(r_rot_ang / 2), torch.zeros_like(rot_cos_sin[..., [0]])], dim=-1)
                # [B, 1, nframes, 22, 3]
                if root_only and motion_rep in {'global_root_local_joints', 'global_root_local_joints_root_rot'}:
                    ...
                    if calc_pos_from_rot:
                        assert motion_rep in {'global_root_local_joints_root_rot'}
                        global_pred_pose, global_pred_rot_quat = recover_from_ric_with_joint_rot(x, self.model.neutral_joints.to(x.device), 
                                                                                                self.model.joint_parents.to(x.device),
                                                                                                 root_dim=5, r_rot_quat=pred_r_rot_quat, r_pos=pred_r_pos, 
                                                                                                 return_r_rot=True)  
                    else:
                        global_pred_pose, global_pred_rot_quat = recover_from_ric_264(x, NUM_JOINTS, r_rot_quat=pred_r_rot_quat, r_pos=pred_r_pos)        
