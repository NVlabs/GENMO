...
class Guide:
        ...
        with torch.enable_grad():
            ...
            for cur_name, cur_guide in self.guides.items():
                if cur_guide.guide_type == guide_funcs.FootSkate.guide_type:
                    ...
                    cur_guide.guide_type == guide_funcs.LocalJointPosFromRot.guide_type
                ):
                    assert self.model.motion_rep in {
                        "global_root_local_joints_root_rot"
                    }
                    r_pos = global_pred_pose[:, :, :, 0]
                    local_pred_pose_from_rot = recover_local_joint_pos_with_joint_rot(
                        pred_c,
                        self.model.neutral_joints.to(pred_c.device),
                        self.model.joint_parents.to(pred_c.device),
                        r_pos,
                        root_dim=5,
                    )[:, :, 1:]  # no root
                    local_pred_pose_from_rot = local_pred_pose_from_rot[
                        :, None
                    ].reshape(local_target_pose.shape)
                    loss_out = cur_guide.loss(
                        local_pred_pose_from_rot, local_target_pose
                    )
                else:
        ...
        for cur_name, cur_guide in self.guides.items():
            if cur_guide.guide_type == guide_funcs.FootSkate.guide_type:
                ...
                if pred_full_rep is not None:
                    ...
                    global_pred_pose_from_rot, _ = self.local_to_global_264(
                        pred=pred_c, calc_pos_from_rot=True
                    )
                    eval_out = cur_guide.eval(
                        local_pred_pose_from_rot,
                        local_target_pose,
                        global_pred_pose_from_rot,
                        global_target_pose,
                    )
            else:
                raise NotImplementedError(
    ...
    def local_to_global_264(
        self, pred=None, target=None, root_only=False, calc_pos_from_rot=False
    ):
        """
            ...
            if x is not None:
                ...
                if root_only and motion_rep in {
                    ...
                    if calc_pos_from_rot:
                        ...
                        global_pred_pose, global_pred_rot_quat = (
                            recover_from_ric_with_joint_rot(
                                x,
                                self.model.neutral_joints.to(x.device),
                        ...
                        global_pred_pose, global_pred_rot_quat = recover_from_ric_264(
                            x, NUM_JOINTS, r_rot_quat=pred_r_rot_quat, r_pos=pred_r_pos
                        )
