...
class FootSkate(GuideFunc):
    ...
    def __init__(self, weight, exp_weighting=False, exp_alpha=0.01):
        ...
        self.foot_joint_idx = [SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in foot_joint_names]
...
class GlobalJointPos(KeyframeBase):
    ...
    def __init__(self, weight, keyframe_inds='all', keyframe_ratio=True, joint_names=None, use_root_2d=False):
        ...
        self.global_joint_inds = [SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in self.joint_names]

    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        super().init_keyframe_inds(batch_size, pred_nframes, tgt_data_len, device)
        if self.keyframe_inds != 'all':
            nkeys = self.keyframe_inds.size(1)
            self.keyframe_inds = self.keyframe_inds[:,None,:,None, None].expand(batch_size, 1, nkeys, 22, 3) # expand to same size as poses 
...
class LocalJointPos(KeyframeBase):
    ...
    def __init__(self, weight, keyframe_inds='all', keyframe_ratio=True, joint_names=None):
        super().__init__(weight, keyframe_inds, keyframe_ratio)
        if joint_names == 'all':
            self.joint_names = set(SMPL_BONE_ORDER_NAMES[1:22]) # no hands and leave out root
        else:
            self.joint_names = set(joint_names)
        assert 'Pelvis' not in self.joint_names, 'Cannot apply a local loss to the root (it is at the origin)'

        self.local_joint_inds = [SMPL_BONE_ORDER_NAMES.index(joint_name)-1 for joint_name in self.joint_names] # the root is not considered here so subtract 1
    ...
    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        ...
        if self.keyframe_inds != 'all':
            nkeys = self.keyframe_inds.size(1)
            self.keyframe_inds = self.keyframe_inds[:,None,:,None].expand(batch_size, 1, nkeys, 63) # expand to same size as local poses

    def compute_keyframe_diff(self, local_pred_pose, local_target_pose):
        # pull out only keyframe poses
        if self.keyframe_inds != 'all':
            # compare all global joint positions at each desired keyframe
            local_pred_keyframes = torch.gather(local_pred_pose, 2, self.keyframe_inds)  # [batch, 1, nkeys, 63]
            local_tgt_keyframes = torch.gather(local_target_pose, 2, self.keyframe_inds) # [batch, 1, nkeys, 63]
        else:
    ...
    def eval(self, local_pred_pose=None, local_target_pose=None, 
        ...
        if global_pred_pose is not None and global_target_pose is not None:
            ...
            if global_keyframe_inds != 'all':
                ...
                global_keyframe_inds = global_keyframe_inds[:,None,:,None, None].expand(global_pred_pose.size(0), 1, nkeys, 22, 3) # expand to same size as global poses
            ...
            # diff calculation
            global_joint_inds = [SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in self.joint_names]
