...
class FootSkate(GuideFunc):
    ...
    def loss(self, global_pred_pose, foot_contacts, t):
        """
        - global_pred_pose : global joint positions [batch, 1, nframes, 22, 3]
...
class KeyframeBase(GuideFunc):
    ...
    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        ...
        if self.keyframe_inds != "all":
            self.keyframe_inds = (
                torch.tensor([self.keyframe_inds]).expand((batch_size, -1)).to(device)
            )
            if self.keyframe_ratio:
                data_len = (
                    torch.ones((batch_size, 1)) * pred_nframes
                    if tgt_data_len is None
                    else tgt_data_len.unsqueeze(1)
                )
                self.keyframe_inds = self.keyframe_inds * (data_len.to(device) - 1)
            self.keyframe_inds = self.keyframe_inds.round().to(
                torch.long
            )  # [B, num_keys]
...
class GlobalJointPos(KeyframeBase):
    ...
    def __init__(
        ...
        if joint_names == "all":
            self.joint_names = set(SMPL_BONE_ORDER_NAMES[:22])  # no hands
        else:
            self.joint_names = set(joint_names)

        self.global_joint_inds = [
            SMPL_BONE_ORDER_NAMES.index(joint_name) for joint_name in self.joint_names
        ]
    ...
    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        ...
        if self.keyframe_inds != "all":
            ...
            self.keyframe_inds = self.keyframe_inds[:, None, :, None, None].expand(
                batch_size, 1, nkeys, 22, 3
    ...
    def compute_keyframe_diff(self, global_pred_pose, global_target_pose):
        ...
        if self.keyframe_inds != "all":
            global_pred_keyframes = torch.gather(
                global_pred_pose, 2, self.keyframe_inds
            )
            global_tgt_keyframes = torch.gather(
                global_target_pose, 2, self.keyframe_inds
            )
        else:
        ...
        if self.use_root_2d and "Pelvis" in self.joint_names:
            # zero out y (up) dim
        ...
        # diff calculation
        global_pose_diff = (
            global_pred_keyframes[:, :, :, self.global_joint_inds]
            - global_tgt_keyframes[:, :, :, self.global_joint_inds]
        )
    ...
    def loss(self, global_pred_pose, global_target_pose):
        """
        - global_pred_pose : global joint positions [batch, 1, nframes, 22, 3]
        - global_target_pose : global joint positions [batch, 1, nframes, 22, 3]
        """
        assert self.keyframe_inds is not None, (
            "Must call init_keyframe_inds before computing loss!"
        )
        global_pose_diff = self.compute_keyframe_diff(
            global_pred_pose, global_target_pose
        )
        global_loss = torch.mean(
            global_pose_diff**2, dim=-1
        ...
        if self.keyframe_inds == "all":
            ...
            global_loss = torch.cat(
                [
                    ...
                    for bi in range(global_pred_pose.size(0))
                ]
            )
    ...
    def eval(self, global_pred_pose, global_target_pose):
        assert self.keyframe_inds is not None, (
            "Must call init_keyframe_inds before computing eval!"
        )
        global_pose_diff = self.compute_keyframe_diff(
            global_pred_pose, global_target_pose
        )
...
class GlobalRootOrient(KeyframeBase):
    ...
    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        super().init_keyframe_inds(batch_size, pred_nframes, tgt_data_len, device)
        if self.keyframe_inds != "all":
            self.keyframe_inds = self.keyframe_inds[:, None, :, None].expand(
                batch_size, 1, self.keyframe_inds.size(1), 4
            )  # expand to same size as rot quats

    def compute_root_orient_diff(self, global_pred_rot_quat, global_target_rot_quat):
        if self.keyframe_inds != "all":
            pred_rot_keyframe = torch.gather(
                global_pred_rot_quat, 2, self.keyframe_inds
            )
            tgt_rot_keyframe = torch.gather(
                global_target_rot_quat, 2, self.keyframe_inds
            )
        else:
    ...
    def loss(self, global_pred_rot_quat, global_target_rot_quat):
        ...
        global_rot_diff = self.compute_root_orient_diff(
            global_pred_rot_quat, global_target_rot_quat
        )
        global_rot_loss = torch.mean(
            global_rot_diff**2, dim=-1, keepdim=True
        )  # [B, 1, num_keys, 1]

        if self.keyframe_inds == "all":
            # extract only the valid frames based on GT sequence length
            global_rot_loss = torch.cat(
                [
                    global_rot_loss[bi, :, : self.tgt_data_len[bi]].flatten()
                    for bi in range(global_pred_rot_quat.size(0))
                ]
            )

        return {"global_root_orient": global_rot_loss}, {
            "global_root_orient": self.weight
        }
...
class LocalJointPos(KeyframeBase):
    ...
    def __init__(
        self, weight, keyframe_inds="all", keyframe_ratio=True, joint_names=None
    ):
        super().__init__(weight, keyframe_inds, keyframe_ratio)
        if joint_names == "all":
            ...
            self.joint_names = set(joint_names)
        assert "Pelvis" not in self.joint_names, (
            "Cannot apply a local loss to the root (it is at the origin)"
        )

        self.local_joint_inds = [
            SMPL_BONE_ORDER_NAMES.index(joint_name) - 1
            for joint_name in self.joint_names
        ]  # the root is not considered here so subtract 1
        self.local_joint_inds = np.concatenate(
            [np.arange(idx * 3, idx * 3 + 3) for idx in self.local_joint_inds]
        )

    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        super().init_keyframe_inds(batch_size, pred_nframes, tgt_data_len, device)
        self.og_keyframe_inds = (
            self.keyframe_inds
        )  # save for computing local_in_global eval later
        if self.keyframe_inds != "all":
            nkeys = self.keyframe_inds.size(1)
            self.keyframe_inds = self.keyframe_inds[:, None, :, None].expand(
                batch_size, 1, nkeys, 63
            )  # expand to same size as local poses

    def compute_keyframe_diff(self, local_pred_pose, local_target_pose):
        # pull out only keyframe poses
        if self.keyframe_inds != "all":
            # compare all global joint positions at each desired keyframe
            local_pred_keyframes = torch.gather(
                local_pred_pose, 2, self.keyframe_inds
            )  # [batch, 1, nkeys, 63]
            local_tgt_keyframes = torch.gather(
                local_target_pose, 2, self.keyframe_inds
            )  # [batch, 1, nkeys, 63]
        else:
        ...
        local_pose_diff = (
            local_pred_keyframes[:, :, :, self.local_joint_inds]
            - local_tgt_keyframes[:, :, :, self.local_joint_inds]
    ...
    def loss(self, local_pred_pose, local_target_pose):
        """
        - local_pred_pose : local joint positions excluding the root [batch, 1, nframes, 63]
        - local_target_pose : local joint positions excluding the root [batch, 1, nframes, 63]
        """
        assert self.keyframe_inds is not None, (
            "Must call init_keyframe_inds before computing loss!"
        )
        ...
        if self.keyframe_inds == "all":
            ...
            local_loss = torch.cat(
                [
                    ...
                    for bi in range(local_pred_pose.size(0))
                ]
            )
    ...
    def eval(
        ...
        if local_pred_pose is not None and local_target_pose is not None:
            ...
            local_pose_diff = self.compute_keyframe_diff(
                local_pred_pose, local_target_pose
            )
            njoints = 21 if self.joint_names == "all" else len(self.joint_names)
            local_pose_diff = local_pose_diff.reshape(
                (local_pred_pose.size(0), 1, -1, njoints, 3)
        ...
        if global_pred_pose is not None and global_target_pose is not None:
            # global joint positions [batch, 1, nframes, 22, 3]
            global_keyframe_inds = self.og_keyframe_inds
            if global_keyframe_inds != "all":
                nkeys = global_keyframe_inds.size(1)
                global_keyframe_inds = global_keyframe_inds[
                    :, None, :, None, None
                ].expand(
                    global_pred_pose.size(0), 1, nkeys, 22, 3
                )  # expand to same size as global poses

            # compare all global joint positions at each desired keyframe
            if global_keyframe_inds != "all":
                global_pred_keyframes = torch.gather(
                    global_pred_pose, 2, global_keyframe_inds
                )
                global_tgt_keyframes = torch.gather(
                    global_target_pose, 2, global_keyframe_inds
                )
            else:
                # will compute error of full sequence and then chop off what we don't need
                global_pred_keyframes = global_pred_pose
...
class Keyframes(KeyframeBase):
    ...
    def __init__(
        ...
        if self.exp_upweight:
            raise NotImplementedError(
                "Exponential up-weighting not currently supported!"
            )
        ...
        global_pos_weight = (
            self.weight[0]
            * (len(self.global_joint_names) / (len(self.global_joint_names) + 1))
            if self.use_root_orient
            else self.weight[0]
        )
        self.global_pos_guide = GlobalJointPos(
            global_pos_weight,
            self.keyframe_inds_in,
            self.keyframe_ratio,
            self.global_joint_names,
            self.use_root_2d,
        )
        if self.use_root_orient:
            global_rrot_weight = self.weight[0] / (len(self.global_joint_names) + 1)
            self.global_rrot_guide = GlobalRootOrient(
                global_rrot_weight, self.keyframe_inds_in, self.keyframe_ratio
            )
        else:
            self.global_rrot_guide = None
        if self.local_joint_names is not None:
            self.local_pos_guide = LocalJointPos(
                self.weight[1],
                self.keyframe_inds_in,
                self.keyframe_ratio,
    ...
    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        super().init_keyframe_inds(batch_size, pred_nframes, tgt_data_len, device)
        self.global_pos_guide.init_keyframe_inds(
            batch_size, pred_nframes, tgt_data_len, device
        )
        if self.global_rrot_guide is not None:
            self.global_rrot_guide.init_keyframe_inds(
                batch_size, pred_nframes, tgt_data_len, device
            )
        if self.local_pos_guide is not None:
            self.local_pos_guide.init_keyframe_inds(
                batch_size, pred_nframes, tgt_data_len, device
            )
    ...
    def loss(
        ...
        losses, weights = self.global_pos_guide.loss(
            global_pred_pose, global_target_pose
        )
        if self.global_rrot_guide is not None:
            global_root_rot_loss, global_root_rot_weight = self.global_rrot_guide.loss(
                global_pred_rot_quat, global_target_rot_quat
            )
            losses.update(global_root_rot_loss)
            weights.update(global_root_rot_weight)
        if self.local_pos_guide is not None:
            local_pos_loss, local_pos_weight = self.local_pos_guide.loss(
                local_pred_pose, local_target_pose
            )
    ...
    def eval(
        ...
        # root rotation
        if (
            self.global_rrot_guide is not None
            and global_pred_rot_quat is not None
            and global_target_rot_quat is not None
        ):
            eval_dict.update(
                self.global_rrot_guide.eval(
                    global_pred_rot_quat, global_target_rot_quat
                )
            )
        # local joint pos
...
GUIDE_CLS = [FootSkate, Keyframes, GlobalJointPos, GlobalRootOrient, LocalJointPos]
