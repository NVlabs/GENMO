...
class Guide():
    def __init__(self, cfg, model, diff_steps, batch_size, pred_nframes, tgt_data_len=None,
                    pred_motion_rep='full263', tgt_motion_rep='full263'):
        '''
        Diffusion guidance.
        '''
        self.cfg = cfg
        ...
        for cur_guide in self.guides.values():
            if cur_guide.guide_type in {'global_joint_pos', 'global_root_orient', 'local_joint_pos', 'keyframes'}:
        ...
        for cur_guide, cur_mask in zip(self.guides.values(), self.mask):
            ...
            if cur_mask is not None and cur_mask != 'no_mask':
                ...
                if cur_mask.split('_')[0] == 'unknownt':
                    assert cur_guide.unknown_time, 'mask and guidance are inconsistent wrt known or unknown time'
