...
class FootSkate(GuideFunc):
    ...
    def __init__(self, weight, exp_weighting=False, exp_alpha=0.01):
        ...
        self.foot_joint_idx = [
            BONES_BONE_ORDER_NAMES.index(joint_name) for joint_name in foot_joint_names
        ]
...
class GlobalJointPos(KeyframeBase):
    ...
    def __init__(
        ...
        self.global_joint_inds = [
            BONES_BONE_ORDER_NAMES.index(joint_name) for joint_name in self.joint_names
        ]
    ...
    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        ...
        if self.keyframe_inds != "all":
            ...
            self.keyframe_inds = self.keyframe_inds[:, None, :, None, None].expand(
                batch_size, 1, nkeys, NUM_JOINTS, 3
...
class LocalJointPos(KeyframeBase):
    ...
    def __init__(
        ...
        if joint_names == "all":
            ...
            self.joint_names = set(joint_names)
        assert ROOT_NAME not in self.joint_names, (
            "Cannot apply a local loss to the root (it is at the origin)"
        )

        self.local_joint_inds = [
            BONES_BONE_ORDER_NAMES.index(joint_name) - 1
    ...
    def init_keyframe_inds(self, batch_size, pred_nframes, tgt_data_len, device):
        ...
        if self.keyframe_inds != "all":
            nkeys = self.keyframe_inds.size(1)
            self.keyframe_inds = self.keyframe_inds[:, None, :, None].expand(
                batch_size, 1, nkeys, NUM_BODY_JOINTS * 3
    ...
    def compute_keyframe_diff(self, local_pred_pose, local_target_pose):
        ...
        if self.keyframe_inds != "all":
            ...
            )  # [batch, 1, nkeys, NUM_BODY_JOINTS*3]
    ...
    def eval(
        ...
        if local_pred_pose is not None and local_target_pose is not None:
            ...
            njoints = (
                NUM_BODY_JOINTS if self.joint_names == "all" else len(self.joint_names)
            )
        ...
        if global_pred_pose is not None and global_target_pose is not None:
            ...
            if global_keyframe_inds != "all":
                ...
                global_keyframe_inds = global_keyframe_inds[
                    ...
                    global_pred_pose.size(0), 1, nkeys, NUM_JOINTS, 3
            ...
            # diff calculation
            global_joint_inds = [
                BONES_BONE_ORDER_NAMES.index(joint_name)
                for joint_name in self.joint_names
            ]
            local_in_glob_diff = (
