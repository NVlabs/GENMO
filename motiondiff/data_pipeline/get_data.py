...
def get_collate_fn(name, hml_mode="train"):
    if hml_mode == "gt":
        from motiondiff.data_pipeline.humanml.data.dataset import (
...
def get_dataset(
    ...
    if name in ["humanml", "kit"]:
        dataset = DATA(
            split=split, num_frames=num_frames, mode=hml_mode, debug=debug, rng=rng
        )
    elif name == "dpo":
        dataset = DATA(
            split=split,
            num_frames=num_frames,
            win_key=data_cfg.win_key,
            lose_key=data_cfg.lose_key,
        )
...
def get_dataset_loader(
    ...
    loader = DataLoader(
        ...
        shuffle=(split == "train") or shuffle,
