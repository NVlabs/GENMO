...
class Human36M(KP2DDataset):
        ...
        for subject in smplx_params.keys():
            for action_idx in smplx_params[subject].keys():
                for subaction_idx in smplx_params[subject][action_idx].keys():
                    datalist.append(
                        {
                            "smplx_param": smplx_params[subject][action_idx][
                                subaction_idx
                            ],
                        }
                    )
    ...
    def __getitem__(self, idx):
        ...
        motion_3d = interp_scipy_ndarray(
            smplx_params["joint_world"], scale=0.4, dim=0
        )  # from 50 fps to 20 fps
