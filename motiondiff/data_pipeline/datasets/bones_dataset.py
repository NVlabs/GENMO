...
class BonesDatasetMix(data.Dataset):
    ...
    def __init__(self, mix_paths, sample_ratio, **bones_kwargs):
        ...
        for mix_base_path in mix_paths:
            ...
            mix_config['use_short_desc'] = True # only use the text that was the input to the model
    ...
    def __getitem__(self, idx):
        ...
        dataset_idx = np.argmax(ratio < self.sample_thresh) # returns first index with 1.0
        samp_idx = np.random.randint(0, self.dataset_lens[dataset_idx]) # randomly sample from corresponding dataset
