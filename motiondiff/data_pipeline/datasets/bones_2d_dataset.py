...
from motiondiff.utils.geom import perspective_projection, batch_triangulate, batch_triangulate_torch, batch_triangulate_torch_single, lookat_correct, spherical_to_cartesian
...
class Bones2DDataset(data.Dataset):
    ...
    def generate_cam_and_2d_motion(self, motion):
        cam_dict = []
        joints_pos, joints_rot = recover_from_ric_with_joint_rot(torch.tensor(motion), self.neutral_joints, self.joint_parents, return_joint_rot=True)
        joints_pos = torch.stack([-joints_pos[..., 0], joints_pos[..., 2], joints_pos[..., 1]], dim=-1)
    ...
    def extract_kpt3d(self, joints_pos):
        ...
        if self.traj_center_xy_range > 0:
            ...
            kpt3d[..., :2] += self.traj_center_xy_range * np.random.rand(1) * np.array([np.cos(theta), np.sin(theta)])
