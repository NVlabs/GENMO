...
from motiondiff.models.common.utils.human_models import smpl_x, smpl, smplx_joint_parent_mapping
...
skip_indices = {
    'v1': [230, 677, 819, 885, 1671, 2193, 2745, 2985, 3004, 3092, 3267, 3750, 4095, 4286, 4381, 4612, 4694, 4974, 5072, 5308, 6171, 6210, 6392, 6516, 6570, 7289, 7314, 7685, 7795, 7900, 8054, 9192, 9232, 10386, 10601, 10812, 11247, 11483, 11677, 11767],
...
class MotionX():
    def __init__(self, data_dir='dataset/motion-x', split='train', num_keypoints=25, num_frames=196, normalize_type='bbox_frame', bbox_scale=1.4, kp_conf_threshold=0.1, skip_ind_version=None, exclude_pelvis=False, normalize_stats_dir=None, **kwargs):
        ...
        for seq in self.sequences:
            ...
            motion_names = sorted([osp.splitext(osp.basename(f))[0] for f in motion_files])
            for motion_name in motion_names:
                self.motion_list.append((seq, motion_name))
        if skip_ind_version is not None:
            self.valid_indices = np.array([i for i in range(len(self.motion_list)) if i not in skip_indices[skip_ind_version]])
        else:
        ...
        self.joint_names = ['Nose', 'L_Eye', 'R_Eye', 'L_Ear', 'R_Ear', 'L_Shoulder', 'R_Shoulder', 'L_Elbow', 'R_Elbow', 'L_Wrist',
                            'R_Wrist', 'L_Hip', 'R_Hip', 'L_Knee', 'R_Knee', 'L_Ankle', 'R_Ankle', 'Head', 'Neck', 'Pelvis', 'L_Big_toe', 'R_Big_toe', 'L_Small_toe', 'R_Small_toe', 'L_Heel', 'R_Heel']
        self.joint_parents = [self.joint_names.index(joint_parent_mapping[j]) if j in joint_parent_mapping else -1 for j in self.joint_names]
        self.smplx_bbox_joints = [1, 2, 3, 4, 5, 6, 8, 9, 10, 11, 12, 13]
        self.smplx_valid_joints = [i for i, j in enumerate(smpl_x.joints_name) if j in self.joint_names]
        if exclude_pelvis:
    ...
    def __getitem__(self, i):
        ...
        if self.normalize_type in {'bbox_frame', 'bbox_seq'}:
            ...
            for t in range(motion_2d.shape[0]):
                front_view = torch.tensor(body_kpts_smplx[t, self.smplx_bbox_joints, :2])  # only use first 14 joints for bbox size
...
if __name__ == '__main__':
    ...
    dataset = MotionX(skip_ind_version='v1', normalize_stats_dir='assets/mv2d/stats/gen2d_mv_test_mask_fc_st_2d_fix_w3d_25kp')
    # data = dataset[326]
    # for i in range(10):
    #     batch = dataset[i]
    #     print(f'data {i}')
