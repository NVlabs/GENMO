...
class KP2DDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        joint_parents_file="assets/mv2d/smplx_joint_parents.p",
    ...
    def tilt_kpt3d(self, kpt3d):
        tilt_angles = (
            torch.tensor(np.random.randn(1, 2) * self.tilt_std)
            .expand(kpt3d.shape[0], 2)
            .float()
        )
        # tilt_angles[:, 0] = 0
        # tilt_angles[:, 1] = 45
        x_rot = angle_axis_to_rotation_matrix(
            torch.tensor([1.0, 0.0, 0.0]) * torch.deg2rad(tilt_angles[:, [0]])
        )
        y_rot = angle_axis_to_rotation_matrix(
            torch.tensor([0.0, 1.0, 0.0]) * torch.deg2rad(tilt_angles[:, [1]])
        )
    ...
    def pad_motion(self, motion, kpt3d):
        if motion.shape[0] >= self.num_frames:
            ...
            kpt3d = np.concatenate(
                [
                    kpt3d,
                    np.zeros(
                        (
                            self.num_frames - kpt3d.shape[0],
                            kpt3d.shape[1],
                            kpt3d.shape[2],
                        )
                    ),
