...
class KP2DDatasetV2(torch.utils.data.Dataset):
    ...
    def generate_eyes(self, target):
        ...
        if "elevation_std" in self.cam_aug_cfg:
            ...
            elevations = (
                rng_val * self.cam_aug_cfg["elevation_std"]
                + self.cam_aug_cfg["elevation_mean"]
            )
        ...
        if "radius_min" in self.cam_aug_cfg:
            ...
            radius = (
                rng_val
                * (self.cam_aug_cfg["radius_max"] - self.cam_aug_cfg["radius_min"])
                + self.cam_aug_cfg["radius_min"]
            )
    ...
    def project_keypoints(self, kpt3d, cam_dict):
        ...
        local_kpt2d_new = (
            cam_dict["P"].transpose(0, 1) @ kpt3d_pad[:, None].transpose(-1, -2)
        ).transpose(-1, -2)
    ...
    def normalize_ours(self, local_kpt2d, center=None, normalize_size=None):
        if self.normalize_type == "image_size":
            ...
            if center is not None:
                ...
                front_view = local_kpt2d[
                    :, 0, self.smplx_bbox_joints
                ]  # only use limb joints for bbox size, and only use front views
    ...
    def get_input(self, target):
        ...
        if self.use_our_normalization:
            ...
            gt_kp2d, bbox = self.keypoints_normalizer(
                gt_kp2d.reshape(-1, *gt_kp2d.shape[2:]),
                target["res"],
                self.cam_intrinsics,
                224,
                224,
                use_multi_view_bbox=True,
                num_views=self.num_views,
                do_augment=False,
            )
            in_kp2d, bbox = self.keypoints_normalizer(
                in_kp2d.reshape(-1, *in_kp2d.shape[2:]),
                target["res"],
                self.cam_intrinsics,
                224,
                224,
            ...
            for key, val in target.items():
                if key in {
                    ...
                    target[key] = np.concatenate(
                        [
                            val,
                            np.zeros((self.num_frames - val.shape[0],) + val.shape[1:]),
                        ],
                        axis=0,
                    )
        return
