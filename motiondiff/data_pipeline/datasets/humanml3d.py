...
class HumanML3DV1(KP2DDataset):
    def __init__(
        ...
        **kwargs,
        ...
        opt.motion_dir = os.path.join(
            opt.data_root, f"new_joint_vecs_v{self.feat_version}"
        )
        ...
        self.w_vectorizer = WordVectorizer(
            os.path.join(abs_base_path, "glove"), "our_vab"
        )
        ...
        assert len(self.t2m_dataset) > 1, (
            "You loaded an empty dataset, it is probably because your data dir has only texts and no motions. To train and evaluate MDM you should get the FULL data as described in the README file."
        )
