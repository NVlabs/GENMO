...
LINKS = [[0,1], [1,2], [2,3], # index finger
         [0,4], [4,5], [5,6],  # middle finger
         [0,7], [7,8], [8,9],  # pinky finger
         [0,10], [10,11], [11,12],  # ring finger
         [0,13], [13,14], [14,15],  # thumb finger
        [15,16],[3,17],[6,18],[9,20],[12,19]] # tips
...
def viz_hand(joints, ax, color='red'):
    ...
    for link in LINKS:
        first = link[0]
        second = link[1]
        ax.plot([joints[first, 0], joints[second, 0]], 
                [joints[first, 1], joints[second, 1]],
                [joints[first, 2], joints[second, 2]], c='b')
    return ax
...
def process_split(dataset_dir, split):
    ...
    lhand_feature = torch.cat([features['lhand']['global_orient'], features['lhand']['transl'], features['lhand']['fullpose']], dim=1)
    rhand_feature = torch.cat([features['rhand']['global_orient'], features['rhand']['transl'], features['rhand']['fullpose']], dim=1)
    ...
    for epi_idx in range(len(episodes)):
        ...
        rhand_m = smplx.create(model_path=body_model_path,
                                model_type='mano',
                                flat_hand_mean=True,
                                # gender=gender,
        ...
        lhand_m = smplx.create(model_path=body_model_path,
                                model_type='mano',
                                flat_hand_mean=True,
                                # gender=gender,
                                # num_pca_comps=n_comps,
        ...
        lhand_mano = lhand_m(
                global_orient = features['lhand']['global_orient'][total_frames:total_frames+episode].float(),
                hand_pose = features['lhand']['fullpose'][total_frames:total_frames+episode].float(),
                transl = features['lhand']['transl'][total_frames:total_frames+episode].float()
            )

        rhand_mano = rhand_m(
                global_orient = features['rhand']['global_orient'][total_frames:total_frames+episode].float(),
                hand_pose = features['rhand']['fullpose'][total_frames:total_frames+episode].float(),
                transl = features['rhand']['transl'][total_frames:total_frames+episode].float()
            )
        ...
        joint_features = np.concatenate([lhand_all_joints, rhand_all_joints], axis=1).reshape(episode, -1)
