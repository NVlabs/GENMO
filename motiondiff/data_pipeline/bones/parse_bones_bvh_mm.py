...
def parse_bvh(bvh_path):
    ...
    joint_rot_mats_path = os.path.join(args.out_dir, bvh_path.replace('BVH', 'joint_rot_mats').replace('bvh', 'npy'))
    posed_joints_path = os.path.join(args.out_dir, bvh_path.replace('BVH', 'posed_joints').replace('bvh', 'npy'))
...
if __name__ == '__main__':
    ...
    todo_job_list = [path for path in job_list_node if not os.path.exists(os.path.join(args.out_dir, path.replace('BVH', 'posed_joints').replace('bvh', 'npy')))]
    ...
    for batch_idx in tqdm(range(num_batch)):
        bvh_path_batch = todo_job_list[args.num_process * batch_idx: args.num_process * (batch_idx + 1)]
