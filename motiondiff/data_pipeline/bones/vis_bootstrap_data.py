...
parser.add_argument('-c', '--cfg', default='mask_bones_twostage_root_rot_v4_body_key_fine_tune_mix')
...
for di in range(num_datasets):
    ...
    print(cfg.train_dataset['sample_ratio'])
    dataloader = get_dataset_loader(name=cfg.train_dataset.name, batch_size=args.batch_size, split='train', hml_mode='train', drop_last=False, data_cfg=cfg.train_dataset,
    ...
    for batch in tqdm(dataloader):
        ...
        global_motion = model.convert_motion_rep(motion).permute((0,2,3,1)) # returns root motion in metric global since normalize_global_pos = False (the rest is normalized)
    ...
    axes[di].hist2d(end_pos_arr[:, 0], end_pos_arr[:, 1], bins=100, range=[[-max_val,max_val],[-max_val,max_val]], cmap='plasma',
