...
'''Skeleton parameters'''
from motiondiff.data_pipeline.bones.skeleton_params import BONESPose, SMPLPose, bones_beta, bones_parents, smpl_ear_indices, smpl_eye_indices, \
    bones_joints_rest_jaw, bones_joints_rest_ear, bones_joints_rest_eye
...
bones_joint_idx = torch.LongTensor([
                        BONESPose.Neck, 
                        BONESPose.LeftArm, BONESPose.RightArm,  
                        BONESPose.LeftForeArm, BONESPose.RightForeArm, BONESPose.LeftHand, BONESPose.RightHand, BONESPose.LeftHandEnd, BONESPose.RightHandEnd,
                        BONESPose.LeftLeg, BONESPose.RightLeg, BONESPose.LeftFoot, BONESPose.RightFoot, BONESPose.LeftToeBase, BONESPose.RightToeBase,
                        ])
smpl_joint_idx = torch.LongTensor([
                    SMPLPose.Neck,
                    SMPLPose.LShoulder, SMPLPose.RShoulder,
                    SMPLPose.LElbow, SMPLPose.RElbow, SMPLPose.LWrist, SMPLPose.RWrist, SMPLPose.LHand, SMPLPose.RHand,
                    SMPLPose.LKnee, SMPLPose.RKnee, SMPLPose.LAnkle, SMPLPose.RAnkle, SMPLPose.LToe, SMPLPose.RToe, 
                    ])
...
def optimize_naive(bones_joints):
    def loss_naive(theta):
        ...
        bones_joints_rel = bones_joints[bones_joint_idx] - bones_joints[BONESPose.Pelvis]
    ...
    for i, bones_joints in enumerate(tqdm(bones_joints)):
        if i == 0:
            theta = torch.zeros((1, 24*3), dtype=torch.float32, device=torch.device('cuda:0'))
        if i % 30 == 0:
...
def optimize_vposer(bones_joints):
    ...
    def loss_vposer(latent):
        ...
        theta = torch.cat([latent[:, :3], pose_body, torch.zeros((1, 6), dtype=torch.float32).to('cuda')], dim=1)
        ...
        if args.add_joints is None:
            ...
            smpl_joints = torch.concat([smpl_motion.joints[0], smpl_motion.vertices[0, smpl_ear_indices]], dim=0)
        elif args.add_joints == 'eye':
            smpl_joints = torch.concat([smpl_motion.joints[0], smpl_motion.vertices[0, smpl_eye_indices]], dim=0)
    ...
    vp, ps = load_model('data/vposer_v2_05', model_code=VPoser,
    ...
    for i, bones_joints in enumerate(tqdm(bones_joints)):
        ...
        if args.debug_vis and i > 30: break
    smpl_thetas = torch.stack(smpl_thetas)
...
parser.add_argument('--dataset_dir', type=str, default='/lustre/fs5/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets')
...
if __name__ == '__main__':
    ...
    if args.debug:
            ...
            args.data_dir = os.path.join('/mnt/nvr_torontoai_humanmotionfm/datasets', args.data_dir)
            args.joints_dir = os.path.join('/mnt/nvr_torontoai_humanmotionfm/datasets', args.joints_dir)
    ...
    bones_rest_joints = torch.from_numpy(np.load(os.path.join(args.joints_dir, 'neutral_pose.npy')))
    if args.add_joints == 'ear':
        bones_rest_joints = torch.cat([bones_rest_joints, bones_joints_rest_ear.unsqueeze(0).cpu()], dim=1)
        bones_parents = torch.cat([bones_parents, torch.LongTensor([BONESPose.Head,  BONESPose.Head])], dim=-1)
        bones_joint_idx = torch.cat([torch.LongTensor([BONESPose.LEar, BONESPose.REar])])
        smpl_joint_idx = torch.cat([smpl_joint_idx, torch.LongTensor([SMPLPose.LEar, SMPLPose.REar])])
        ...
        bones_parents = torch.cat([bones_parents, torch.LongTensor([BONESPose.Head,  BONESPose.Head])], dim=-1)
        bones_joint_idx = torch.cat([bones_joint_idx, torch.LongTensor([BONESPose.LEye, BONESPose.REye])])
        smpl_joint_idx = torch.cat([smpl_joint_idx, torch.LongTensor([SMPLPose.LEye, SMPLPose.REye])])
        weights = torch.cat([weights, torch.FloatTensor([0.5, 0.5]).to('cuda')])
    ...
    if args.vis_only:
        job_list_todo = [path for path in job_list_node if os.path.exists(os.path.join(args.out_dir, path.replace('BVH', 'smpl').replace('bvh', 'npz')))]
    else:
        job_list_todo = [path for path in job_list_node if not os.path.exists(os.path.join(args.out_dir, path.replace('BVH', 'smpl').replace('bvh', 'npz')))]
    ...
    logging.basicConfig(level=logging.INFO, handlers=[
        logging.FileHandler(os.path.join(args.out_dir, f'node_{args.node_idx}_log.txt')),
        logging.StreamHandler()
    ])

    logging.info(f"Node {args.node_idx} started working with {len(job_list_todo)} files...")
    for idx, bvh_path in enumerate(job_list_todo):
        logging.info(f"Node {args.node_idx} started working with {idx} / {len(job_list_todo)} job: {bvh_path}")
        joint_path = os.path.join(args.joints_dir, bvh_path.replace('BVH', 'posed_joints').replace('bvh', 'npy'))
        rotmat_path = os.path.join(args.joints_dir, bvh_path.replace('BVH', 'joint_rot_mats').replace('bvh', 'npy'))
        bones_joints = torch.from_numpy(np.load(joint_path)[::args.downsample_rate]).to('cuda')
        ...
        if args.add_joints == 'ear':
            bones_rotmats = torch.cat([bones_rotmats, bones_rotmats[:, -2:]], dim=1) # any rotations for ears
        elif args.add_joints == 'eye':
        ...
        bones_joints, global_rot_mat = batch_rigid_transform(bones_rotmats, bones_rest_joints_seq, bones_parents)
        bones_joints += bones_joints_trans

        out_smpl_path = os.path.join(args.out_dir, bvh_path.replace('BVH', 'smpl').replace('bvh', 'npz'))
        video_path = os.path.join(args.out_dir, 'video', os.path.basename(bvh_path).replace('bvh', 'mp4'))
        
        if not args.vis_only:
            # smpl_thetas = optimize_naive(bones_joints)
            smpl_thetas, losses = optimize_vposer(bones_joints)
            if smpl_thetas is None: continue
        ...
        if not args.vis_only:
            ...
            np.savez(out_smpl_path, thetas=smpl_thetas.cpu().numpy(), root_trans=bones_joints[:, 0].cpu().numpy(), losses=losses)
        if args.vis:
            vis(visualizer, bones_joints, smpl_thetas, video_path, fps=120/args.downsample_rate)
