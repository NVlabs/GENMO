...
if __name__ == "__main__":
    ...
    bones_joints, global_rot_mat = batch_rigid_transform(
        rot_mats, joints, bones_parents
    )

    smpl = SMPL("data/smpl_data", create_transl=False, gender="neutral").to("cuda")
    global_orient = torch.zeros(
        (1, 3), dtype=torch.float32, device=torch.device("cuda:0")
    )
    body_pose = torch.zeros(
        (1, 23 * 3), dtype=torch.float32, device=torch.device("cuda:0")
    )
    ...
    def f(betas):
        ...
        bones_dist = [
            torch.abs(
                bones_joints[0, BONESPose.Neck, 1]
                - bones_joints[0, BONESPose.Pelvis, 1]
            ),
            torch.abs(
                bones_joints[0, BONESPose.LeftToeBase, 1]
                - bones_joints[0, BONESPose.Pelvis, 1]
            ),
            torch.abs(
                bones_joints[0, BONESPose.RightToeBase, 1]
                - bones_joints[0, BONESPose.Pelvis, 1]
            ),
            torch.abs(
                bones_joints[0, BONESPose.LeftLeg, 1]
                - bones_joints[0, BONESPose.Pelvis, 1]
            ),
            torch.abs(
                bones_joints[0, BONESPose.RightLeg, 1]
                - bones_joints[0, BONESPose.Pelvis, 1]
            ),
        ]
        smpl_dist = [
            torch.abs(
                smpl_joints[0, SMPLPose.Neck, 1] - smpl_joints[0, SMPLPose.Pelvis, 1]
            ),
            torch.abs(
                smpl_joints[0, SMPLPose.LToe, 1] - smpl_joints[0, SMPLPose.Pelvis, 1]
            ),
            torch.abs(
                smpl_joints[0, SMPLPose.RToe, 1] - smpl_joints[0, SMPLPose.Pelvis, 1]
            ),
            torch.abs(
                smpl_joints[0, SMPLPose.LKnee, 1] - smpl_joints[0, SMPLPose.Pelvis, 1]
            ),
            torch.abs(
                smpl_joints[0, SMPLPose.RKnee, 1] - smpl_joints[0, SMPLPose.Pelvis, 1]
            ),
        ]
        ...
        bones_dist = [
            torch.norm(
                bones_joints[0, BONESPose.RightForeArm]
                - bones_joints[0, BONESPose.RightHand]
            ),
            torch.norm(
                bones_joints[0, BONESPose.LeftForeArm]
                - bones_joints[0, BONESPose.LeftHand]
            ),
            torch.norm(
                bones_joints[0, BONESPose.RightLeg]
                - bones_joints[0, BONESPose.RightFoot]
            ),
            torch.norm(
                bones_joints[0, BONESPose.LeftLeg] - bones_joints[0, BONESPose.LeftFoot]
            ),
        ]
        smpl_dist = [
            torch.norm(
                smpl_joints[0, SMPLPose.RElbow] - smpl_joints[0, SMPLPose.RWrist]
            ),
            torch.norm(
                smpl_joints[0, SMPLPose.LElbow] - smpl_joints[0, SMPLPose.LWrist]
            ),
            torch.norm(
                smpl_joints[0, SMPLPose.RKnee] - smpl_joints[0, SMPLPose.RAnkle]
            ),
            torch.norm(
                smpl_joints[0, SMPLPose.LKnee] - smpl_joints[0, SMPLPose.LAnkle]
            ),
        ]
        for d1, d2 in zip(bones_dist, smpl_dist):
    ...
    jaw_joints_rest = (
        global_rot_mat[0, BONESPose.Head].T
        @ (jaw_joints.cpu() - bones_joints[0, BONESPose.Pelvis]).T
    ).T + joints[0, BONESPose.Pelvis]
    print("Jaw joints at rest:", jaw_joints_rest)
    bones_joints = torch.concat(
        [bones_joints, smpl_joints.cpu()[:1, SMPLPose.Head].unsqueeze(0)], dim=1
    )
    bones_parents = torch.concat(
        [bones_parents, torch.LongTensor([BONESPose.Head])], dim=-1
    )
    ...
    smpl_joints = torch.stack(
        [smpl_joints[:, :, 0], smpl_joints[:, :, 2], smpl_joints[:, :, 1]], dim=-1
    )
    smpl_joints[:, :, 2] += 1
    smpl_joints[:, :, 1] += 0.1
