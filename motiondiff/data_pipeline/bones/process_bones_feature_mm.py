...
def process_one_sequence(params):
    ...
    posed_joints = np.load(os.path.join(args.joints_dir, npy_path.replace('BVH', 'posed_joints')))
    joint_rot_mats = np.load(os.path.join(args.joints_dir, npy_path.replace('BVH', 'joint_rot_mats')))

    feature = extract_features(joint_rot_mats, posed_joints, use_root_local_rot=True, use_joint_local_height=args.use_joint_local_height, use_joint_local_vel=args.use_joint_local_vel)
...
if __name__ == '__main__':
    ...
    if not args.vis:
        ...
        job_list_node = job_list_all[args.node_idx : len(csv.move_bvh_path) : args.num_nodes]
        todo_job_list = [(idx, path) for (idx, path) in job_list_node if not os.path.exists(os.path.join(args.out_dir, 'new_joint_vecs', f'{idx:06d}.npy'))]
        ...
        for batch_idx in tqdm(range(num_batch)):
            job_batch = todo_job_list[args.num_process * batch_idx: args.num_process * (batch_idx + 1)]
            if len(job_batch) == 0: break
    ...
    if args.debug and args.vis:
        from motiondiff.data_pipeline.bones.motion_process import recover_from_ric, recover_from_ric_with_joint_rot
        ...
        sampled_paths = random.sample(os.listdir(os.path.join(args.out_dir, 'new_joint_vecs')), 10)
        ...
        for path in sampled_paths:
            ...
            positions, joints_rot = recover_from_ric_with_joint_rot(torch.tensor(feature).float(), 
                                                                    neutral_joints, joint_parents,
            ...
            vis = SMPLVisualizer(joint_parents=bones_parents, distance=7, elevation=2, verbose=False)
            ...
            vis.save_animation_as_video(out_path, init_args={'smpl_seq': smpl_seq, 'mode': 'gt', 'view_dir': 'Y-'}, window_size=(800, 800), frame_dir=frame_dir, fps=20, crf=15)
