...
def solve_regressor(verts, joints, L=200):
    ...
    for j in range(1, njoints):
        ...
        result = minimize(
            objective, x0, args=(A_sub, b), bounds=bounds, constraints=constraints
        )
...
if __name__ == "__main__":
    ...
    args.bones_joints_dir = os.path.join(
        "/mnt/nvr_torontoai_humanmotionfm/datasets", args.bones_joints_dir
    )
    ...
    bones_rest_joints = torch.from_numpy(
        np.load(os.path.join(args.bones_joints_dir, "neutral_pose.npy"))
    )
    ...
    if not os.path.exists(regressor_path) or args.recompute:
        ...
        for idx, smpl_path in enumerate(tqdm(smpl_path_train)):
            ...
            rotmat_path = os.path.join(
                args.bones_joints_dir,
                smpl_path.replace("smpl", "joint_rot_mats").replace("npz", "npy"),
            )
            bones_rotmats = torch.from_numpy(
                np.load(rotmat_path)[:: args.downsample_rate]
            )[: smpl_res["thetas"].shape[0]][valid_frames]
            if args.add_joints is not None:
            ...
            if args.regress_ver in ["concat_xyz", "split_xyz"]:
                ...
                bones_joints = torch.matmul(
                    torch.from_numpy(bones_joints_noroot).to("cuda"), rot_mats
                )
                smpl_verts_train += [smpl_verts.cpu().numpy()]
                bones_joints_train += [bones_joints.cpu().numpy()]

            if args.debug_vis and idx > 10:
                break
        ...
        print(
            f"Computing regressor matrix given {smpl_verts_train.shape[0]} paired frames ..."
        )
        start_time = time.time()
        if args.regress_ver == "concat_xyz":
            """ 6890*3 x 29*3"""
            smpl_verts_train = smpl_verts_train.reshape(smpl_verts_train.shape[0], -1)
            bones_joints_train = bones_joints_train.reshape(
                bones_joints_train.shape[0], -1
            )
        elif (
            args.regress_ver == "split_xyz"
            or args.regress_ver == "wo_glb_orient_split_xyz"
        ):
            """ 6890 x 29"""
            smpl_verts_train = np.transpose(smpl_verts_train, [0, 2, 1]).reshape(
                -1, smpl_verts_train.shape[1]
            )
            bones_joints_train = np.transpose(bones_joints_train, [0, 2, 1]).reshape(
                -1, bones_joints_train.shape[1]
            )
    ...
    for idx, smpl_path in enumerate(tqdm(smpl_path_test)):
        ...
        video_path = os.path.join(
            args.out_dir,
            f"video_regress_test_{args.regress_ver}",
            os.path.basename(smpl_path).replace("npz", "mp4"),
        )
        rotmat_path = os.path.join(
            args.bones_joints_dir,
            smpl_path.replace("smpl", "joint_rot_mats").replace("npz", "npy"),
        ...
        bones_joints_noroot_gt, _ = batch_rigid_transform(
            bones_rotmats, bones_rest_joints_seq, bones_parents
        )
        ...
        err_regressor = np.linalg.norm(
            (bones_joints_noroot_gt - bones_joints_retarget)
            .cpu()
            .numpy()[:, selected_joints],
            axis=2,
        ).mean()
        print(f"Retarget error regressor = {err_regressor}")
        err_ik = np.linalg.norm(
            (bones_joints_retarget - bones_joints_retarget_ik)
            .cpu()
            .numpy()[:, selected_joints],
            axis=2,
        ).mean()
        print(f"Retarget error IK = {err_ik}")
        err_regressor_ik = np.linalg.norm(
            (bones_joints_noroot_gt - bones_joints_retarget_ik)
            .cpu()
            .numpy()[:, selected_joints],
            axis=2,
        ).mean()
    ...
    print(
        f"{args.regress_ver} produces average errors regressor = {np.average(errors_regressor)}"
    )
    print(f"{args.regress_ver} produces average errors IK = {np.average(errors_ik)}")
    print(
        f"{args.regress_ver} produces average errors regressor + IK = {np.average(errors_regressor_ik)}"
    )
