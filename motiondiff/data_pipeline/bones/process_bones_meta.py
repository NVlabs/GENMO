...
parser.add_argument('--feature_dir', type=str, default='dataset/bones/bones_full353_v2.0')
...
if __name__ == '__main__':
    ...
    if not args.update:
        ...
        new_meta_dict = {
            'feature_path': [f"{i:06d}.npy" for i in range(len(initial_csv.move_bvh_path))],
        ...
        initial_meta = pd.read_csv(os.path.join(args.feature_dir, args.initial_meta_file))
        ...
        num_updates = 0
        f_change = open(os.path.join(args.feature_dir, 'meta_change_log_240416_v3.txt'), 'w')
        ...
        for i, bvh_path in enumerate(tqdm(new_csv.move_bvh_path)):
            if bvh_path in initial_bvh_path:
                ...
                for col in ['content_natural_desc_1', 'content_natural_desc_2', 'content_natural_desc_3', 'content_technical_description', 'content_short_description']:
                    ...
                    if (isinstance(initial_val, str) and 'talent' in initial_val) or \
                    (isinstance(initial_val, str) and ' lose' in initial_val) or \
                        initial_val != new_val:
                        if 'lose' in new_val:
