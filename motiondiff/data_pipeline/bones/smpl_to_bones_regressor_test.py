...
def vis(visualizer, smpl_motion, bones_joints_retarget, bones_joints_retarget_ik, video_path, fps=120):
...
parser.add_argument('--out_dir', type=str, default='dataset/bones/bones_to_smpl/bones_to_smpl_v14.6')
...
if __name__ == '__main__':
    ...
    for i in range(len(motion_files)):
        ...
        samples = motion.T.reshape(1, motion.shape[-1], 1, -1) #[batch, nfeat, 1, seq_len]
        samples = torch.from_numpy(samples).float().to('cuda')
        smpl_thetas, smpl_trans, _ = humanml_to_smpl(samples, mean=mean, std=std, smpl=smpl) # Z-up
        ...
        ''' IK '''
        phis = torch.tensor([1.0, 0.0], device='cuda').expand(smpl_thetas.shape[0], njoints, -1)
        if args.add_joints is not None:
            leaf_thetas = torch.eye(3, device='cuda').expand(smpl_thetas.shape[0], 8, -1, -1)
        else:
            leaf_thetas = torch.eye(3, device='cuda').expand(smpl_thetas.shape[0], 7, -1, -1)
        bones_rest_joints_seq = bones_rest_joints.repeat_interleave(smpl_thetas.shape[0], dim=0).to('cuda')
        ...
        err_ik = np.linalg.norm((bones_joints_retarget - bones_joints_retarget_ik).cpu().numpy(), axis=2).mean()
        ...
        video_path = os.path.join(args.out_dir, f'video_regress_test_{args.dataset}', f"{i:06d}.mp4")
        vis(visualizer, smpl_motion, bones_joints_retarget, bones_joints_retarget_ik, video_path, fps=30)
