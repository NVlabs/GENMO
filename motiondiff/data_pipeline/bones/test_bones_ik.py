...
if __name__ == '__main__':
    ...
    for idx, bvh_path in enumerate(tqdm(csv.move_bvh_path)):
        ...
        video_path = os.path.join(args.out_dir, os.path.basename(bvh_path).replace('bvh', 'mp4'))
        ...
        bones_joints_ori = torch.stack([bones_joints[..., 0], bones_joints[..., 2], bones_joints[..., 1]], dim=-1)
        ...
        bones_rotmats_ik, _, _ = batch_inverse_kinematics_transform_bones(bones_joints_noroot, None, phis, bones_rest_joints_seq, bones_children_map, bones_parents, leaf_thetas, False, add_eye=args.add_joints is not None)
        bones_joints_ik, _ = batch_rigid_transform(bones_rotmats_ik.to('cuda'), bones_rest_joints_seq, bones_parents)
        ...
        err_ik = np.linalg.norm((bones_joints_noroot - bones_joints_ik).cpu().numpy()[:, selected_joints], axis=2).mean()
        ...
        vis(visualizer, bones_joints, bones_joints_ik, video_path, fps=120 / args.downsample_rate)
