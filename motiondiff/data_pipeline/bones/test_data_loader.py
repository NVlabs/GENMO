        ...
        if data.shape[0] >= self.num_frames:
            ...
            data = np.concatenate(
                [
                    data,
                    np.zeros(
                        (self.num_frames - data.shape[0], data.shape[1]),
                        dtype=np.float32,
                    ),
                ],
                axis=0,
...
if __name__ == "__main__":
    ...
    for num_workers in [4, 8, 32, 64]:
        ...
        print(
            f"Running {num_epochs} epochs with batch_size={batch_size} and num_workers={num_workers} takes {time_dis_per_step:.2f}s per step when loading separately, takes {time_single_per_step:.2f}s per step when loading from single file"
        )
