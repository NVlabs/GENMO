...
class SMPLVisualizer(Visualizer3D):
    def __init__(
        ...
        **kwargs,
        ...
        if "smpl" in kwargs:
            ...
            self.smpl_faces = faces = np.hstack(
                [np.ones_like(faces[:, [0]]) * 3, faces]
            )
            del kwargs["smpl"]
    ...
    def update_smpl_seq(self, smpl_seq=None, mode="gt"):
        ...
        for i, (skel_name, pose_dict) in enumerate(smpl_seq.items()):
            ...
            joint_parents = (
                pose_dict.get("parents")
                if pose_dict.get("parents") is not None
                else self.joint_parents
            )
            if pose_dict.get("show_smpl_mesh"):
                pose_dict["smpl_actor"] = SMPLActor(
                    self.pl,
                    pose_dict["smpl_verts"][0].cpu().numpy(),
                    self.smpl_faces,
                    color="#e0c3fc",
                )
                ...
                pose_dict["skeleton_jpos"] = SkeletonActor(
                    self.pl, joint_parents, joint_color=colors[0], bone_color=colors[1]
                )
    ...
    def update_camera(self, interactive):
        if "gt" in self.smpl_seq:
            ...
            root_pos = (
                self.smpl_seq["smpl_joints"]["joints_pos"][self.fr, 0].cpu().numpy()
            )
            ...
            for skel_name, pose_dict in self.smpl_seq.items():
                ...
                if "smpl_actor" in pose_dict:
                    pose_dict["smpl_actor"].update_verts(
                        pose_dict["smpl_verts"][self.fr].cpu().numpy()
                    )
