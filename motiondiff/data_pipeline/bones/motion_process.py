...
import random
sys.path.append('./')
from motiondiff.utils.torch_transform import rotation_matrix_to_quaternion, quat_mul, quat_conjugate, get_y_heading_q
...
def foot_detect(positions, vel_thres, height_thresh):
    ...
    velfactor, heightfactor = np.array([vel_thres, vel_thres]), np.array([height_thresh, height_thresh])

    feet_l_x = (positions[1:, fid_l, 0] - positions[:-1, fid_l, 0]) ** 2
    feet_l_y = (positions[1:, fid_l, 1] - positions[:-1, fid_l, 1]) ** 2
    feet_l_z = (positions[1:, fid_l, 2] - positions[:-1, fid_l, 2]) ** 2
    feet_l_h = positions[:-1,fid_l,1]
    feet_l = np.logical_and((feet_l_x + feet_l_y + feet_l_z) < velfactor, feet_l_h < heightfactor).astype(float)
    ...
    feet_r = np.logical_and((feet_r_x + feet_r_y + feet_r_z) < velfactor, feet_r_h < heightfactor).astype(float)
...
def extract_features(joint_rot_mats, posed_joints, use_root_local_rot=False, use_joint_local_height=False, use_joint_local_vel=False):
    ...
    heading_quat = qmul_np(heading_quat, init_heading_quat_inv)  # normalize heading coordiante, so the heading is 0 for the first frame
    heading_quat_inv = qinv_np(heading_quat)
    ...
    if use_root_local_rot:
        rot_joint_quat = np.concatenate([root_quat_wo_heading[:, None], rot_joint_quat], axis=1)
    cont_6d_params, r_velocity, velocity, r_rot = get_cont6d_params(positions, rot_joint_quat, heading_quat, heading_quat_inv)
    ...
    if not use_joint_local_vel:
        local_vel = qrot_np(heading_quat_inv[:-1],
    ...
    data = root_data
    data = np.concatenate([data, ric_data, rot_data, local_vel, feet_l, feet_r], axis=-1)
    return data
...
def recover_from_ric(data, joints_num, r_rot_quat=None, r_pos=None, return_r_rot=False, use_global_joint_height=True):
...
def recover_from_ric_264(data, joints_num, r_rot_quat=None, r_pos=None, return_r_rot=True):
    '''
...
def recover_from_ric_with_joint_rot(data, joints, parents, root_dim=4, r_rot_quat=None, r_pos=None, return_r_rot=False, return_joint_rot=False):
    '''
    data : [..., seq_len, nfeat]
    '''
    ...
    if r_rot_quat is None or r_pos is None:
        if root_dim == 5:
            raise NotImplementedError('ERROR (recover_from_ric_with_joint_rot) : must feed in r_pos and r_rot_quat when using this function with global reps')
        r_rot_quat, r_pos = recover_root_rot_pos(data)
    else:
    ...
    final_rotmat = torch.cat([root_rot.unsqueeze(2), rotmat[:, :, 1:]], dim=2) # [bsize, num_frames, num_joints, 3, 3]
    final_rotmat_flat = final_rotmat.view(-1, joints_num, 3, 3)
    positions_flat = batch_rigid_transform(final_rotmat_flat, joints.repeat(final_rotmat_flat.shape[0], 1, 1), parents)[0]
    positions = positions_flat.reshape(final_rotmat.shape[:2] + positions_flat.shape[1:])
...
def recover_from_mano_with_joint_rot(data, root_dim=4, r_rot_quat=None, r_pos=None, return_r_rot=False, return_joint_rot=False):
    '''
    data : [..., seq_len, nfeat]
    '''
    rots = data[:, :, :102]
    ...
    if r_rot_quat is None or r_pos is None:
        if root_dim == 5:
            raise NotImplementedError('ERROR (recover_from_mano_with_joint_rot) : must feed in r_pos and r_rot_quat when using this function with global reps')
        r_rot_quat, r_pos = recover_root_rot_pos(data)
    else:
        r_rot_quat = r_rot_quat.reshape(-1, *r_rot_quat.shape[-2:])
        r_pos = r_pos.reshape(-1, *r_pos.shape[-2:])
...
def recover_local_joint_pos_with_joint_rot(data, joints, parents, r_pos, root_dim=4):
    ...
    # FK
    positions_flat = batch_rigid_transform(rotmat_flat, joints.repeat(rotmat_flat.shape[0], 1, 1), parents)[0]
    ...
    up_mask = torch.stack([torch.zeros(r_pos.shape[:-1]), torch.ones(r_pos.shape[:-1]), torch.zeros(r_pos.shape[:-1])], dim=-1) # y up
...
if __name__ == '__main__':
    ...
    bvh = np.load('/lustre/fs5/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets/bones_full_joints/P1/joint_rot_mats/talking_with_child_001__A034_M.npy')
    ...
    joint_rot_mats = np.load('/lustre/fs5/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets/bones_full_joints/P5/joint_rot_mats/sad_idle_turn_360_002__A174.npy')
    posed_joints = np.load('/lustre/fs5/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets/bones_full_joints/P5/posed_joints/sad_idle_turn_360_002__A174.npy')
    ...
    positions_rot = recover_from_ric_with_joint_rot(torch.tensor(data).float(), joints, parents)
    print('Done.')
    
    positions = torch.stack([-positions[..., 0], positions[..., 2], positions[..., 1]], dim=-1)
    positions_rot = torch.stack([-positions_rot[..., 0], positions_rot[..., 2], positions_rot[..., 1]], dim=-1)
    ...
    parents = [-1, 0, 1, 2, 3, 4, 5, 4, 7, 8, 9, 10, 10, 12, 4, 14, 15, 16, 17, 17, 19, 0, 21, 22, 23, 0, 25, 26, 27]
    ...
    sampled_paths = random.sample(os.listdir(os.path.join(data_dir, 'posed_joints')), 10)
    for path in sampled_paths:
        ...
        vis.save_animation_as_video(out_path, init_args={'smpl_seq': smpl_seq, 'mode': 'gt', 'camera': {'azimuth': -90}}, window_size=(800, 800), fps=20, crf=15)
