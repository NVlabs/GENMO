...
class SkeletonActor():
    ...
    def __init__(self, pl, joint_parents, joint_color='green', bone_color='yellow', joint_radius=0.03, bone_radius=0.02, visible=True, color_left_right=False):
        self.pl = pl
        ...
        for j, pa in enumerate(self.joint_parents):
            cur_joint_color = joint_color
            if color_left_right:
                # right
                if j in {7, 8, 9, 10, 11, 12, 13, 21, 22, 23, 24}:
                    cur_joint_color = "red"
                # left
                elif j in {14, 15, 16, 17, 18, 19, 20, 25, 26, 27, 28}:
                    cur_joint_color = "blue"
            # joint
            ...
            joint_actor = self.pl.add_mesh(joint_mesh, color=cur_joint_color, ambient=0.3, diffuse=0.5, specular=0.8, specular_power=5, smooth_shading=True)
            joint_actor = ActorWrapper(joint_actor, joint_mesh, self, cur_joint_color)
    ...
    def set_color(self, color):
        rgb_color = Color(color).float_rgb
        for actor in self.joint_actors:
            actor.GetProperty().SetColor(rgb_color)
        for actor in self.bone_actors:
            actor.GetProperty().SetColor(rgb_color)

    def set_joint_colors(self, color, actor_idx=None):
        actor_idx = set(range(len(self.joint_actors))) if actor_idx is None else set(actor_idx)
        rgb_color = Color(color).float_rgb
        for ai, actor in enumerate(self.joint_actors):
            if ai in actor_idx:
                actor.GetProperty().SetColor(rgb_color)
...
class SMPLVisualizer(Visualizer3D):
    ...
    def __init__(self, joint_parents=None, generator_func=None, device=torch.device('cpu'), show_skeleton=True, color_left_right=False, **kwargs):
        ...
        self.color_left_right = color_left_right
    ...
    def update_smpl_seq(self, smpl_seq=None, mode='gt'):
        ...
        for i, (skel_name, pose_dict) in enumerate(smpl_seq.items()):
            ...
            pose_dict['skeleton_jpos'] = SkeletonActor(self.pl, self.joint_parents, joint_color=colors[0], bone_color=colors[1], color_left_right=self.color_left_right)
    ...
    def init_scene(self, init_args):
        ...
        if 'camera' in init_args:
            for k, v in init_args['camera'].items():
                setattr(self.pl.camera, k, v)
    ...
    def update_camera(self, interactive):
        ...
        new_focal = np.array([root_pos[0], root_pos[1], 1.25])
    ...
    def update_scene(self):
        ...
        if self.show_skeleton:
            for i, (skel_name, pose_dict) in enumerate(self.smpl_seq.items()):
                colors = self.color_sequences[i % len(self.color_sequences)]
                pose_dict['skeleton_jpos'].update_joints(pose_dict['joints_pos'][self.fr].cpu().numpy())
                if 'foot_contacts' in pose_dict:
                    foot_idx = np.array([27, 28, 23, 24])
                    contact_idx = foot_idx[pose_dict['foot_contacts'][self.fr] == 1]
                    noncontact_idx = foot_idx[pose_dict['foot_contacts'][self.fr] == 0]
                    pose_dict['skeleton_jpos'].set_joint_colors("red", contact_idx)
                    pose_dict['skeleton_jpos'].set_joint_colors(colors[0], noncontact_idx)
                
        if 'texts' in pose_dict:
            self.pl.add_text('\n\n'.join(pose_dict['texts']), font_size=14, color='black', position='upper_left', font='courier')
...
if __name__ == '__main__':
    ...
    vis.show_animation(init_args={'smpl_seq': smpl_seq, 'mode': 'gt', 'camera': {'azimuth': -90}},
