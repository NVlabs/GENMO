...
class SkeletonActor:
    ...
    def set_joint_colors(self, color, actor_idx=None):
        actor_idx = (
            set(range(len(self.joint_actors))) if actor_idx is None else set(actor_idx)
        )
        rgb_color = Color(color).float_rgb
...
class SMPLVisualizer(Visualizer3D):
    ...
    def update_smpl_seq(self, smpl_seq=None, mode="gt"):
        ...
        for i, (skel_name, pose_dict) in enumerate(smpl_seq.items()):
            ...
            pose_dict["skeleton_jpos"] = SkeletonActor(
                ...
                bone_color=colors[1],
                color_left_right=self.color_left_right,
            )
    ...
    def init_scene(self, init_args):
        ...
        self.update_smpl_seq(
            init_args.get("smpl_seq", None), init_args.get("mode", "gt")
        )
        ...
        if self.show_skeleton:
            for i, (skel_name, pose_dict) in enumerate(self.smpl_seq.items()):
                ...
                pose_dict["skeleton_jpos"].update_joints(
                    pose_dict["joints_pos"][self.fr].cpu().numpy()
                )
                if "foot_contacts" in pose_dict:
                    ...
                    pose_dict["skeleton_jpos"].set_joint_colors(
                        colors[0], noncontact_idx
                    )

        if "texts" in pose_dict:
            self.pl.add_text(
                "\n\n".join(pose_dict["texts"]),
                font_size=14,
                color="black",
                position="upper_left",
                font="courier",
...
if __name__ == "__main__":
    ...
    smpl_seq = {"gt": {"joints_pos": posed_joints.float() * 0.01}}
