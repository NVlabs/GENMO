...
def hstack_videos(
    ...
    if write_text:
        ...
        draw_str = (
            f"drawtext=fontsize={text_size}:fontfile={font_file}:fontcolor={'black'}:text='{text1}':x=0:y=20:line_spacing={text_size // 2}"
...
for i in range(args.start, args.end, 100):
    ...
    try:
        ...
        mirror_joints, mirror_texts, mirror_parents = load_motion(
            mirror_bvh_fname, mirror_row
        )
        # print(mirror_texts)
        ...
        vis.save_animation_as_video(
            video_path_s,
            init_args={"smpl_seq": mirror_smpl_seq, "mode": "gt"},
            window_size=(800, 800),
            frame_dir=frame_dir,
            fps=120,
            crf=15,
