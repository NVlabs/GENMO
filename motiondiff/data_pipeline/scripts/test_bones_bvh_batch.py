...
argparser.add_argument('--offset', type=int, default=1)
...
out_dir = '/lustre/fs5/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets/bones_processed/vis_videos_v1'
tmp_dir = '/lustre/fs5/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets/bones_processed/tmp'
...
def hstack_videos(video1_path, video2_path, out_path, crf=15, verbose=True, text1=None, text2=None, text_color='black', text_size=20):
...
for i in range(args.start + args.offset, args.end + args.offset, 10):
    t0 = time.time()
    row = meta.iloc[i]
    ...
    print(f'{i}/({args.start}, {args.end}) start processing {bvh_fname}, time: {time.time() - t0:.2f}')
    ...
    if vis is None:
        vis = SMPLVisualizer(joint_parents=parents, distance=7, elevation=10, verbose=False, display_num=f':{args.job_id + 50}')
    ...
    vis.save_animation_as_video(video_path_f, init_args={'smpl_seq': smpl_seq, 'mode': 'gt', 'camera': {'azimuth': 0}}, window_size=(800, 800), frame_dir=frame_dir, fps=120, crf=15)
    vis.save_animation_as_video(video_path_s, init_args={'smpl_seq': smpl_seq, 'mode': 'gt', 'camera': {'azimuth': -90}}, window_size=(800, 800), frame_dir=frame_dir, fps=120, crf=15)
