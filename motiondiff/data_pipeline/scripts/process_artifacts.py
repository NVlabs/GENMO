...
skip_files = set()
with open(input_artifacts, 'r') as f_in, open(output_artifacts, 'w', newline='') as f_out:
...
skip_two_idx = [i for i, num_artifacts in skip_inds.items() if num_artifacts >= 2] + added_skips
print(f'{len(skip_two_idx)} motions >= 2 artifacts to skip, saving...')
np.save(skip_two_out_path, np.array(skip_two_idx, dtype=int))

skip_three_idx = [i for i, num_artifacts in skip_inds.items() if num_artifacts >= 3] + added_skips
