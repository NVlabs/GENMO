...
parser.add_argument(
    "--initial_csv_path",
    type=str,
    default="dataset/bones/bones_full_raw/Metadata_240416_v3.csv",
)
parser.add_argument(
    "--new_csv_path",
    type=str,
    default="dataset/bones/bones_full_raw_v14/metadata_240527_v014.csv",
)
parser.add_argument(
    "--log_path",
    type=str,
    default="dataset/bones/bones_full353_v2.0/meta_change_log_240527_v014.txt",
)
...
if __name__ == "__main__":
    ...
    initial_bvh_map = {
        path.split("/")[-1]: i for i, path in enumerate(initial_csv.move_bvh_path)
    }

    initial_bvh_paths = [
        path.split("/")[-1] for i, path in enumerate(initial_csv.move_bvh_path)
    ]
    new_bvh_paths = [
        path.split("/")[-1] for i, path in enumerate(new_csv.move_bvh_path)
    ]
    ...
    f_change.close()
    print(
        f"Find {num_updates} text updates, {num_not_found} bvh files not found in old meta"
    )
