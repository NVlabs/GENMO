...
if __name__ == "__main__":
    ...
    todo_path_list = csv.move_bvh_path[
    ...
    for batch_idx in tqdm(range(len(todo_path_list) // args.num_process + 1)):
        ...
        bvh_path_batch = [
            path
            for path in bvh_path_batch
            if os.path.exists(os.path.join(data_dir, path))
        ]
        if len(bvh_path_batch) < args.num_process:
            continue
