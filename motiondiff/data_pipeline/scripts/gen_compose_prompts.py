...
def compose_atom_sentence(
    all_gen_prompts, cur_prompt, template, keywords, all_keywords_list
):
    if len(template) == 0:
    ...
    if cur_subsentence in ["<lower_body>", "<lower_body_present>"]:
        for (
            lower_body_verb,
            lower_body_verb_present,
            lower_body_keywords,
        ) in lowerbody_action_lists:
        ...
        for (
            upper_body_verb,
            upper_body_verb_present,
            upper_body_keywords,
        ) in upperbody_action_lists:
        ...
        compose_atom_sentence(
            all_gen_prompts,
            (cur_prompt + cur_subsentence).strip(),
            template[1:],
            keywords,
            all_keywords_list,
        )
...
def compose_temporal_sentence(
    all_gen_prompts, cur_prompt, template, atom_dict, used_sentences, random_n=0
):
    if len(template) == 0:
        all_gen_prompts.append(cur_prompt)
        return
    ...
    if cur_subsentence[0] == "<" and cur_subsentence[-1] == ">":
        ...
        if random_n > 0:
            index = np.random.permutation(len(atom_dict[key]))[
                : min(random_n, len(atom_dict[key]))
            ]
        else:
...
for key, template in atom_sentence_templates.items():
    ...
    compose_atom_sentence(
        atom_sentences_dict[key], "", template, [], atom_keywords_dict[key]
    )
    ...
    print(
        f'[Atom] "{key}" total prompts: {len(atom_sentences_dict[key])}, filtered total prompts: {len(atom_sentences_filter_dict[key])}'
    )
