...
def process_bvh_files(fname, all_files_out, check_artifacts_out, lock):
    ...
    if CHECK_BONE_FLIP:
        ...
        rot_quat = torch.tensor(
            Rotation.from_matrix(joint_rot_mats.reshape((T * J, 3, 3)))
            .as_quat()
            .reshape((T, num_joints, 4))
        )
        rot_quat = torch.concatenate(
            [rot_quat[..., [3]], rot_quat[..., :3]], dim=-1
        )  # w,x,y,z format
        qdiff = qmul(qinv(rot_quat[:-1]), rot_quat[1:])
        rot_vel = 2 * torch.atan2(
            torch.norm(qdiff[..., 1:], dim=-1), qdiff[..., 0]
        )  # [T, 29]
    ...
    if CHECK_FOOT_SKATE:
        ...
        if torch.sum(toe_heights < FOOT_SKATE_HEIGHT_THRESH).item() > 0:
            floor_height = torch.median(
                toe_heights[toe_heights < FOOT_SKATE_HEIGHT_THRESH]
            )  # the floor may vary by a few cm and cause false postives
        else:
            floor_height = FOOT_SKATE_HEIGHT_THRESH
        # print(f'floor_height={floor_height}')
        toe_heights = toe_heights - floor_height
        contact_idx = torch.logical_and(
            toe_heights < FOOT_SKATE_HEIGHT_THRESH,
            toe_heights > -FOOT_SKATE_HEIGHT_THRESH,
        )
    ...
    if VIZ or check_results != "":
        ...
        frame_dir = os.path.join(
            out_dir, f"videos/frames/{os.path.basename(fname)[:-4]}"
        )
...
with multiprocessing.Pool(processes=num_proc) as pool:
    results = pool.starmap(
        process_bvh_files,
        [(arg, all_files_out, check_results_out, lock) for arg in file_names],
    )
