...
from motiondiff.utils.torch_transform import rotation_matrix_to_quaternion, get_y_heading_q
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO, handlers=[
        logging.FileHandler(os.path.join(args.out_dir, 'log', f'node_{args.node_idx}_log.txt')),
        logging.StreamHandler()
    ])

    if args.skel_version == '29-joints':
        parents = torch.LongTensor([-1,  0,  1,  2,  3,  4,  5,  4,  7,  8,  9, 10, 10, 12,  4, 14, 15, 16,
                17, 17, 19,  0, 21, 22, 23,  0, 25, 26, 27])
    elif args.skel_version == '27-joints':
        parents = torch.LongTensor([-1,  0,  1,  2,  3,  4,  5,  4,  7,  8,  9, 10, 10,  4, 13, 14, 15,
                16, 16,  0, 19, 20, 21,  0, 23, 24, 25])
    ...
    if args.gen_meta:
        ...
        for idx in range(len(row_list_all)):
            meta = {
                'id': idx, 
                'video': csv.move_bvh_path[row_list_all[idx]].replace('BVH', 'video').replace('bvh', 'mp4'),
                'conversations': [
                    {'from': 'human', 'value' : '<video>\n Can you describe the motion of the person in the video.'},
                    {'from': 'gpt', 'value': csv.content_natural_desc_1[row_list_all[idx]]},
                ]
            }
            if idx < num_train:
                meta_train += [meta]
                ...
                meta_test += [meta]
            meta_all += [meta]
        json.dump(meta_train, open(os.path.join(args.out_dir, 'train.json'), 'w'), indent=4)
        json.dump(meta_test, open(os.path.join(args.out_dir, 'test.json'), 'w'), indent=4)
    ...
    job_list_todo = [path for path in job_list_node if not os.path.exists(os.path.join(args.out_dir, path.replace('BVH', 'video').replace('bvh', 'mp4')))]
    # if not args.debug:
    #     random.shuffle(job_list_todo)

    visualizer = SMPLVisualizer(joint_parents=parents, distance=6, elevation=2, use_floor_texture=False, verbose=False, display_num=f':{args.node_idx + 500}')
    if args.debug_vis:
    ...
    for idx, bvh_path in enumerate(tqdm(job_list_todo)):
        ...
        smpl_seq = {
            'gt':{
                'joints_pos': bones_joints
            }
        }
        video_path = os.path.join(args.out_dir, bvh_path.replace('BVH', 'video').replace('bvh', 'mp4'))
        os.makedirs(os.path.dirname(video_path), exist_ok=True)
        frame_dir = f'out/bones/frames/{bvh_path}'
        visualizer.save_animation_as_video(video_path, init_args={'smpl_seq': smpl_seq, 'mode': 'gt', 'view_dir': 'Y-'}, window_size=(1000, 1000), frame_dir=frame_dir, fps=args.fps, enable_shadow=True)
        if args.resolution != 1000:
            video_path_tmp = video_path[:-4] + '_tmp.mp4'
            os.system(f"ffmpeg -i {video_path} -s {args.resolution}x{args.resolution} -c:a copy {video_path_tmp} -hide_banner -loglevel error")
