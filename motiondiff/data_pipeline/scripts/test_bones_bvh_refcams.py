...
out_dir = '/lustre/fs5/portfolios/nvr/projects/nvr_torontoai_humanmotionfm/datasets/bones_verify_refcams'
# bvh_dir = '../bones_data/foundation_data'
# out_dir = './out/bones_mirror'
...
def hstack_videos(video1_path, video2_path, out_path, crf=15, verbose=True, text1=None, text_size=12, vid_width=800):
    ...
    cmd = [FFMPEG_PATH, '-y', '-i', video1_path, '-i', video2_path, '-filter_complex', # 'hstack,format=yuv420p', 
           "color=d=0.1[c];[c][0]scale2ref[c][v1];[c][1]scale2ref=w=main_w+iw:h=max(main_h\,ih)[c][v2];[c][v1]overlay=0:0[ol-vid1];[ol-vid1][v2]overlay=W-w:0,setsar=1",
...
def load_motion(bvh_fname, row):
    ...
    texts = [
        ...
        break_sentence('4. ' + row['content_technical_description']),
        break_sentence('5. ' + row['content_short_description'])
    ]
    skeleton = Skeleton()
...
# # check if all have refcam
# print(f'Checking all refcams are present...')
...
#     refcam_fname = row['refcam_path']
#     full_refcam_path = os.path.join(bvh_dir, refcam_fname)
#     if not os.path.exists(full_refcam_path):
#         print(f'No refcam found for {bvh_fname}!')

print(f'Rendering refcams...')
for i in range(args.start, args.end, 200):
    if i > len(non_mirror_names)-1:
        break
    try:
        t0 = time.time()
        ...
        # load refcam
        full_refcam_path = os.path.join(bvh_dir, row['refcam_path'])
        if not os.path.exists(full_refcam_path):
            continue
        ...
        vis = SMPLVisualizer(joint_parents=parents, distance=7, elevation=10, verbose=False, color_left_right=True)
        smpl_seq = {
            'gt':{
        ...
        vis.save_animation_as_video(video_path_f, init_args={'smpl_seq': smpl_seq, 'mode': 'gt', 'camera': {'azimuth': -180}}, window_size=(800, 800), frame_dir=frame_dir, fps=120, crf=15, display_num=f':{args.job_id + 1000}')

        hstack_videos(video_path_f, full_refcam_path, video_path, crf=15, verbose=False, text_size=15, vid_width=800)
