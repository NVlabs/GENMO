...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, debug=False, rng=None):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
...
class TextOnlyDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file, rng=None):
        self.mean = mean
        self.std = std
        self.opt = opt
        self.data_dict = []
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", debug=False, rng=None, **kwargs):
        self.mode = mode
        ...
        if mode == 'text_only':
            self.t2m_dataset = TextOnlyDataset(self.opt, self.mean, self.std, self.split_file, rng=rng)
        else:
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
            self.t2m_dataset = Text2MotionDatasetV2(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, self.debug, rng=rng)
    ...
    def normalize_motion_for_eval(self, normed_motion):
        denormed_motion = self.t2m_dataset.inv_transform(normed_motion).astype(np.float32)
        return (denormed_motion - self.mean_for_eval) / self.std_for_eval
